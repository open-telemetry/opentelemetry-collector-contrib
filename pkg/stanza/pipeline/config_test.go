...
func TestBuildPipelineSuccess(t *testing.T) {
	cfg := Config{
		Operators: []operator.Config{
			{
				Builder: noop.NewOperatorConfig("noop"),
			},
		},
	}
...
func TestBuildAPipelineDefaultOperator(t *testing.T) {
	cfg := Config{
		Operators: []operator.Config{
			...
			{
				Builder: noop.NewOperatorConfig("noop1"),
			},
		},
...
func newDummyJSON(dummyID string) operator.Config {
	return operator.Config{Builder: json.NewParserConfig(dummyID)}
}

func newDummyCopy(dummyID string) operator.Config {
	return operator.Config{Builder: copy.NewOperatorConfig(dummyID)}
}
