...
func TestBookmarkOpenSyscallFailure(t *testing.T) {
	...
	defer mockWithDeferredRestore(&evtCreateBookmark, func(*uint16) (uintptr, error) { return 0, ErrorNotSupported })()
...
func TestBookmarkOpenSuccess(t *testing.T) {
	bookmark := NewBookmark()
	xml := "<bookmark><\\bookmark>"
	defer mockWithDeferredRestore(&evtCreateBookmark, func(*uint16) (uintptr, error) { return 5, nil })()
	err := bookmark.Open(xml)
...
func TestBookmarkUpdateFailureOnUpdateSyscall(t *testing.T) {
	event := NewEvent(1)
	bookmark := NewBookmark()
	defer mockWithDeferredRestore(&evtCreateBookmark, func(*uint16) (uintptr, error) { return 1, nil })()
	defer mockWithDeferredRestore(&evtUpdateBookmark, func(_, _ uintptr) error { return ErrorNotSupported })()
	err := bookmark.Update(event)
...
func TestBookmarkUpdateSuccess(t *testing.T) {
	...
	bookmark := NewBookmark()
	defer mockWithDeferredRestore(&evtCreateBookmark, func(*uint16) (uintptr, error) { return 5, nil })()
	defer mockWithDeferredRestore(&evtUpdateBookmark, func(_, _ uintptr) error { return nil })()
	err := bookmark.Update(event)
	require.NoError(t, err)
	require.Equal(t, uintptr(5), bookmark.handle)
}
...
func TestBookmarkRenderInvalidSyscall(t *testing.T) {
	bookmark := Bookmark{handle: 5}
	buffer := NewBuffer()
	defer mockWithDeferredRestore(&evtRender, func(_, _ uintptr, _, _ uint32, _ *byte) (*uint32, error) { return new(uint32), ErrorNotSupported })()
	_, err := bookmark.Render(buffer)
