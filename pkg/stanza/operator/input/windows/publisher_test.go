...
func TestPublisherOpenSyscallFailure(t *testing.T) {
	publisher := NewPublisher()
	provider := "provider"
	defer mockWithDeferredRestore(&evtOpenPublisherMetadata, func(uintptr, *uint16, *uint16, uint32, uint32) (uintptr, error) { return 0, ErrorNotSupported })()
...
func TestPublisherOpenSuccess(t *testing.T) {
	publisher := NewPublisher()
	provider := "provider"
	defer mockWithDeferredRestore(&evtOpenPublisherMetadata, func(uintptr, *uint16, *uint16, uint32, uint32) (uintptr, error) { return 5, nil })()
	err := publisher.Open(provider, nil)
