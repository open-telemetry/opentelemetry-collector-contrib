...
func TestSplitterConfig_Build(t *testing.T) {
	type fields struct {
	...
	tests := []struct {
		...
		{
			name: "Customize splitter configuration",
			fields: fields{
				EncodingConfig: NewEncodingConfig(),
				Multiline:      NewMultilineConfig(),
				Flusher:        NewFlusherConfig(),
		...
		{
			name: "Customize splitter configuration:Splitter is nil ",
			fields: fields{
				EncodingConfig: NewEncodingConfig(),
				Multiline:      NewMultilineConfig(),
				Flusher:        NewFlusherConfig(),
		...
		},
		{
			...
			fields: fields{
				EncodingConfig: NewEncodingConfig(),
				Multiline:      MultilineConfig{LineStartPattern: "START"},
				Flusher:        NewFlusherConfig(),
				Customization: CustomizedConfig{
					Enabled: true,
					Splitter: func(data []byte, atEOF bool) (advance int, token []byte, err error) {
