...
func TestProcess(t *testing.T) {
	...
	testCases := []struct {
		name   string
		config func() (*ParserConfig, error)
		input  *entry.Entry
		expect *entry.Entry
	}{
		{
...
func runTimeParseTest(t *testing.T, cfg *ParserConfig, ent *entry.Entry, buildErr bool, parseErr bool, expected time.Time) func(*testing.T) {
	return runLossyTimeParseTest(t, cfg, ent, buildErr, parseErr, expected, time.Duration(0))
}

func runLossyTimeParseTest(_ *testing.T, cfg *ParserConfig, ent *entry.Entry, buildErr bool, parseErr bool, expected time.Time, maxLoss time.Duration) func(*testing.T) {
...
func parseTimeTestConfig(layoutType, layout string, parseFrom entry.Field) *ParserConfig {
