...
func TestMemoryCacheGetAndAdd(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for expectKey, expectItem := range tc.input {
...
func TestMemoryCacheFIFOEviction(t *testing.T) {
	...
	// make sure the cache has the expected size
...
func TestThrottledCache(t *testing.T) {
	...
	for i := 1; i <= 6; i++ {
		...
		require.True(t, added, "expected add to succeed")
		require.False(t, c.limiter.throttled())
	}
	...
	added := c.add("7", "should be limited")
	require.True(t, c.limiter.throttled())
	require.True(t, added, "7th addition should still succeed")
