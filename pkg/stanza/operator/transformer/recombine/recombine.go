...
func init() {
	operator.Register("recombine", func() operator.Builder { return NewOperatorConfig("") })
}

// NewOperatorConfig creates a new recombine config with default values
...
// OperatorConfig is the configuration of a recombine operator
...
// Build creates a new Operator from a config
...
// Operator is an operator that combines a field from consecutive log entries
// into a single
...
func (r *Operator) Start(_ operator.Persister) error {
	go r.flushLoop()

	return nil
}

func (r *Operator) flushLoop() {
	for {
		select {
		case <-r.ticker.C:
			r.Lock()
...
func (r *Operator) Process(ctx context.Context, e *entry.Entry) error {
...
func (r *Operator) matchIndicatesFirst() bool {
	return r.matchFirstLine
}

func (r *Operator) matchIndicatesLast() bool {
	return !r.matchFirstLine
}

// addToBatch adds the current entry to the current batch of entries that will be combined
func (r *Operator) addToBatch(_ context.Context, e *entry.Entry, source string) {
...
func (r *Operator) flushUncombined(ctx context.Context) {
...
func (r *Operator) flushSource(source string) error {
