...
func init() {
	operator.Register("filter", func() operator.Builder { return NewOperatorConfig("") })
}
...
// NewOperatorConfig creates a filter operator config with default values
...
// OperatorConfig is the configuration of a filter operator
...
// Operator is an operator that filters entries based on matching expressions
...
func (f *Operator) Process(ctx context.Context, entry *entry.Entry) error {
