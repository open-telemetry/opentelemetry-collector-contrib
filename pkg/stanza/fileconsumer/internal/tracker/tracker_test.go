...
func TestFileTrackerUnmatchedSliceReuse(t *testing.T) {
	...
	require.Equal(t, 0, len(ft.unmatchedFps))
	require.Equal(t, 0, len(ft.unmatchedFiles))
...
func TestNoStateTrackerMemoryReuse(t *testing.T) {
	...
	require.Equal(t, 0, len(nst.unmatchedFiles))
	require.Equal(t, 0, len(nst.unmatchedFps))
}
