...
func testManagerWithSink(t *testing.T, cfg *Config, sink *emittest.Sink, opts ...Option) *Manager {
	...
	input.tracker = tracker.NewFileTracker(set, cfg.MaxBatches, archive.NewArchive(context.Background(), set.Logger, cfg.PollsToArchive, testutil.NewUnscopedMockPersister()))
