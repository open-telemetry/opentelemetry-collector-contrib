...
func TestNewSortRegexFilterOptionErr(t *testing.T) {
	var err error
...
type withOpts func(t *testing.T, ascending bool) []RegexFilterOption
...
func TestSort(t *testing.T) {
	cases := []struct {
		...
		{
			name: "Numeric",
			withOpts: func(t *testing.T, ascending bool) []RegexFilterOption {
				o, err := SortNumeric("num", ascending)
				require.NoError(t, err)
				return []RegexFilterOption{o}
			},
			regex:           `(?P<num>\d{2}).*log`,
	...
	for _, tc := range cases {
		for _, ascending := range []bool{true, false} {
			t.Run(fmt.Sprintf("%s/%t", tc.name, ascending), func(t *testing.T) {
				f := NewRegexFilter(regexp.MustCompile(tc.regex), tc.withOpts(t, ascending)...)
