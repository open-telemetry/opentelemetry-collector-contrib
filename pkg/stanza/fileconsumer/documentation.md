...
| `filelog.skipUnchangedPathByMtime` | alpha | When enabled, a file whose path+mtime is unchanged from a previously tracked reader (within the `knownFiles` generation window) is skipped during the poll: no open, no fingerprint, no read. The retained reader metadata is promoted into the current generation so it ages on the same schedule as if it had been processed. | v0.154.0 | N/A | [Link](https://github.com/open-telemetry/opentelemetry-collector-contrib/issues/47861) |
