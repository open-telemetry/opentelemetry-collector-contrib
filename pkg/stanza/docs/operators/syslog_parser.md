...
| `protocol`                           | required         | The protocol to parse the syslog messages as. Options are `rfc3164`, `rfc5424`, and `auto`. `auto` tries RFC5424 first, then RFC3164. |
...
| `enable_octet_counting`              | `false`          | Wether or not to enable [RFC 6587](https://www.rfc-editor.org/rfc/rfc6587#section-3.4.1) Octet Counting on syslog parsing (Syslog RFC 5424 or auto only).  |
...
| `non_transparent_framing_trailer`    | `nil`            | The framing trailer, either `LF` or `NUL`, when using [RFC 6587](https://www.rfc-editor.org/rfc/rfc6587#section-3.4.2) Non-Transparent-Framing (Syslog RFC 5424 or auto only). |
