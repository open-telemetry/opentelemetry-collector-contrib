...
| `protocol`                           | required         | The protocol to parse the syslog messages as. Options are `rfc3164`, `rfc5424`, and `none`. Use `none` for raw syslog data that does not conform to RFC3164 or RFC5424; the message is stored as-is, except that a leading PRI header (e.g. `<34>`) is decoded into the `priority`, `facility`, `facility_text`, and severity fields when present. |
