...
| `discovery_interval`            | `0` (walk every poll)                | When set, the file-discovery glob walk runs at most once per this interval (rounded up to a whole number of `poll_interval`s) and its result is reused on the polls in between, decoupling the discovery-walk cost from read latency.                             |
