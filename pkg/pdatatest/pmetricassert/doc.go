...
// [WithAttributeExists] and [WithAttributeRegex] generate these matchers when
// writing snapshots.
//
...
// exactly by default, which the /exact suffix spells out. They may use the
// /include suffix to assert that the listed items are present while allowing
// additional ones, or the /count suffix to assert the size of the collection.
//
