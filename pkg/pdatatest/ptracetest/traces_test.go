...
func TestCompareTraces(t *testing.T) {
	tcs := []struct {
		...
		},
		{
			...
			withoutOptions: multierr.Combine(
				errors.New("resource \"map[host.name:node1]\": scope \"collector\": span \"\": event \"event1\": attributes don't match expected: map[eventkey1:value1], actual: map[eventkey1:value2]"),
			),
		},
		{
			name: "scopespans-spans-events-dropattributescount-mismatch",
			withoutOptions: multierr.Combine(
				errors.New("resource \"map[host.name:node1]\": scope \"collector\": span \"\": event \"event1\": dropped attributes count doesn't match expected: 1, actual: 0"),
			),
		},
		{
			name: "scopespans-spans-events-name-mismatch",
			withoutOptions: multierr.Combine(
				errors.New("resource \"map[host.name:node1]\": scope \"collector\": span \"\": event \"event1\": name doesn't match expected: event2, actual: event1"),
			),
		},
		{
			name: "scopespans-spans-events-timestamp-mismatch",
			withoutOptions: multierr.Combine(
				errors.New("resource \"map[host.name:node1]\": scope \"collector\": span \"\": event \"event1\": name doesn't match expected: event2, actual: event1; resource \"map[host.name:node1]\": scope \"collector\": span \"\": event \"event1\": timestamp doesn't match expected: 11651379494838206400, actual: 11651379494838206464"),
			),
		},
		{
		...
		},
		{
			...
			withoutOptions: multierr.Combine(
				errors.New("resource \"map[host.name:node1]\": scope \"collector\": span \"\": number of links doesn't match expected: 1, actual: 0"),
			),
		},
		{
			name: "scopespans-spans-links-traceid-mismatch",
			withoutOptions: multierr.Combine(
				errors.New("resource \"map[host.name:node1]\": scope \"collector\": span \"\": link \"\": trace ID doesn't match expected: 8c8b1765a7b0acf0b66aa4623fcb7bd5, actual: "),
			),
		},
		{
			name: "scopespans-spans-links-spanid-mismatch",
			withoutOptions: multierr.Combine(
				errors.New("resource \"map[host.name:node1]\": scope \"collector\": span \"\": link \"\": span ID doesn't match expected: fd0da883bb27cd6b, actual: "),
			),
		},
		{
			name: "scopespans-spans-links-tracestate-mismatch",
			withoutOptions: multierr.Combine(
				errors.New("resource \"map[host.name:node1]\": scope \"collector\": span \"\": link \"fd0da883bb27cd6b\": trace state doesn't match expected: placeholder, actual: "),
			),
		},
		{
			name: "scopespans-spans-links-attributes-mismatch",
			withoutOptions: multierr.Combine(
				errors.New("resource \"map[host.name:node1]\": scope \"collector\": span \"\": link \"fd0da883bb27cd6b\": attributes don't match expected: map[testKey1:teststringvalue1], actual: map[testKey1:teststringvalue2]"),
			),
		},
		{
			name: "scopespans-spans-links-dropattributescount-mismatch",
			withoutOptions: multierr.Combine(
				errors.New("resource \"map[host.name:node1]\": scope \"collector\": span \"\": link \"fd0da883bb27cd6b\": dropped attributes count doesn't match expected: 1, actual: 0"),
			),
		},
		{
