...
func TestClearAfterCopy(t *testing.T) {
	...
	wme := &wrapperMetricsExporter{excludeServiceAttributes: false, clearAfterCopy: true}
	outMd := wme.convertToMetricsAttributes(md)

	// After converting resource to labels — the wrapper mutates md in-place
	// (Capabilities{MutatesData: true}); outMd and md alias the same underlying data.
	assert.Equal(t, 0, outMd.ResourceMetrics().At(0).Resource().Attributes().Len())
	assert.Equal(t, 2, outMd.ResourceMetrics().At(0).ScopeMetrics().At(0).Metrics().At(0).Sum().DataPoints().At(0).Attributes().Len())
