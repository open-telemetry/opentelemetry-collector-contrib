...
func TestConvertPprofToProfiles_SetsScopeNameAndVersion_WhenProvided(t *testing.T) {
	prof := &profile.Profile{
		SampleType: []*profile.ValueType{{Type: "cpu", Unit: "nanoseconds"}},
		Sample: []*profile.Sample{
			{Value: []int64{1}},
	...
	out, err := ConvertPprofToProfiles(prof, ScopeInfo{
		Name:    "otelcol/pprofreceiver/httpscraper",
		Version: "1.2.3",
	})
	require.NoError(t, err)

	sp := out.ResourceProfiles().At(0).ScopeProfiles().At(0)
	require.Equal(t, "otelcol/pprofreceiver/httpscraper", sp.Scope().Name())
	require.Equal(t, "1.2.3", sp.Scope().Version())
}

func TestConvertPprofToProfiles_DefaultsScopeName_WhenOmitted(t *testing.T) {
	prof := &profile.Profile{
		SampleType: []*profile.ValueType{{Type: "cpu", Unit: "nanoseconds"}},
		Sample: []*profile.Sample{
			{Value: []int64{1}},
	...
	sp := out.ResourceProfiles().At(0).ScopeProfiles().At(0)
	require.Equal(t, "github.com/open-telemetry/opentelemetry-collector-contrib/pkg/translator/pprof", sp.Scope().Name())
	require.Empty(t, sp.Scope().Version())
}
