...
func Test_newIDExprFunc_rawBytes(t *testing.T) {
	target, err := ottl.NewTestingLikeLiteralGetter(true, makeIDGetter([]byte{1, 2, 3, 4, 5, 6, 7, 8}))
...
func Test_newIDExprFunc_hexBytes(t *testing.T) {
	target, err := ottl.NewTestingLikeLiteralGetter(true, makeIDGetter([]byte("0102030405060708090a0b0c0d0e0f10")))
	require.NoError(t, err)
...
func Test_newIDExprFunc_literalInitErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			target, err := ottl.NewTestingLikeLiteralGetter(true, makeIDGetter(tt.value))
			require.NoError(t, err)
...
func Test_newIDExprFunc_literalStringInitErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			target, err := ottl.NewTestingLikeLiteralGetter(true, makeIDGetter([]byte(tt.value)))
			require.NoError(t, err)
