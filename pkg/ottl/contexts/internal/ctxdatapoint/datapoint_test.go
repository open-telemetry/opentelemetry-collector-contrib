...
func Test_AddDataPoint_DataPointTypes(t *testing.T) {
	tests := []struct {
		name     string
		data     any
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, ctxdatapoint.AddDataPoint(encoder, "datapoint", tt.data))

			if tt.expected == nil {
				assert.NotContains(t, encoder.Fields, "datapoint")
				return
			}
