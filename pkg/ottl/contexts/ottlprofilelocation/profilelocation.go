...
const ContextName = ctxprofilelocation.Name

var (
	_ ctxresource.Context        = TransformContext{}
	_ ctxscope.Context           = TransformContext{}
	_ ctxprofilelocation.Context = TransformContext{}
...
func (tCtx TransformContext) MarshalLogObject(encoder zapcore.ObjectEncoder) error {
	...
	err = errors.Join(err, encoder.AddObject("location", logprofile.ProfileLocation{Location: tCtx.location, Dictionary: tCtx.dictionary}))
...
func NewTransformContext(location pprofile.Location, dictionary pprofile.ProfilesDictionary, instrumentationScope pcommon.InstrumentationScope, resource pcommon.Resource, scopeProfiles pprofile.ScopeProfiles, resourceProfiles pprofile.ResourceProfiles, options ...TransformContextOption) TransformContext {
	tc := TransformContext{
		location:             location,
...
// GetProfileLocation returns the location from the TransformContext.
func (tCtx TransformContext) GetProfileLocation() pprofile.Location {
	return tCtx.location
}
...
func pathExpressionParser(cacheGetter ctxcache.Getter[TransformContext]) ottl.PathExpressionParser[TransformContext] {
	return ctxcommon.PathExpressionParser(
		...
		map[string]ottl.PathExpressionParser[TransformContext]{
			...
			ctxprofilelocation.Name: ctxprofilelocation.PathGetSetter[TransformContext],
		})
}
