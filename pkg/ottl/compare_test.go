	...
	for _, tt := range tests {
		for _, op := range ops {
			t.Run(fmt.Sprintf("%s %v", tt.name, op), func(t *testing.T) {
				p := NewParser[interface{}](WithTelemetrySettings[interface{}](componenttest.NewNopTelemetrySettings()))
...
func BenchmarkCompareEQInt64(b *testing.B) {
	testParser := NewParser[interface{}](WithTelemetrySettings(componenttest.NewNopTelemetrySettings()))
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
