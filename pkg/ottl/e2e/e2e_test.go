...
func Test_e2e_ottl_features(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "Using Contains case insensitive",
			statement: `set(attributes["test"], "pass") where Contains(body, ["tionA", "tionB"], false)`,
			want: func(tCtx ottllog.TransformContext) {
				tCtx.GetLogRecord().Attributes().PutStr("test", "pass")
			},
		},
		{
			name:      "Using Contains case sensitive",
			statement: `set(attributes["test"], "pass") where Contains(body, ["A"], true)`,
			want: func(tCtx ottllog.TransformContext) {
				tCtx.GetLogRecord().Attributes().PutStr("test", "pass")
			},
		},
