...
The [CHANGELOG.md](./CHANGELOG.md) file in this repo is autogenerated from yaml files in the `./changelog` directory.
All `./changelog/*.yaml` files are removed during the release process, as they are added to `CHANGELOG.md`.

Your pull-request should add a new `.yaml` file to the `./changelog` directory. The name of your file must be unique since the last release.

**Recommended Steps**
1. Create an entry file using `make chlog-new`. This generates a file based on your current branch (e.g. `./changelog/my-branch.yaml`)
2. Fill in all fields in the new file
3. Run `make chlog-validate` to ensure the new file is valid
4. Commit and push the file

Alternately, copy `./changelog/TEMPLATE.yaml`, or just create your file from scratch.
