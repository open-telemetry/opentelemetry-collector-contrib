...
func TestParquetVPCFlow(t *testing.T) {
	...
	// 2. Start gofakes3 with virtual-hosted-style bucket support enabled.
	// WithHostBucket(true) makes gofakes3 extract the bucket name from the HTTP
	// Host header (e.g. "test-bucket.localhost:PORT" → bucket "test-bucket").
	fakeS3 := gofakes3.New(s3mem.New(), gofakes3.WithHostBucket(true))
	s3Server := httptest.NewServer(fakeS3.Server())
	t.Cleanup(s3Server.Close)

	// Derive a localhost-based URL so that "test-bucket.localhost:PORT" is
	// resolvable by the OS (*.localhost → 127.0.0.1 on most Linux systems via
	// the nss-myhostname / systemd-resolved wildcard).
	serverParsed, err := url.Parse(s3Server.URL)
	require.NoError(t, err)
	s3EndpointURL := "http://localhost:" + serverParsed.Port()
	...
	// 4. Upload the Parquet file to gofakes3 using the same SDK defaults the
	// receiver will use (virtual-hosted style, no UsePathStyle override).
	// "test-bucket.localhost:PORT" resolves to 127.0.0.1 via the OS wildcard.
