...
func (m *metricRiakNodeOperationCount) recordDataPoint(start pdata.Timestamp, ts pdata.Timestamp, val int64, operationAttributeValue string) {
	if !m.settings.Enabled {
		return
	}
	dp := m.data.Sum().DataPoints().AppendEmpty()
...
func (m *metricRiakNodeOperationTimeMean) init() {
	...
	m.data.SetDescription("The mean time between request and response for operations performed by the node.")
	m.data.SetUnit("ms")
	m.data.SetDataType(pdata.MetricDataTypeGauge)
	m.data.Gauge().DataPoints().EnsureCapacity(m.capacity)
}

func (m *metricRiakNodeOperationTimeMean) recordDataPoint(start pdata.Timestamp, ts pdata.Timestamp, val int64, operationAttributeValue string) {
	if !m.settings.Enabled {
		return
	}
...
func (m *metricRiakNodeReadRepairCount) init() {
	...
	m.data.SetDescription("The number of read repairs performed by the node over the last minute.")
...
func (m *metricRiakVnodeOperationCount) recordDataPoint(start pdata.Timestamp, ts pdata.Timestamp, val int64, operationAttributeValue string) {
	if !m.settings.Enabled {
		return
	}
	dp := m.data.Sum().DataPoints().AppendEmpty()
...
func (mb *MetricsBuilder) RecordRiakNodeOperationCountDataPoint(ts pdata.Timestamp, val int64, operationAttributeValue string) {
	mb.metricRiakNodeOperationCount.recordDataPoint(mb.startTime, ts, val, operationAttributeValue)
}

// RecordRiakNodeOperationTimeMeanDataPoint adds a data point to riak.node.operation.time.mean metric.
func (mb *MetricsBuilder) RecordRiakNodeOperationTimeMeanDataPoint(ts pdata.Timestamp, val int64, operationAttributeValue string) {
	mb.metricRiakNodeOperationTimeMean.recordDataPoint(mb.startTime, ts, val, operationAttributeValue)
}
...
func (mb *MetricsBuilder) RecordRiakVnodeOperationCountDataPoint(ts pdata.Timestamp, val int64, operationAttributeValue string) {
	mb.metricRiakVnodeOperationCount.recordDataPoint(mb.startTime, ts, val, operationAttributeValue)
}
...
var Attributes = struct {
	...
	Operation string
	// RiakNodeName (The name of the Riak node.)
	RiakNodeName string
}{
	"state",
	"riak.node.name",
