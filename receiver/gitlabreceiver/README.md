...
The GitLab receiver processes GitLab webhook events and converts them into OpenTelemetry telemetry signals:

- **Traces**: Generated from [`pipeline`](https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html#pipeline-events) events
- **Metrics**: Generated from [`job`](https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html#job-events) events
...
The GitLab receiver generates metrics from GitLab [`job`](https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html#job-events) webhook events. Job events provide detailed information about CI/CD job execution, including duration, queuing time, and status.

### Supported Metrics

The receiver generates the following metrics from job events:

#### `gitlab.job.duration` (Gauge)
...
- `service.name`: The GitLab project name (e.g., `gitlab-org/gitlab-test`)
- `gitlab.project.id`: The GitLab project ID
- `gitlab.project.name`: The GitLab project name

### Job Event Processing

The receiver processes job events with the following behavior:

1. **Completed Jobs**: Only jobs with status `success`, `failed`, `canceled`, or `skipped` are processed
2. **Incomplete Jobs**: Jobs with status `running` or `pending` are skipped (no metrics generated)
3. **Missing Data**: Jobs without required timestamps (`started_at` or `finished_at`) are skipped

### Configuration

The same webhook configuration used for traces applies to metrics. No additional configuration is required.
...
When a job completes, you'll receive metrics like:
...
- **Job Status**: Only completed jobs generate metrics. Running or pending jobs are ignored
- **Timestamp Requirements**: Jobs must have both `started_at` and `finished_at` timestamps to generate metrics
- **Queued Duration**: The `queued_duration` metric is only generated when the value is greater than 0