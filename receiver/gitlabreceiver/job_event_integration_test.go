...
func TestJobEventMetricAttributes(t *testing.T) {
	...
	attrs := dp.Attributes()
	_, found := attrs.Get("gitlab.job.id")
	require.True(t, found)
	_, found = attrs.Get("gitlab.job.name")
	require.True(t, found)
	_, found = attrs.Get("gitlab.job.stage")
	require.True(t, found)
	_, found = attrs.Get("gitlab.job.status")
	require.True(t, found)
	_, found = attrs.Get("gitlab.job.runner.id")
	require.True(t, found)
	_, found = attrs.Get("gitlab.job.runner.description")
	require.True(t, found)
