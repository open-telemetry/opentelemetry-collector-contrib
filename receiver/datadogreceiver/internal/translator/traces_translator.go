...
func processGRPCSpan(span *pb.Span, newSpan *ptrace.Span) {
	...
	newSpan.Attributes().PutStr(string(conventions138.RPCSystemKey), conventions138.RPCSystemGRPC.Value.AsString())
	newSpan.Attributes().PutInt(string(conventions138.RPCGRPCStatusCodeKey), int64(span.GetError()))
	...
	if method != "" {
		...
		newSpan.Attributes().PutStr(string(conventions138.RPCServiceKey), service)
...
func processAWSSdkSpan(span *pb.Span, newSpan *ptrace.Span) {
	...
	newSpan.Attributes().PutStr(string(conventions138.RPCSystemKey), "aws-api")
