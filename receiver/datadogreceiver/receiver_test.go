...
func TestDatadogMetricsV1_EndToEnd(t *testing.T) {
	...
	assert.InDelta(t, 0.7, metric.Sum().DataPoints().At(0).DoubleValue(), 0.01)
...
func TestDatadogMetricsV2_EndToEnd(t *testing.T) {
	...
	assert.False(t, metric.Sum().IsMonotonic())
	assert.Equal(t, pcommon.Timestamp(1636629071*1_000_000_000), metric.Sum().DataPoints().At(0).Timestamp())
	assert.InDelta(t, 1.5, metric.Sum().DataPoints().At(0).DoubleValue(), 0.01)
	...
	assert.InDelta(t, 2.0, metric.Sum().DataPoints().At(1).DoubleValue(), 0.01)
...
func TestDatadogSketches_EndToEnd(t *testing.T) {
	...
	assert.InDelta(t, 11.0, metric.ExponentialHistogram().DataPoints().At(0).Sum(), 0.01)
	assert.InDelta(t, -6.0, metric.ExponentialHistogram().DataPoints().At(0).Min(), 0.01)
	assert.InDelta(t, 6.0, metric.ExponentialHistogram().DataPoints().At(0).Max(), 0.01)
