...
func Test_splunkhecReceiver_handleReq(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := f.CreateLogs(context.Background(), receivertest.NewNopSettingsWithType(metadata.Type), config, sink)
			assert.NoError(t, err)
			rcv, err := f.CreateMetrics(context.Background(), receivertest.NewNopSettingsWithType(metadata.Type), config, metricsSink)
			assert.NoError(t, err)
...
func Test_splunkhecReceiver_AccessTokenPassthrough(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.metric {
				exporter, err := factory.CreateMetrics(context.Background(), exportertest.NewNopSettingsWithType(metadata.Type), exporterConfig)
				...
				exporter, err := factory.CreateLogs(context.Background(), exportertest.NewNopSettingsWithType(metadata.Type), exporterConfig)
				assert.NoError(t, exporter.Start(context.Background(), nil))
				defer func() {
					require.NoError(t, exporter.Shutdown(context.Background()))
				}()
...
func Test_Logs_splunkhecReceiver_IndexSourceTypePassthrough(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rcv, err := newReceiver(receivertest.NewNopSettingsWithType(metadata.Type), *cfg)
			assert.NoError(t, err)
			rcv.logsConsumer = exporter
...
func Test_splunkhecReceiver_Start(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			sink := new(consumertest.LogsSink)
			rcv, err := newReceiver(receivertest.NewNopSettingsWithType(metadata.Type), *tt.getConfig())
			assert.NoError(t, err)
			rcv.logsConsumer = sink
