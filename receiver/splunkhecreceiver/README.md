...
* `ack_extension` (no default): Specifies the ack extension ID the receiver should use. If left blank, ack is turned off.
* `ack_path` (default = '/services/collector/ack'): The path ack extension will listen on for ack requests, if the extension is enabled.
Example:
