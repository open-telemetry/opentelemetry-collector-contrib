...
func (e *expVarScraper) start(ctx context.Context, host component.Host) error {
	client, err := e.cfg.ClientConfig.ToClient(ctx, host.GetExtensions(), e.set.TelemetrySettings)
	if err != nil {
		return err
	}
	e.client = client
...
func (e *expVarScraper) scrape(ctx context.Context) (pmetric.Metrics, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, e.cfg.ClientConfig.Endpoint, http.NoBody)
	if err != nil {
