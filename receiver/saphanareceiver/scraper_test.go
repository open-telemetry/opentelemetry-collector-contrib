...
func TestScraper(t *testing.T) {
	...
	sc, err := newSapHanaScraper(receivertest.NewNopSettingsWithType(metadata.Type), createDefaultConfig().(*Config), &testConnectionFactory{dbWrapper})
...
func TestDisabledMetrics(t *testing.T) {
	...
	sc, err := newSapHanaScraper(receivertest.NewNopSettingsWithType(metadata.Type), cfg, &testConnectionFactory{dbWrapper})
	require.NoError(t, err)
