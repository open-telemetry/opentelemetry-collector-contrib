...
func TestConvertToOtel(t *testing.T) {
	...
	expectedAttributes.PutStr(string(conventions.SourceAddressKey), "192.168.1.1")
	expectedAttributes.PutInt(string(conventions.SourcePortKey), 0)
	expectedAttributes.PutStr(string(conventions.DestinationAddressKey), "192.168.1.2")
	expectedAttributes.PutInt(string(conventions.DestinationPortKey), 2055)
	expectedAttributes.PutStr(string(conventions.NetworkTransportKey), getTransportName(6))
	expectedAttributes.PutStr(string(conventions.NetworkTypeKey), getEtypeName(0x800))
...
func TestEmptyConvertToOtel(t *testing.T) {
	...
	expectedAttributes := pcommon.NewMap()
	expectedAttributes.PutStr(string(conventions.SourceAddressKey), "invalid IP")
	expectedAttributes.PutInt(string(conventions.SourcePortKey), 0)
	expectedAttributes.PutStr(string(conventions.DestinationAddressKey), "invalid IP")
	expectedAttributes.PutInt(string(conventions.DestinationPortKey), 0)
	expectedAttributes.PutStr(string(conventions.NetworkTransportKey), "hopopt")
	expectedAttributes.PutStr(string(conventions.NetworkTypeKey), "unknown")
