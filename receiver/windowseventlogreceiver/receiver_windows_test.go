...
func TestDynamicBatchSizing(t *testing.T) {
	tests := []struct {
		name           string
		initialReads   int
		expectedReads  []int // sequence of expected batch sizes after errors
		expectedEvents int   // total events we expect to receive
	}{
		{
			name:           "halves_batch_size_on_rpc_error",
			initialReads:   100,
			expectedReads:  []int{50, 25},
			expectedEvents: 75, // 50 + 25 events total
		},
		{
			name:           "minimum_batch_size_one",
			initialReads:   4,
			expectedReads:  []int{2, 1, 1}, // Should stay at 1 and not go lower
			expectedEvents: 4,
		},
		{
			name:           "large_initial_batch",
			initialReads:   1000,
			expectedReads:  []int{500, 250},
			expectedEvents: 750,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cfg := createTestConfig()
			cfg.InputConfig.MaxReads = tt.initialReads
			sink := new(consumertest.LogsSink)
			...
			// Simulate RPC_S_INVALID_BOUND errors and verify batch size adjustments
			input := receiver.(*adapter.Receiver).Input.(*windows.Input)

			// Verify initial batch size
			assert.Equal(t, tt.initialReads, input.currentMaxReads)

			// Simulate errors and check batch size adjustments
			for _, expectedRead := range tt.expectedReads {
				// Trigger RPC_S_INVALID_BOUND error
				events, err := input.subscription.Read(input.currentMaxReads)
				require.Error(t, err)
				require.ErrorIs(t, err, windows.RPC_S_INVALID_BOUND)

				// Verify batch size was halved
				assert.Equal(t, expectedRead, input.currentMaxReads)

				// Process some events successfully
				assert.Eventually(t, func() bool {
					return sink.LogRecordCount() >= tt.expectedEvents
...
func TestRecoveryFromRPCError(t *testing.T) {
	ctx := context.Background()
	factory := newFactoryAdapter()
	createSettings := receivertest.NewNopSettings()

	cfg := createTestConfig()
	cfg.InputConfig.MaxReads = 100
	sink := new(consumertest.LogsSink)
	...
	// Initial batch size should be set
	assert.Equal(t, 100, input.currentMaxReads)

	// Simulate RPC_S_INVALID_BOUND error
	events, err := input.subscription.Read(input.currentMaxReads)
	require.Error(t, err)
	require.ErrorIs(t, err, windows.RPC_S_INVALID_BOUND)
	...
	// Verify we can still process events after recovery
	assert.Eventually(t, func() bool {
