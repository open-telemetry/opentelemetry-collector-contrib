...
- OAuth coverage lives in `test/integration/kafkaoauth` (tagged `integration`). The suite spins up Keycloak and a Kafka broker, then verifies exporter-to-receiver round trips using the franz-go client.
- The tests require Docker and pre-pulled Kafka/Keycloak images (testcontainers may still pull helper images like `testcontainers/ryuk` if missing). Run them from the integration module:
...
- Scenarios covered from the exporter module:
  - Happy path round trip
  - Token refresh with short-lived access tokens
  - Negative path: disabled Keycloak client cannot mint tokens
- Approximate runtime: typically under ~1 minute per run on a healthy Docker setup (with images already pulled).
...
When Kafka brokers expect a generic SASL `OAUTHBEARER` token, configure the
[`oauth2client`](../../extension/oauth2clientauthextension) extension and reference it from the
receiver so that tokens are fetched and refreshed automatically.
...
If you already have a process that writes tokens to a file, point the receiver at that file instead.
