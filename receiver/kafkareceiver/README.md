...
- `encoding` (default = otlp_proto): The encoding of the payload received from kafka. Available encodings:
  ...
  - `json`: (logs only) the payload is decoded as JSON and inserted as the body of a log record. The unmarshaler expects a top level JSON object, and does not support a top level JSON array.
