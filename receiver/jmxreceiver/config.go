...
const (
	...
	// receiver config (per-platform `password_file` validation + keystore/truststore password lookup via the password file).
...
type Config struct {
	...
	TargetSource string `mapstructure:"target_source,omitempty"`
	// Comma-separated list of paths to custom YAML metrics definition,
	// mandatory when TargetSystem is not set.
	// Supported by: jmx-scraper
	JmxConfigs string `mapstructure:"jmx_configs,omitempty"`
	...
	// Wired into the Java `otel.jmx.remote.registry.ssl` property in `buildJMXConfig()` below.
	// Supported by: jmx-scraper and jmx-metric-gatherer
	...
	// Wired into the Java `otel.jmx.aggregate.across.mbeans` property in `buildJMXConfig()` below.
	// Supported by: jmx-metric-gatherer (jmx-scraper silently ignores this property)
...
func (c *Config) Validate() error {
	...
	// Validate password file permissions and contents when set.
	if c.PasswordFile != "" {
		if err := c.validatePasswordFilePermissions(); err != nil {
			return err
		}
		...
		if err := c.validatePasswords(passwordMap); err != nil {
			return err
		}
	}
...
// validatePasswords ensures that whenever a `_path`/`username` field is configured but the
// matching inline `_password`/`password` is empty, the password file supplies the corresponding
// entry. Keys consumed: `<username>`, `keystore`, `truststore`.
...
// parsePasswordFile reads a Java `.properties`-format file and returns the map of keys to
// values.
...
func (c *Config) buildJMXConfig() (string, error) {
	...
	// Always emitted. Both Java backends ignore unknown properties, and emitting an
	// explicit `false` matches the gatherer's documented default behavior — keeping the
	// output stable regardless of which JAR is in use.
	...
	// Read password file once for keystore/truststore/user fallback.
