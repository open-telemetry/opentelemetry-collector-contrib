...
func TestConvertSpansToTraceSpans_protobuf(t *testing.T) {
	...
	// First span/resource
	want.ResourceSpans().AppendEmpty().Resource().Attributes().PutString(conventions.AttributeServiceName, "svc-1")
	...
	span0.Attributes().PutString(conventions.AttributeNetHostIP, "192.168.0.1")
	span0.Attributes().PutInt(conventions.AttributeNetHostPort, 8009)
	span0.Attributes().PutString(conventions.AttributeNetPeerName, "memcached")
	span0.Attributes().PutString(conventions.AttributeNetPeerIP, "fe80::1453:a77c:da4d:d21b")
	span0.Attributes().PutInt(conventions.AttributeNetPeerPort, 11211)
	span0.Attributes().PutString(tracetranslator.TagSpanKind, string(tracetranslator.OpenTracingSpanKindConsumer))

	// Second span/resource
	want.ResourceSpans().AppendEmpty().Resource().Attributes().PutString(conventions.AttributeServiceName, "search")
	...
	span1.Attributes().PutString(conventions.AttributeNetHostIP, "10.0.0.13")
	span1.Attributes().PutInt(conventions.AttributeNetHostPort, 8009)
	span1.Attributes().PutString(conventions.AttributeNetPeerName, "redis")
	span1.Attributes().PutString(conventions.AttributeNetPeerIP, "fe80::1453:a77c:da4d:d21b")
	span1.Attributes().PutInt(conventions.AttributeNetPeerPort, 6379)
	span1.Attributes().PutString(tracetranslator.TagSpanKind, string(tracetranslator.OpenTracingSpanKindProducer))
