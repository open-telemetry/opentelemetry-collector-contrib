...
| `enable_otel_attributes`            | 'false'                              | If `true`, well-known journald fields are mapped to OpenTelemetry semantic convention attributes instead of being placed in the log body. `MESSAGE` becomes the log body, `PRIORITY` sets the severity, process/host fields become resource attributes (e.g. `process.pid`, `host.name`), and code/thread/syslog fields become log attributes (e.g. `code.filepath`, `syslog.facility`). Remaining fields are kept as log attributes with their original journald field names. |
