...
func TestNewReceiver_invalid_version_err(t *testing.T) {
	...
	r, err := newMetricsReceiver(context.Background(), *c, component.ComponentSettings{}, nil)
	assert.Error(t, err)
	assert.Nil(t, r)
}
...
func TestNewReceiver(t *testing.T) {
	...
	allScrapers["brokers"] = mockScraper
	r, err := newMetricsReceiver(context.Background(), *c, component.ComponentSettings{}, consumertest.NewNop())
	assert.Nil(t, err)
	assert.NotNil(t, r)
}
