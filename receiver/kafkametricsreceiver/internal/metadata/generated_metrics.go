...
func (m *metricKafkaConsumerGroupLag) recordDataPoint(start pcommon.Timestamp, ts pcommon.Timestamp, val int64, groupAttributeValue string, topicAttributeValue string, partitionAttributeValue int64) {
	...
	dp.SetTimestamp(ts)
	dp.SetIntVal(val)
	dp.Attributes().InsertString("group", groupAttributeValue)
	dp.Attributes().InsertString("topic", topicAttributeValue)
	dp.Attributes().InsertInt("partition", partitionAttributeValue)
}
