...
func TestConsumerScraper_createConsumerScraper(t *testing.T) {
	...
	cs, err := createConsumerScraper(context.Background(), Config{}, sc, receivertest.NewNopSettingsWithType(metadata.Type))
...
func TestConsumerScraper_createScraper_handles_invalid_topic_match(t *testing.T) {
	...
	cs, err := createConsumerScraper(context.Background(), Config{
		TopicMatch: "[",
	}, sc, receivertest.NewNopSettingsWithType(metadata.Type))
	assert.Error(t, err)
	assert.Nil(t, cs)
}
