...
func TestCreateSameReceiver(t *testing.T) {
	...
	attrs := attribute.NewSet(
		// attribute.String(componentattribute.SignalKey, "traces"), // should be removed - 内部包，暂时注释
		// attribute.String(componentattribute.ComponentIDKey, "otlp"), // 内部包，暂时注释
	)
	creationSet := receivertest.NewNopSettings(factory.Type())
	// creationSet.Logger = zap.New(componentattribute.NewConsoleCoreWithAttributes(core, attribute.NewSet())) // 内部包，暂时注释
	creationSet.Logger = zap.New(core) // 简化版本
	// creationSet.TelemetrySettings = telemetry.WithAttributeSet(creationSet.TelemetrySettings, attrs) // 内部包，暂时注释
