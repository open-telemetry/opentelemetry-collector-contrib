...
func TestNewECSInfo(t *testing.T) {
	...
	ecsinfo, _ := NewECSInfo(context.Background(), time.Minute, hostIPProvider, componenttest.NewNopHost(), componenttest.NewNopTelemetrySettings(), containerInstanceInfoCreatorOpt, taskinfoCreatorOpt, cgroupScannerCreatorOpt)
