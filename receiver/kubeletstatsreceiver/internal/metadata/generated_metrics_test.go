		...
		t.Run(test.name, func(t *testing.T) {
			...
			for i := 0; i < ms.Len(); i++ {
				...
				case "k8s.node.runtime.imagefs.inodes":
					...
					assert.Equal(t, pmetric.NumberDataPointValueTypeInt, dp.ValueType())
					assert.Equal(t, int64(1), dp.IntValue())
				case "k8s.node.runtime.imagefs.inodes_free":
					assert.False(t, validatedMetrics["k8s.node.runtime.imagefs.inodes_free"], "Found a duplicate in the metrics slice: k8s.node.runtime.imagefs.inodes_free")
					validatedMetrics["k8s.node.runtime.imagefs.inodes_free"] = true
					assert.Equal(t, pmetric.MetricTypeGauge, ms.At(i).Type())
					assert.Equal(t, 1, ms.At(i).Gauge().DataPoints().Len())
					assert.Equal(t, "Node runtime imagefs inodes_free", ms.At(i).Description())
					assert.Equal(t, "By", ms.At(i).Unit())
					dp := ms.At(i).Gauge().DataPoints().At(0)
					assert.Equal(t, start, dp.StartTimestamp())
					assert.Equal(t, ts, dp.Timestamp())
					assert.Equal(t, pmetric.NumberDataPointValueTypeInt, dp.ValueType())
					assert.Equal(t, int64(1), dp.IntValue())
				case "k8s.node.runtime.imagefs.inodes_used":
					assert.False(t, validatedMetrics["k8s.node.runtime.imagefs.inodes_used"], "Found a duplicate in the metrics slice: k8s.node.runtime.imagefs.inodes_used")
					validatedMetrics["k8s.node.runtime.imagefs.inodes_used"] = true
					assert.Equal(t, pmetric.MetricTypeGauge, ms.At(i).Type())
					assert.Equal(t, 1, ms.At(i).Gauge().DataPoints().Len())
					assert.Equal(t, "Node runtime imagefs inodes_used.This may not equal Inodes - InodesFree because this filesystem may share inodes with other filesystems.", ms.At(i).Description())
					assert.Equal(t, "By", ms.At(i).Unit())
					dp := ms.At(i).Gauge().DataPoints().At(0)
					assert.Equal(t, start, dp.StartTimestamp())
					assert.Equal(t, ts, dp.Timestamp())
