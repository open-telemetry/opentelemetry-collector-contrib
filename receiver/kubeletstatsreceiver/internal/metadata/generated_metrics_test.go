...
func TestMetricsBuilder(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			allMetricsCount++
			mb.RecordK8sContainerCPUNodeLimitUtilizationDataPoint(ts, 1)
			...
			for i := 0; i < ms.Len(); i++ {
				...
				case "container.uptime":
					...
					assert.Equal(t, pmetric.NumberDataPointValueTypeInt, dp.ValueType())
					assert.Equal(t, int64(1), dp.IntValue())
				case "k8s.container.cpu.node_limit_utilization":
					assert.False(t, validatedMetrics["k8s.container.cpu.node_limit_utilization"], "Found a duplicate in the metrics slice: k8s.container.cpu.node_limit_utilization")
					validatedMetrics["k8s.container.cpu.node_limit_utilization"] = true
					assert.Equal(t, pmetric.MetricTypeGauge, ms.At(i).Type())
					assert.Equal(t, 1, ms.At(i).Gauge().DataPoints().Len())
