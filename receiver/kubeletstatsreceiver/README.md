...
> **Note:** The receiver type has been renamed from `kubeletstats` to `kubelet_stats` to follow the
> snake_case naming convention. The old name `kubeletstats` is preserved as a deprecated alias and
> will continue to work, but a deprecation warning will be logged at startup. Please update your
> configuration to use `kubelet_stats:`.
