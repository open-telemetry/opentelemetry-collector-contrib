...
var MetricsInfo = metricsInfo{
	...
	SystemdServiceMemoryPeak: metricInfo{
		Name: "systemd.service.memory.peak",
	},
...
type metricSystemdServiceMemoryPeak struct {
	data     pmetric.Metric                       // data buffer for generated metric.
	config   SystemdServiceMemoryPeakMetricConfig // metric config provided by user.
	capacity int                                  // max observed number of data points added to the metric.
}

// init fills systemd.service.memory.peak metric with initial data.
func (m *metricSystemdServiceMemoryPeak) init() {
	m.data.SetName("systemd.service.memory.peak")
	m.data.SetDescription("Peak bytes of memory used by this service.")
	m.data.SetUnit("By")
	m.data.SetEmptySum()
...
func (m *metricSystemdServiceMemoryPeak) recordDataPoint(start pcommon.Timestamp, ts pcommon.Timestamp, val int64) {
	if !m.config.Enabled {
		return
	}
...
// updateCapacity saves max length of data point slices that will be used for the slice capacity.
func (m *metricSystemdServiceMemoryPeak) updateCapacity() {
	if m.data.Sum().DataPoints().Len() > m.capacity {
		m.capacity = m.data.Sum().DataPoints().Len()
	}
}
...
// emit appends recorded metric data to a metrics slice and prepares it for recording another set of data points.
func (m *metricSystemdServiceMemoryPeak) emit(metrics pmetric.MetricSlice) {
	if m.config.Enabled && m.data.Sum().DataPoints().Len() > 0 {
		m.updateCapacity()
		m.data.MoveTo(metrics.AppendEmpty())
		m.init()
...
func newMetricSystemdServiceMemoryPeak(cfg SystemdServiceMemoryPeakMetricConfig) metricSystemdServiceMemoryPeak {
	m := metricSystemdServiceMemoryPeak{config: cfg}
...
type MetricsBuilder struct {
	...
	metricSystemdServiceMemoryPeak  metricSystemdServiceMemoryPeak
...
func NewMetricsBuilder(mbc MetricsBuilderConfig, settings receiver.Settings, options ...MetricBuilderOption) *MetricsBuilder {
	mb := &MetricsBuilder{
		...
		metricSystemdServiceMemoryPeak:  newMetricSystemdServiceMemoryPeak(mbc.Metrics.SystemdServiceMemoryPeak),
...
func (mb *MetricsBuilder) EmitForResource(options ...ResourceMetricsOption) {
	...
	mb.metricSystemdServiceMemoryPeak.emit(ils.Metrics())
...
// RecordSystemdServiceMemoryPeakDataPoint adds a data point to systemd.service.memory.peak metric.
func (mb *MetricsBuilder) RecordSystemdServiceMemoryPeakDataPoint(ts pcommon.Timestamp, val int64) {
	mb.metricSystemdServiceMemoryPeak.recordDataPoint(mb.startTime, ts, val)
}
