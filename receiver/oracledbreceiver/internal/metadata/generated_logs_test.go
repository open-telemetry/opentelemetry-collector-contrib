...
func TestLogsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			allEventsCount++
			lb.RecordDbServerQuerySampleEvent(ctx, timestamp, "db.query.text-val", "db.system.name-val", "user.name-val", "db.namespace-val", "client.address-val", 11, "network.peer.address-val", 17, "oracledb.plan_hash_value-val", "oracledb.sql_id-val", "oracledb.child_number-val", "oracledb.child_address-val", "oracledb.sid-val", "oracledb.serial-val", "oracledb.process-val", "oracledb.schemaname-val", "oracledb.program-val", "oracledb.module-val", "oracledb.status-val", "oracledb.state-val", "oracledb.wait_class-val", "oracledb.event-val", 21, "oracledb.procedure_name-val", "oracledb.procedure_type-val", "oracledb.osuser-val", 21.100000, "oracledb.blocker.session.id-val", "oracledb.blocker.root_session.id-val", "oracledb.blocker.session_relationship.state-val", 22, "oracledb.lock.type-val", "oracledb.lock.id1-val", "oracledb.lock.id2-val")
			...
			for i := 0; i < lrs.Len(); i++ {
				...
				case "db.server.query_sample":
					...
					attrVal, ok = lr.Attributes().Get("oracledb.blocker.session.id")
					assert.True(t, ok)
					assert.Equal(t, "oracledb.blocker.session.id-val", attrVal.Str())
					attrVal, ok = lr.Attributes().Get("oracledb.blocker.root_session.id")
					assert.True(t, ok)
					assert.Equal(t, "oracledb.blocker.root_session.id-val", attrVal.Str())
					...
					attrVal, ok = lr.Attributes().Get("oracledb.lock.id1")
					assert.True(t, ok)
					assert.Equal(t, "oracledb.lock.id1-val", attrVal.Str())
					attrVal, ok = lr.Attributes().Get("oracledb.lock.id2")
					assert.True(t, ok)
					assert.Equal(t, "oracledb.lock.id2-val", attrVal.Str())
