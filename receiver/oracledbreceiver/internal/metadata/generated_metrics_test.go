...
func TestMetricsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			defaultMetricsCount++
			allMetricsCount++
			mb.RecordOracledbSessionsUsageDataPoint(ts, "1", "oracle.db.pdb-val", "session_status-val", "session_type-val")
			if tt.name == "reaggregate_set" {
				mb.RecordOracledbSessionsUsageDataPoint(ts, "3", "oracle.db.pdb-val-2", "session_status-val-2", "session_type-val-2")
			}
			...
			defaultMetricsCount++
			allMetricsCount++
			mb.RecordOracledbTablespaceSizeLimitDataPoint(ts, 1, "oracle.db.pdb-val", "tablespace_name-val")
			if tt.name == "reaggregate_set" {
				mb.RecordOracledbTablespaceSizeLimitDataPoint(ts, 3, "oracle.db.pdb-val-2", "tablespace_name-val-2")
			}

			defaultMetricsCount++
			allMetricsCount++
			mb.RecordOracledbTablespaceSizeUsageDataPoint(ts, 1, "oracle.db.pdb-val", "tablespace_name-val")
			if tt.name == "reaggregate_set" {
				mb.RecordOracledbTablespaceSizeUsageDataPoint(ts, 3, "oracle.db.pdb-val-2", "tablespace_name-val-2")
			}
			...
			for _, mi := range allMetricsList {
				...
				case "oracledb.consistent_gets":
					if tt.name != "reaggregate_set" {
						...
						oracleDbPdbAttrVal, ok := dp.Attributes().Get("oracle.db.pdb")
						assert.True(t, ok)
