...
const (
	...
	tablespaceUsageSQL      = `
		select um.TABLESPACE_NAME, um.USED_SPACE, um.TABLESPACE_SIZE, ts.BLOCK_SIZE from
			(select TABLESPACE_NAME, USED_SPACE, TABLESPACE_SIZE from DBA_TABLESPACE_USAGE_METRICS group by TABLESPACE_NAME, USED_SPACE, TABLESPACE_SIZE) um,
			(select TABLESPACE_NAME, BLOCK_SIZE from DBA_TABLESPACES group by TABLESPACE_NAME, BLOCK_SIZE) ts
