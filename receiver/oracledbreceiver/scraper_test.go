...
func TestScraper_ScrapeIOPerformanceMetrics(t *testing.T) {
	cfg := metadata.NewDefaultMetricsBuilderConfig()
	cfg.Metrics.OracledbPhysicalIoBytes.Enabled = true
	cfg.Metrics.OracledbPhysicalIoRequests.Enabled = true
	cfg.Metrics.OracledbPhysicalWritesFromCache.Enabled = true
	cfg.Metrics.OracledbSqlnetBytes.Enabled = true
	...
	assert.Equal(t, int64(1024000), got["oracledb.physical_io.bytes"]["direction=read,io_type=buffered"])
	assert.Equal(t, int64(512000), got["oracledb.physical_io.bytes"]["direction=write,io_type=buffered"])
	assert.Equal(t, int64(2048000), got["oracledb.physical_io.bytes"]["direction=read,io_type=total"])
	assert.Equal(t, int64(1024000), got["oracledb.physical_io.bytes"]["direction=write,io_type=total"])

	assert.Equal(t, int64(5000), got["oracledb.physical_io.requests"]["block_size=all,direction=read"])
	assert.Equal(t, int64(2500), got["oracledb.physical_io.requests"]["block_size=all,direction=write"])
	assert.Equal(t, int64(1500), got["oracledb.physical_io.requests"]["block_size=multi,direction=read"])
	assert.Equal(t, int64(750), got["oracledb.physical_io.requests"]["block_size=multi,direction=write"])

	assert.Equal(t, int64(1800), got["oracledb.physical_writes_from_cache"][""])

	assert.Equal(t, int64(300000), got["oracledb.sqlnet.bytes"]["peer=client,sqlnet_direction=received"])
	assert.Equal(t, int64(600000), got["oracledb.sqlnet.bytes"]["peer=client,sqlnet_direction=sent"])
	assert.Equal(t, int64(150000), got["oracledb.sqlnet.bytes"]["peer=dblink,sqlnet_direction=received"])
	assert.Equal(t, int64(75000), got["oracledb.sqlnet.bytes"]["peer=dblink,sqlnet_direction=sent"])
}
