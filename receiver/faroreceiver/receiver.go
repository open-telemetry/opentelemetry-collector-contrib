...
import (
	...
	httpHelper "github.com/open-telemetry/opentelemetry-collector-contrib/receiver/faroreceiver/internal/httphelper"
)
...
func (r *faroReceiver) errorHandler(w http.ResponseWriter, _ *http.Request, errMsg string, statusCode int) {
	...
	r.writeStatusResponse(w, jsEncoder, statusCode, s.Proto())
}

func (r *faroReceiver) writeStatusResponse(w http.ResponseWriter, enc encoder, statusCode int, rsp *spb.Status) {
	msg, err := enc.marshalStatus(rsp)
	if err != nil {
		r.writeResponse(w, enc.contentType(), 500, []byte(err.Error()))
		return
	}
	r.writeResponse(w, enc.contentType(), statusCode, msg)
}

func (r *faroReceiver) writeResponse(w http.ResponseWriter, contentType string, statusCode int, msg []byte) {
	w.Header().Set("Content-Type", contentType)
	w.WriteHeader(statusCode)
	if _, err := w.Write(msg); err != nil {
		r.settings.Logger.Error("Could not write response", zap.Any("message", msg), zap.Any("errors", err))
	}
}
