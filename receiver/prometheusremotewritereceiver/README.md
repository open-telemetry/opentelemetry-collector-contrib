...
Target info metrics and "normal" metrics are a match when they have the same job/instance. But these metrics not always comes in the same order. For this reason, the receiver use an internal LRU and stateless cache implementation to store resource metrics across requests.
...
This approach has some limitations, like:
