...
| [Code Owners](https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/CONTRIBUTING.md#becoming-a-code-owner)    | [@dmitryax](https://www.github.com/dmitryax), [@TylerHelmuth](https://www.github.com/TylerHelmuth), [@ChrsMark](https://www.github.com/ChrsMark), [@krisztianfekete](https://www.github.com/krisztianfekete) |
| Emeritus      | [@hvaghani221](https://www.github.com/hvaghani221) |
...
- `mode`: define in which way it collects this type of object, either "pull" or "watch".  Both modes maintain an in-memory cache of the collected objects. Memory usage scales with the number of objects collected and their size; for very large clusters or high-cardinality resources, size the collector accordingly.
...
- `exclude_watch_type`: allows excluding specific watch types. Valid values are `ADDED`, `MODIFIED`, and `DELETED`. Only usable in `watch` mode. `BOOKMARK` and `ERROR` events are never emitted by the receiver: the cache-backed watcher does not surface them, so those values in this list are no-ops.
...
Both `pull` and `watch` modes maintain a cache backed by `List` + `Watch`, so the receiver requires both the `list` and `watch`
verbs on every configured resource. Depending on `error_mode`, missing `watch` permission on a resource can fail collector
startup. For watch mode the receiver also relists to recover from
[410 Gone scenarios](https://kubernetes.io/docs/reference/using-api/api-concepts/#410-gone-responses).
