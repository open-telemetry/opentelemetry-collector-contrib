...
- `api_qps` (default = `5`): Maximum number of queries per second to the Kubernetes API. Increase this if you see `client-side throttling` warnings in the collector logs when watching or polling many resources simultaneously.
- `api_burst` (default = `10`): Maximum burst size for requests to the Kubernetes API. Increase this alongside `api_qps` if you see `client-side throttling` warnings.
