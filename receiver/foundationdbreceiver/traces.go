...
func (t *Trace) getSpan(span *ptrace.Span) {
	...
	// extract the service name from operation names which are in the form: SS:getKeyValues
	p := strings.Index(t.OperationName, ":")
	if p != -1 {
		span.SetName(t.OperationName[p+1:])
		span.Attributes().PutStr(semconv.AttributeServiceName, t.OperationName[:p])
	} else {
