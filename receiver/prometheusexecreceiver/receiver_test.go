...
func assertErrorWhenExecKeyMissing(t *testing.T, errorReceiverConfig config.Receiver) {
	_, err := newPromExecReceiver(component.ComponentSettings{Logger: zap.NewNop()}, errorReceiverConfig.(*Config), nil)
...
func endToEndScrapeTest(t *testing.T, receiverConfig config.Receiver, testName string) {
	...
	wrapper, err := newPromExecReceiver(component.ComponentSettings{Logger: zap.NewNop()}, receiverConfig.(*Config), sink)
