	...
	testCases := []struct {
	...
	}{
		{
			...
			mockDockerEngine: func(t *testing.T) *httptest.Server {
				...
				mockServer, err := dockerMockServer(&map[string]string{
					"/v1.44/containers/json":                      filepath.Join(mockFolder, "single_container", "containers.json"),
					"/v1.44/containers/" + containerID + "/json":  filepath.Join(mockFolder, "single_container", "container.json"),
					"/v1.44/containers/" + containerID + "/stats": filepath.Join(mockFolder, "single_container", "stats.json"),
				})
		...
		},
		{
			...
			mockDockerEngine: func(t *testing.T) *httptest.Server {
				...
				mockServer, err := dockerMockServer(&map[string]string{
					"/v1.44/containers/json":                          filepath.Join(mockFolder, "two_containers", "containers.json"),
					"/v1.44/containers/" + containerIDs[0] + "/json":  filepath.Join(mockFolder, "two_containers", "container1.json"),
					"/v1.44/containers/" + containerIDs[1] + "/json":  filepath.Join(mockFolder, "two_containers", "container2.json"),
					"/v1.44/containers/" + containerIDs[0] + "/stats": filepath.Join(mockFolder, "two_containers", "stats1.json"),
					"/v1.44/containers/" + containerIDs[1] + "/stats": filepath.Join(mockFolder, "two_containers", "stats2.json"),
				})
				require.NoError(t, err)
				return mockServer
			},
		...
		},
		{
			...
			mockDockerEngine: func(t *testing.T) *httptest.Server {
				...
				containerID := "10b703fb312b25e8368ab5a3bce3a1610d1cee5d71a94920f1a7adbc5b0cb326"
				mockServer, err := dockerMockServer(&map[string]string{
					"/v1.44/containers/json":                      filepath.Join(mockFolder, "no_pids_stats", "containers.json"),
					"/v1.44/containers/" + containerID + "/json":  filepath.Join(mockFolder, "no_pids_stats", "container.json"),
					"/v1.44/containers/" + containerID + "/stats": filepath.Join(mockFolder, "no_pids_stats", "stats.json"),
				})
				require.NoError(t, err)
				return mockServer
			},
		...
		},
		{
			...
			mockDockerEngine: func(t *testing.T) *httptest.Server {
				...
				mockServer, err := dockerMockServer(&map[string]string{
					"/v1.44/containers/json":                      filepath.Join(mockFolder, "pids_stats_max", "containers.json"),
					"/v1.44/containers/" + containerID + "/json":  filepath.Join(mockFolder, "pids_stats_max", "container.json"),
					"/v1.44/containers/" + containerID + "/stats": filepath.Join(mockFolder, "pids_stats_max", "stats.json"),
				})
				require.NoError(t, err)
				return mockServer
			},
		...
		},
		{
			...
			mockDockerEngine: func(t *testing.T) *httptest.Server {
				...
				mockServer, err := dockerMockServer(&map[string]string{
					"/v1.44/containers/json":                      filepath.Join(mockFolder, "cpu_limit", "containers.json"),
					"/v1.44/containers/" + containerID + "/json":  filepath.Join(mockFolder, "cpu_limit", "container.json"),
					"/v1.44/containers/" + containerID + "/stats": filepath.Join(mockFolder, "cpu_limit", "stats.json"),
				})
				require.NoError(t, err)
				return mockServer
			},
		...
		},
		{
			...
			mockDockerEngine: func(t *testing.T) *httptest.Server {
				...
				mockServer, err := dockerMockServer(&map[string]string{
					"/v1.44/containers/json":                      filepath.Join(mockFolder, "cgroups_v2", "containers.json"),
					"/v1.44/containers/" + containerID + "/json":  filepath.Join(mockFolder, "cgroups_v2", "container.json"),
					"/v1.44/containers/" + containerID + "/stats": filepath.Join(mockFolder, "cgroups_v2", "stats.json"),
				})
				require.NoError(t, err)
				return mockServer
			},
		...
		},
		{
			...
			mockDockerEngine: func(t *testing.T) *httptest.Server {
				...
				mockServer, err := dockerMockServer(&map[string]string{
					"/v1.44/containers/json":                      filepath.Join(mockFolder, "single_container_with_optional_resource_attributes", "containers.json"),
					"/v1.44/containers/" + containerID + "/json":  filepath.Join(mockFolder, "single_container_with_optional_resource_attributes", "container.json"),
					"/v1.44/containers/" + containerID + "/stats": filepath.Join(mockFolder, "single_container_with_optional_resource_attributes", "stats.json"),
				})
				require.NoError(t, err)
				return mockServer
			},
