...
func (p *StatsDParser) Aggregate(line string, enableMetricType bool) error {
	parsedMetric, err := parseMessageToMetric(line, enableMetricType)
	if err != nil {
		return err
	}
...
func parseMessageToMetric(line string, enableMetricType bool) (statsDMetric, error) {
	...
	result.labelKeys = make([]*metricspb.LabelKey, 0)
	result.labelValues = make([]*metricspb.LabelValue, 0)
