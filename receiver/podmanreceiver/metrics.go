...
func translateStatsToMetrics(stats *define.ContainerStats, ts time.Time) (pdata.Metrics, error) {
...
func appendMemoryMetrics(ms pdata.MetricSlice, stats *define.ContainerStats, ts pdata.Timestamp) {
...
func appendNetworkMetrics(ms pdata.MetricSlice, stats *define.ContainerStats, ts pdata.Timestamp) {
...
func appendIOMetrics(ms pdata.MetricSlice, stats *define.ContainerStats, ts pdata.Timestamp) {
...
func appendCPUMetrics(ms pdata.MetricSlice, stats *define.ContainerStats, ts pdata.Timestamp) {
...
func gauge(ms pdata.MetricSlice, metricName string, unit string, ts pdata.Timestamp) pdata.NumberDataPointSlice {
...
func gaugeI(ms pdata.MetricSlice, metricName string, unit string, points []point, ts pdata.Timestamp) {
	dataPoints := gauge(ms, metricName, unit, ts)
	for _, pt := range points {
		dataPoint := dataPoints.AppendEmpty()
		dataPoint.SetTimestamp(ts)
		dataPoint.SetIntVal(int64(pt.intVal))
