...
func TestCreateTracesWrongConfig(t *testing.T) {
	factory := NewFactory()
	_, err := factory.CreateTraces(context.Background(), receivertest.NewNopSettingsWithType(metadata.Type), nil, nil)
...
func TestCreateTracesBadConfigNoAuth(t *testing.T) {
	...
	factory := NewFactory()
	_, err := factory.CreateTraces(context.Background(), receivertest.NewNopSettingsWithType(metadata.Type), cfg, consumertest.NewNop())
