...
func (mb *MetricsBuilder) ResourceMetricsBuilder(res pcommon.Resource, options ...resourceMetricsBuilderOption) *ResourceMetricsBuilder {
	...
	rmb := &ResourceMetricsBuilder{
		...
		resource:        res,
		metricFileAtime: newMetricFileAtime(mb.config.Metrics.FileAtime),
		metricFileCtime: newMetricFileCtime(mb.config.Metrics.FileCtime),
		metricFileMtime: newMetricFileMtime(mb.config.Metrics.FileMtime),
		metricFileSize:  newMetricFileSize(mb.config.Metrics.FileSize),
	}
...
func (rmb *ResourceMetricsBuilder) emit(m pmetric.Metrics) bool {
	...
	rmb.metricFileAtime.emit(sm.Metrics())
	rmb.metricFileCtime.emit(sm.Metrics())
	rmb.metricFileMtime.emit(sm.Metrics())
	rmb.metricFileSize.emit(sm.Metrics())
	if sm.Metrics().Len() == 0 {
		return false
	}
	rmb.updateCapacity(sm.Metrics())
	sm.Scope().SetName("otelcol/filestatsreceiver")
...
func (rmb *ResourceMetricsBuilder) RecordFileAtimeDataPoint(ts pcommon.Timestamp, val int64) {
	rmb.metricFileAtime.recordDataPoint(rmb.startTime, ts, val)
}

// RecordFileCtimeDataPoint adds a data point to file.ctime metric.
func (rmb *ResourceMetricsBuilder) RecordFileCtimeDataPoint(ts pcommon.Timestamp, val int64, filePermissionsAttributeValue string) {
	rmb.metricFileCtime.recordDataPoint(rmb.startTime, ts, val, filePermissionsAttributeValue)
}

// RecordFileMtimeDataPoint adds a data point to file.mtime metric.
func (rmb *ResourceMetricsBuilder) RecordFileMtimeDataPoint(ts pcommon.Timestamp, val int64) {
	rmb.metricFileMtime.recordDataPoint(rmb.startTime, ts, val)
}

// RecordFileSizeDataPoint adds a data point to file.size metric.
func (rmb *ResourceMetricsBuilder) RecordFileSizeDataPoint(ts pcommon.Timestamp, val int64) {
	rmb.metricFileSize.recordDataPoint(rmb.startTime, ts, val)
}
