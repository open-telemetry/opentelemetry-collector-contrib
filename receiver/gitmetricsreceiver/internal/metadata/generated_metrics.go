...
type metricGitRepositoryBranchCommitCount struct {
	data     pmetric.Metric // data buffer for generated metric.
	config   MetricConfig   // metric config provided by user.
	capacity int            // max observed number of data points added to the metric.
}

// init fills git.repository.branch.commit.count metric with initial data.
func (m *metricGitRepositoryBranchCommitCount) init() {
	m.data.SetName("git.repository.branch.commit.count")
...
func (m *metricGitRepositoryBranchCommitCount) recordDataPoint(start pcommon.Timestamp, ts pcommon.Timestamp, val int64, repositoryNameAttributeValue string, branchNameAttributeValue string) {
	if !m.config.Enabled {
		return
	}
...
func (m *metricGitRepositoryBranchCommitCount) updateCapacity() {
...
func (m *metricGitRepositoryBranchCommitCount) emit(metrics pmetric.MetricSlice) {
...
func newMetricGitRepositoryBranchCommitCount(cfg MetricConfig) metricGitRepositoryBranchCommitCount {
	m := metricGitRepositoryBranchCommitCount{config: cfg}
	if cfg.Enabled {
...
type MetricsBuilder struct {
	...
	metricGitRepositoryBranchCommitCount metricGitRepositoryBranchCommitCount
...
func NewMetricsBuilder(mbc MetricsBuilderConfig, settings receiver.CreateSettings, options ...metricBuilderOption) *MetricsBuilder {
	mb := &MetricsBuilder{
		...
		metricGitRepositoryBranchCommitCount: newMetricGitRepositoryBranchCommitCount(mbc.Metrics.GitRepositoryBranchCommitCount),
...
func (mb *MetricsBuilder) EmitForResource(rmo ...ResourceMetricsOption) {
	...
	ils.Scope().SetName("otelcol/gitmetricsreceiver")
	ils.Scope().SetVersion(mb.buildInfo.Version)
	ils.Metrics().EnsureCapacity(mb.metricsCapacity)
	mb.metricGitRepositoryBranchCommitCount.emit(ils.Metrics())
...
// RecordGitRepositoryBranchCommitCountDataPoint adds a data point to git.repository.branch.commit.count metric.
func (mb *MetricsBuilder) RecordGitRepositoryBranchCommitCountDataPoint(ts pcommon.Timestamp, val int64, repositoryNameAttributeValue string, branchNameAttributeValue string) {
	mb.metricGitRepositoryBranchCommitCount.recordDataPoint(mb.startTime, ts, val, repositoryNameAttributeValue, branchNameAttributeValue)
}
