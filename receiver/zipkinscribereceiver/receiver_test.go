...
func TestScribeReceiverServer(t *testing.T) {
	...
	traceReceiver, err := New(endpoint, "zipkin", sink)
	if err != nil {
		t.Fatalf("Failed to create receiver: %v", err)
	}
			...
			TimeEvents: &tracepb.Span_TimeEvents{
				TimeEvent: []*tracepb.Span_TimeEvent{
					...
					},
					{
						...
						Value: &tracepb.Span_TimeEvent_Annotation_{
							Annotation: &tracepb.Span_TimeEvent_Annotation{
								Description: &tracepb.TruncatableString{Value: "Gc(9,0.PSScavenge,2015-09-17 12:37:02 +0000,304.milliseconds+762.microseconds)"},
							},
						},
					},
				},
