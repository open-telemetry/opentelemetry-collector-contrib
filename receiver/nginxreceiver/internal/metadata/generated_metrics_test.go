...
func TestMetricsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			aggMap["NginxConnectionsAccepted"] = mb.metricNginxConnectionsAccepted.config.AggregationStrategy
			aggMap["NginxConnectionsCurrent"] = mb.metricNginxConnectionsCurrent.config.AggregationStrategy
			aggMap["NginxConnectionsHandled"] = mb.metricNginxConnectionsHandled.config.AggregationStrategy
			aggMap["NginxRequests"] = mb.metricNginxRequests.config.AggregationStrategy
			...
			if tt.name == "reaggregate_set" {
				mb.RecordNginxConnectionsAcceptedDataPoint(ts, 3)
			}
			...
			if tt.name == "reaggregate_set" {
				mb.RecordNginxConnectionsCurrentDataPoint(ts, 3, AttributeStateActive)
			}
			...
			if tt.name == "reaggregate_set" {
				mb.RecordNginxConnectionsHandledDataPoint(ts, 3)
			}
			...
			if tt.name == "reaggregate_set" {
				mb.RecordNginxRequestsDataPoint(ts, 3)
			}

			res := pcommon.NewResource()
			metrics := mb.Emit(WithResource(res))
			if tt.name == "reaggregate_set" {
				assert.Empty(t, mb.metricNginxConnectionsAccepted.aggDataPoints)
				assert.Empty(t, mb.metricNginxConnectionsCurrent.aggDataPoints)
				assert.Empty(t, mb.metricNginxConnectionsHandled.aggDataPoints)
				assert.Empty(t, mb.metricNginxRequests.aggDataPoints)
			}
			...
			for i := 0; i < ms.Len(); i++ {
				...
				case "nginx.connections_accepted":
					if tt.name != "reaggregate_set" {
						...
						assert.Equal(t, ts, dp.Timestamp())
						assert.Equal(t, pmetric.NumberDataPointValueTypeInt, dp.ValueType())
						switch aggMap["nginx.connections_accepted"] {
						case "sum":
				...
				case "nginx.connections_handled":
					if tt.name != "reaggregate_set" {
						...
						assert.Equal(t, ts, dp.Timestamp())
						assert.Equal(t, pmetric.NumberDataPointValueTypeInt, dp.ValueType())
						switch aggMap["nginx.connections_handled"] {
						case "sum":
							assert.Equal(t, int64(4), dp.IntValue())
						case "avg":
							assert.Equal(t, int64(2), dp.IntValue())
