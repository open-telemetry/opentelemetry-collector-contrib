...
func TestLogsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			allEventsCount++
			lb.RecordDbServerQuerySampleEvent(ctx, timestamp, "client.address-val", 11, AttributeDbSystemNameMongodb, "db.namespace-val", "db.collection.name-val", "db.operation.name-val", "db.query.text-val", AttributeMongodbQueryTruncatedTruncated, "user.name-val", "mongodb.application.name-val", false, 37, 39, "mongodb.cursor.id-val", false, "mongodb.cursor.operation_using_cursor_id-val", "mongodb.cursor.originating_command-val", false, "mongodb.database.name-val", "mongodb.lsid.id-val", "mongodb.namespace-val", "mongodb.operation.id-val", "mongodb.operation.plan_summary-val", "mongodb.operation.query_framework-val", AttributeMongodbOperationStatusActive, "mongodb.operation.type-val", 26.100000, 45, 38, 29, true, "mongodb.operation.locks-val", "mongodb.operation.lock_stats-val", true, "mongodb.operation.flow_control_stats-val", false, "mongodb.operation.waiting_for_latch.details-val")
			...
			for i := 0; i < lrs.Len(); i++ {
				...
				case "db.server.query_sample":
					...
					attrVal, ok = lr.Attributes().Get("mongodb.query_truncated")
					assert.True(t, ok)
					assert.Equal(t, "truncated", attrVal.Str())
					...
					assert.True(t, ok)
					assert.False(t, attrVal.Bool())
					attrVal, ok = lr.Attributes().Get("mongodb.database.name")
					assert.True(t, ok)
					assert.Equal(t, "mongodb.database.name-val", attrVal.Str())
					...
					attrVal, ok = lr.Attributes().Get("mongodb.operation.waiting_for_lock")
					assert.True(t, ok)
					...
					attrVal, ok = lr.Attributes().Get("mongodb.operation.waiting_for_flow_control")
					assert.True(t, ok)
					assert.True(t, attrVal.Bool())
					...
					attrVal, ok = lr.Attributes().Get("mongodb.operation.waiting_for_latch")
					assert.True(t, ok)
					assert.False(t, attrVal.Bool())
