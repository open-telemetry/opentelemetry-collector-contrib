		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < lrs.Len(); i++ {
				...
				case "db.server.top_query":
					...
					assert.True(t, ok)
					assert.Equal(t, "mongodb.cursor.originating_command-val", attrVal.Str())
					attrVal, ok = lr.Attributes().Get("mongodb.explain_plan.hash")
					assert.True(t, ok)
					assert.Equal(t, "mongodb.explain_plan.hash-val", attrVal.Str())
					attrVal, ok = lr.Attributes().Get("mongodb.explain_plan.text")
					assert.True(t, ok)
					assert.Equal(t, "mongodb.explain_plan.text-val", attrVal.Str())
					attrVal, ok = lr.Attributes().Get("mongodb.operation.comment")
					assert.True(t, ok)
					assert.Equal(t, []any{"mongodb.operation.comment-item1", "mongodb.operation.comment-item2"}, attrVal.Slice().AsRaw())
