...
- `query_sample_collection`: Additional configuration for query sample collection (`db.server.query_sample` event):
  - `max_rows_per_query`: (default = `100`) The maximum number of eligible query samples to emit per `$currentOp` query. Operations skipped by the receiver do not count toward this limit.
