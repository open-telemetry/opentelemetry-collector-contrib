...
- `mongodb.assert.count` with `mongodb.assert.type=tripwire` — only emitted by servers that report `serverStatus.asserts.tripwire`. The other assertion types emit on every supported version, and a server without the field is not reported as an error.
- `mongodb.write_concern.wait.time` — read from `serverStatus.metrics.getLastError.wtime.totalMillis`, whose name is a legacy artifact of a command removed in MongoDB `5.1`. The counter increments for writes specifying a write concern greater than `w:1`, including the `w:"majority"` default applied to replica sets since MongoDB `5.0`. It therefore accumulates on a replica set under ordinary write traffic, and stays at zero on a standalone deployment and for writes that specify `w:1`.
