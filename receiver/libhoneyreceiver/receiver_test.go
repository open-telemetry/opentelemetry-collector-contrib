...
// TestLibhoneyReceiver_ZstdDecompressionPanic tests for nil pointer dereference
// in zstd decompression that may be related to Go 1.23->1.24 upgrade
func TestLibhoneyReceiver_ZstdDecompressionPanic(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectErrorCode: 503,   // Currently returns 503 for internal server error
		...
		},
		{
			...
			expectErrorCode: 503,   // Currently returns 503, should be 400
		...
		},
		{
			...
			expectPanic:     false, // BUG: This might currently panic but shouldn't
			expectErrorCode: 503,   // Currently returns 503
			description:     "Corrupted zstd block should return error, not panic (bug reproduction)",
		},
		{
			name: "valid_uncompressed_data",
			createPayload: func() []byte {
				// Create valid JSON payload that should work
				events := []libhoneyevent.LibhoneyEvent{
					{
						Time:       time.Now().Format(time.RFC3339),
			...
			description:     "Valid JSON should process successfully",
		},
		{
			name: "nil_pointer_trigger_test",
			createPayload: func() []byte {
				// Create a specific pattern that's known to trigger the nil pointer
				// This test case expects a panic to validate the test structure works
				return []byte{0x28, 0xb5, 0x2f, 0xfd, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08}
			},
			expectPanic:     true, // This test expects a panic to validate test structure
			expectErrorCode: 0,    // Don't check status code if panic occurs
			description:     "Test case designed to trigger panic (for test validation)",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.name == "valid_uncompressed_data" {
			...
			defer func() {
				...
				} else if !tt.expectPanic && panicOccurred {
