...
func (r *libhoneyReceiver) startHTTPServer(ctx context.Context, host component.Host) error {
	...
	var err error
	if r.server, err = httpCfg.ServerConfig.ToServer(ctx, host.GetExtensions(), r.settings.TelemetrySettings, httpMux); err != nil {
		return err
	}

	r.settings.Logger.Info("Starting HTTP server", zap.String("endpoint", httpCfg.ServerConfig.NetAddr.Endpoint))
