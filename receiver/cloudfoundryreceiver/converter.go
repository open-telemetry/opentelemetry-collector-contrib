...
func collectMetricsFromEnvelope(envelope *loggregator_v2.Envelope, metricSlice pdata.MetricSlice, startTime time.Time) {
	envelopeLabels := envelope.Tags
	namePrefix := envelopeLabels["origin"] + "."

	if envelope.SourceId != "" {
		envelopeLabels["source_id"] = envelope.SourceId
	}

	if envelope.InstanceId != "" {
		envelopeLabels["instance_id"] = envelope.InstanceId
	}
	...
	case *loggregator_v2.Envelope_Counter:
		...
		dataPoint.SetStartTimestamp(pdata.TimestampFromTime(startTime))
		dataPoint.LabelsMap().InitFromMap(envelopeLabels)
