...
func TestScrape_ConcurrencyLimit(t *testing.T) {
	t.Run("respects concurrency limit", func(t *testing.T) {
		// This test verifies that the semaphore properly limits concurrent goroutines
		// We can't easily test the actual concurrency in a unit test without
		// instrumentation, but we can verify the scraper completes successfully
		// with a large number of repos and a low concurrency limit

		factory := &Factory{}
		defaultConfig := factory.CreateDefaultConfig().(*Config)

		cfg := &Config{
			MetricsBuilderConfig: defaultConfig.MetricsBuilderConfig,
			ClientConfig:         defaultConfig.ClientConfig,
			GitHubOrg:            "testorg",
			ConcurrencyLimit:     5, // Low limit to test semaphore
		}
		...
		// Note: Full integration test would require mock GraphQL server
		// with many repositories - this validates config/validation only
	})

	t.Run("rejects negative concurrency limit", func(t *testing.T) {
		cfg := &Config{
			ConcurrencyLimit: -1,
		}
		...
		err := cfg.Validate()
		require.Error(t, err)
		require.Contains(t, err.Error(), "concurrency_limit must be non-negative")
	})

	t.Run("allows zero concurrency limit (unlimited)", func(t *testing.T) {
		cfg := &Config{
			ConcurrencyLimit: 0,
		}
