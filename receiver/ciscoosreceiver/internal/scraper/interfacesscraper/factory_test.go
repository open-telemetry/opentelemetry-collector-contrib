...
func TestFactory_ConfigWithDevice(t *testing.T) {
	...
	assert.Equal(t, 22, cfg.Device.Host.Port)
}
...
func TestConfig_SingleDevice(t *testing.T) {
	...
	assert.Equal(t, "switch1", config.Device.Host.Name)
	assert.Equal(t, "10.0.0.1", config.Device.Host.IP)
	assert.Equal(t, 22, config.Device.Host.Port)
...
func TestConfig_EmptyDevice(t *testing.T) {
	...
	assert.NotNil(t, config)
	assert.Empty(t, config.Device.Host.IP)
