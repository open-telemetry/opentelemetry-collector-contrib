...
func Test_transaction_pdata(t *testing.T) {
	...
	t.Run("Commit Without Adding", func(t *testing.T) {
		...
		tr := newTransaction(scrapeCtx, &txConfig{nil, true, "", rID, nomc, nil, componenttest.NewNopReceiverCreateSettings()})
	...
	t.Run("Add One Good", func(t *testing.T) {
		...
		tr := newTransaction(scrapeCtx, &txConfig{nil, true, "", rID, sink, nil, componenttest.NewNopReceiverCreateSettings()})
