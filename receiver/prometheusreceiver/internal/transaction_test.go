...
func testTransactionCommitWithoutAdding(t *testing.T, enableNativeHistograms bool) {
	tr := newTransaction(scrapeCtx, &startTimeAdjuster{startTime: startTimestamp}, consumertest.NewNop(), labels.EmptyLabels(), receivertest.NewNopSettingsWithType(metadata.Type), nopObsRecv(t), false, enableNativeHistograms)
...
func testTransactionAppendResource(t *testing.T, enableNativeHistograms bool) {
	...
	tr := newTransaction(scrapeCtx, &startTimeAdjuster{startTime: startTimestamp}, sink, labels.EmptyLabels(), receivertest.NewNopSettingsWithType(metadata.Type), nopObsRecv(t), false, enableNativeHistograms)
	_, err := tr.Append(0, labels.FromMap(map[string]string{
		model.InstanceLabel:   "localhost:8080",
		model.JobLabel:        "test",
...
func testTransactionCommitErrorWhenAdjusterError(t *testing.T, enableNativeHistograms bool) {
	...
	tr := newTransaction(scrapeCtx, &errorAdjuster{err: adjusterErr}, sink, labels.EmptyLabels(), receivertest.NewNopSettingsWithType(metadata.Type), nopObsRecv(t), false, enableNativeHistograms)
...
func testTransactionAppendHistogramNoLe(t *testing.T, enableNativeHistograms bool) {
	sink := new(consumertest.MetricsSink)
	receiverSettings := receivertest.NewNopSettingsWithType(metadata.Type)
...
func testTransactionAppendWithEmptyLabelArrayFallbackToTargetLabels(t *testing.T, enableNativeHistograms bool) {
	...
	tr := newTransaction(ctx, &startTimeAdjuster{startTime: startTimestamp}, sink, labels.EmptyLabels(), receivertest.NewNopSettingsWithType(metadata.Type), nopObsRecv(t), false, enableNativeHistograms)
