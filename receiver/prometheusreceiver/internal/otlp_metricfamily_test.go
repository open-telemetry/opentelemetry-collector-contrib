	...
	tests := []struct {
	...
	}{
		{
			...
			want: func() pmetric.HistogramDataPoint {
				...
				point.SetExplicitBounds(pcommon.NewImmutableFloat64SliceFromValue(&[]float64{0.75, 2.75}))
				point.SetBucketCounts(pcommon.NewImmutableUInt64SliceFromValue(&[]uint64{33, 22, 11}))
		...
		},
		{
			...
			want: func() pmetric.HistogramDataPoint {
				...
				point.SetExplicitBounds(pcommon.NewImmutableFloat64SliceFromValue(&[]float64{0.75, 2.75}))
				point.SetBucketCounts(pcommon.NewImmutableUInt64SliceFromValue(&[]uint64{0, 0, 0}))
				point.SetStartTimestamp(pcommon.Timestamp(11 * time.Millisecond)) // the time in milliseconds -> nanoseconds.
				attributes := point.Attributes()
				attributes.InsertString("a", "A")
				attributes.InsertString("b", "B")
