...
func TestLoadTargetAllocatorConfig(t *testing.T) {
	...
	r3 := cfg.(*Config)
	ta3 := r3.TargetAllocator.Get()
	assert.Equal(t, "https://target-allocator-service:80", ta3.Endpoint)
	assert.Equal(t, 30*time.Second, ta3.Interval)
	assert.Equal(t, "collector-1", ta3.CollectorID)
