...
// TestScrapeOnShutdown verifies that when ScrapeOnShutdown is enabled,
...
// TestDiscoveryReloadOnStartup verifies that when DiscoveryReloadOnStartup is enabled,
// target discovery is loaded immediately on start, bypassing a long discovery reload interval.
func TestDiscoveryReloadOnStartup(t *testing.T) {
	ctx := t.Context()
	settings := receivertest.NewNopSettings(metadata.Type)
...
// TestInitialScrapeOffset verifies that when InitialScrapeOffset is configured,
// the first scrape is delayed by at least the configured duration.
func TestInitialScrapeOffset(t *testing.T) {
	ctx := t.Context()
	settings := receivertest.NewNopSettings(metadata.Type)
