...
type metricSystemProcessesCount struct {
	...
	config        SystemProcessesCountConfig // metric config provided by user.
...
func (m *metricSystemProcessesCount) recordDataPoint(start pcommon.Timestamp, ts pcommon.Timestamp, val int64, statusAttributeValue string) {
	...
	dp.SetTimestamp(ts)
	if slices.Contains(m.config.EnabledAttributes, SystemProcessesCountAttributeKeyStatus) {
...
func newMetricSystemProcessesCount(cfg SystemProcessesCountConfig) metricSystemProcessesCount {
...
type metricSystemProcessesCreated struct {
	data     pmetric.Metric               // data buffer for generated metric.
	config   SystemProcessesCreatedConfig // metric config provided by user.
	capacity int                          // max observed number of data points added to the metric.
}
...
func newMetricSystemProcessesCreated(cfg SystemProcessesCreatedConfig) metricSystemProcessesCreated {
