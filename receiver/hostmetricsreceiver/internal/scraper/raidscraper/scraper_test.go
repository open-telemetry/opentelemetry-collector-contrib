...
func TestScrapeSimple(t *testing.T) {
	...
	dp := getGaugeDatapointIntValueByLabels(metric.Gauge(), map[string]string{"linux.mdraid.device.name": "disk1"})
	assert.NotNil(t, dp)
	...
	assert.Equalf(t, pmetric.MetricTypeGauge, metric.Type(), "invalid metric type: %v", metric.Type())
	dp = getGaugeDatapointIntValueByLabels(metric.Gauge(), map[string]string{"linux.mdraid.device.name": "disk1", "linux.mdraid.device.state": "spare"})
	assert.NotNil(t, dp)
	assert.Equal(t, int64(3), dp.IntValue())

	metric = getMetric(t, "system.linux.mdraid.disks", metrics.ResourceMetrics())
	assert.Equalf(t, pmetric.MetricTypeGauge, metric.Type(), "invalid metric type: %v", metric.Type())
	dp = getGaugeDatapointIntValueByLabels(metric.Gauge(), map[string]string{"linux.mdraid.device.name": "disk1", "linux.mdraid.device.state": "active"})
	assert.NotNil(t, dp)
	assert.Equal(t, int64(5), dp.IntValue())

	metric = getMetric(t, "system.linux.mdraid.disks", metrics.ResourceMetrics())
	assert.Equalf(t, pmetric.MetricTypeGauge, metric.Type(), "invalid metric type: %v", metric.Type())
	dp = getGaugeDatapointIntValueByLabels(metric.Gauge(), map[string]string{"linux.mdraid.device.name": "disk1", "linux.mdraid.device.state": "failed"})
	assert.NotNil(t, dp)
	assert.Equal(t, int64(2), dp.IntValue())

	metric = getMetric(t, "system.linux.mdraid.activity_state", metrics.ResourceMetrics())
	assert.Equalf(t, pmetric.MetricTypeGauge, metric.Type(), "invalid metric type: %v", metric.Type())
	dp = getGaugeDatapointIntValueByLabels(metric.Gauge(), map[string]string{"linux.mdraid.device.name": "disk1", "linux.mdraid.device.activity_state": "resync"})
	assert.NotNil(t, dp)
	assert.Equal(t, int64(1), dp.IntValue())

	metric = getMetric(t, "system.linux.mdraid.activity_state", metrics.ResourceMetrics())
	assert.Equalf(t, pmetric.MetricTypeGauge, metric.Type(), "invalid metric type: %v", metric.Type())
	dp = getGaugeDatapointIntValueByLabels(metric.Gauge(), map[string]string{"linux.mdraid.device.name": "disk1", "linux.mdraid.device.activity_state": "active"})
	assert.NotNil(t, dp)
	assert.Equal(t, int64(0), dp.IntValue())

	metric = getMetric(t, "system.linux.mdraid.activity_state", metrics.ResourceMetrics())
	assert.Equalf(t, pmetric.MetricTypeGauge, metric.Type(), "invalid metric type: %v", metric.Type())
	dp = getGaugeDatapointIntValueByLabels(metric.Gauge(), map[string]string{"linux.mdraid.device.name": "disk1", "linux.mdraid.device.activity_state": "recovering"})
	assert.NotNil(t, dp)
	assert.Equal(t, int64(0), dp.IntValue())

	metric = getMetric(t, "system.linux.mdraid.activity_state", metrics.ResourceMetrics())
	assert.Equalf(t, pmetric.MetricTypeGauge, metric.Type(), "invalid metric type: %v", metric.Type())
	dp = getGaugeDatapointIntValueByLabels(metric.Gauge(), map[string]string{"linux.mdraid.device.name": "disk1", "linux.mdraid.device.activity_state": "inactive"})
	assert.NotNil(t, dp)
	assert.Equal(t, int64(0), dp.IntValue())

	metric = getMetric(t, "system.linux.mdraid.activity_state", metrics.ResourceMetrics())
	assert.Equalf(t, pmetric.MetricTypeGauge, metric.Type(), "invalid metric type: %v", metric.Type())
	dp = getGaugeDatapointIntValueByLabels(metric.Gauge(), map[string]string{"linux.mdraid.device.name": "disk1", "linux.mdraid.device.activity_state": "check"})
	assert.NotNil(t, dp)
	assert.Equal(t, int64(0), dp.IntValue())
