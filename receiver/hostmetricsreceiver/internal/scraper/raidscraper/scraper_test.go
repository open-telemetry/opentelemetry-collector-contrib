...
func TestScrapeSimple(t *testing.T) {
	...
	metric := getMetric(t, "md.blocks.total", metrics.ResourceMetrics())
	...
	metric = getMetric(t, "md.blocks.synced", metrics.ResourceMetrics())
	assert.Equalf(t, pmetric.MetricTypeGauge, metric.Type(), "invalid metric type: %v", metric.Type())
	dp = getGaugeDatapointIntValueByLabels(metric.Gauge(), map[string]string{"device": "disk1"})
	assert.NotNil(t, dp)
	assert.Equal(t, int64(1), dp.IntValue())

	metric = getMetric(t, "md.disks", metrics.ResourceMetrics())
	assert.Equalf(t, pmetric.MetricTypeGauge, metric.Type(), "invalid metric type: %v", metric.Type())
	...
	metric = getMetric(t, "md.state", metrics.ResourceMetrics())
	assert.Equalf(t, pmetric.MetricTypeGauge, metric.Type(), "invalid metric type: %v", metric.Type())
	dp = getGaugeDatapointIntValueByLabels(metric.Gauge(), map[string]string{"device": "disk1", "state": "resync"})
	assert.NotNil(t, dp)
	assert.Equal(t, int64(1), dp.IntValue())
	...
	metric = getMetric(t, "md.state", metrics.ResourceMetrics())
	assert.Equalf(t, pmetric.MetricTypeGauge, metric.Type(), "invalid metric type: %v", metric.Type())
	dp = getGaugeDatapointIntValueByLabels(metric.Gauge(), map[string]string{"device": "disk1", "state": "recovering"})
	assert.NotNil(t, dp)
	assert.Equal(t, int64(0), dp.IntValue())

	metric = getMetric(t, "md.state", metrics.ResourceMetrics())
	assert.Equalf(t, pmetric.MetricTypeGauge, metric.Type(), "invalid metric type: %v", metric.Type())
	dp = getGaugeDatapointIntValueByLabels(metric.Gauge(), map[string]string{"device": "disk1", "state": "inactive"})
	assert.NotNil(t, dp)
	assert.Equal(t, int64(0), dp.IntValue())

	metric = getMetric(t, "md.state", metrics.ResourceMetrics())
	assert.Equalf(t, pmetric.MetricTypeGauge, metric.Type(), "invalid metric type: %v", metric.Type())
	dp = getGaugeDatapointIntValueByLabels(metric.Gauge(), map[string]string{"device": "disk1", "state": "check"})
	assert.NotNil(t, dp)
	assert.Equal(t, int64(0), dp.IntValue())

	metric = getMetric(t, "md.raid.disks", metrics.ResourceMetrics())
	assert.Equalf(t, pmetric.MetricTypeGauge, metric.Type(), "invalid metric type: %v", metric.Type())
	dp = getGaugeDatapointIntValueByLabels(metric.Gauge(), map[string]string{"device": "disk1"})
	assert.NotNil(t, dp)
	assert.Equal(t, int64(4), dp.IntValue())

	metric = getMetric(t, "md.raid.degraded", metrics.ResourceMetrics())
	assert.Equalf(t, pmetric.MetricTypeGauge, metric.Type(), "invalid metric type: %v", metric.Type())
	dp = getGaugeDatapointIntValueByLabels(metric.Gauge(), map[string]string{"device": "disk1"})
	assert.NotNil(t, dp)
	assert.Equal(t, int64(2), dp.IntValue())
