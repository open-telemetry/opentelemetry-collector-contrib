...
func newCPUEmitter(cfg *Config) func(context.Context, pcommon.Timestamp, *metadata.MetricsBuilder) error {
	if useCPUTicks.IsEnabled() {
		return newCputicksEmitter(cfg.rootPath)
	}
	return newGopsutilEmitter(cpu.TimesWithContext)
}

func newCputicksEmitter(rootPath string) func(context.Context, pcommon.Timestamp, *metadata.MetricsBuilder) error {
	reader := cputicks.NewReader(rootPath, defaultTicksPerSecond)
	...
	return func(ctx context.Context, now pcommon.Timestamp, mb *metadata.MetricsBuilder) error {
		...
		if prevTicks != nil {
			for _, curr := range ticks {
				prev, ok := prevTicks[curr.CPU]
				if !ok {
					return fmt.Errorf("getting ticks for cpu %s: %w", curr.CPU, ucal.ErrTimeStatNotFound)
				}
