...
func TestMetricsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			allMetricsCount++
			mb.RecordSystemNetworkConnectionCountDataPoint(ts, 1, "process.name-val", "server.address-val", 11)
			if tt.name == "reaggregate_set" {
				mb.RecordSystemNetworkConnectionCountDataPoint(ts, 3, "process.name-val-2", "server.address-val-2", 12)
			}
			...
			for _, mi := range allMetricsList {
				...
				case "system.network.connection.count":
					if tt.name != "reaggregate_set" {
						...
						assert.Equal(t, "The number of network connections, grouped by process and remote endpoint.", mi.Description())
						...
						processNameAttrVal, ok := dp.Attributes().Get("process.name")
						assert.True(t, ok)
						assert.Equal(t, "process.name-val", processNameAttrVal.Str())
						...
						_, ok := dp.Attributes().Get("process.name")
						assert.False(t, ok)
