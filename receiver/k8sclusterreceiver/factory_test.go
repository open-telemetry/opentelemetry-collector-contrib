...
func newTestReceiver(t *testing.T, cfg *Config) *kubernetesReceiver {
	r, err := newReceiver(context.Background(), receivertest.NewNopSettingsWithType(metadata.Type), cfg)
	require.NoError(t, err)
	require.NotNil(t, r)
...
func TestNewSharedReceiver(t *testing.T) {
	...
	mr, err := newMetricsReceiver(context.Background(), receivertest.NewNopSettingsWithType(metadata.Type), cfg, mc)
	require.NoError(t, err)
	...
	lr, err := newLogsReceiver(context.Background(), receivertest.NewNopSettingsWithType(metadata.Type), cfg, lc)
	require.NoError(t, err)
