...
func TestResourceBuilder(t *testing.T) {
	for _, test := range []string{"default", "all_set", "none_set"} {
		t.Run(test, func(t *testing.T) {
			...
			rb.SetK8sContainerStatusWaitingReason("k8s.container.status.waiting_reason-val")
			...
			val, ok = res.Attributes().Get("k8s.container.status.waiting_reason")
			assert.Equal(t, test == "all_set", ok)
			if ok {
				assert.EqualValues(t, "k8s.container.status.waiting_reason-val", val.Str())
			}
