...
func TestMetricsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			allMetricsCount++
			mb.RecordK8sContainerStatusWaitingDataPoint(ts, 1)
			...
			for i := 0; i < ms.Len(); i++ {
				...
				case "k8s.container.restarts":
					...
					assert.Equal(t, pmetric.NumberDataPointValueTypeInt, dp.ValueType())
					assert.Equal(t, int64(1), dp.IntValue())
				case "k8s.container.status.waiting":
					assert.False(t, validatedMetrics["k8s.container.status.waiting"], "Found a duplicate in the metrics slice: k8s.container.status.waiting")
					validatedMetrics["k8s.container.status.waiting"] = true
					assert.Equal(t, pmetric.MetricTypeGauge, ms.At(i).Type())
					assert.Equal(t, 1, ms.At(i).Gauge().DataPoints().Len())
					assert.Equal(t, "Whether container is in waiting state. (0 for now, 1 for yes)", ms.At(i).Description())
					assert.Equal(t, "", ms.At(i).Unit())
					dp := ms.At(i).Gauge().DataPoints().At(0)
					assert.Equal(t, start, dp.StartTimestamp())
					assert.Equal(t, ts, dp.Timestamp())
