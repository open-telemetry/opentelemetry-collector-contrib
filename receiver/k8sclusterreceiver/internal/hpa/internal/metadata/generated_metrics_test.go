...
func TestMetricsBuilder(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			metrics := mb.Emit(WithK8sHpaName("attr-val"), WithK8sHpaUID("attr-val"), WithK8sNamespaceName("attr-val"))
			...
			attrCount++
			assert.Equal(t, mb.resourceAttributesSettings.K8sHpaName.Enabled, ok)
			if mb.resourceAttributesSettings.K8sHpaName.Enabled {
				enabledAttrCount++
				assert.EqualValues(t, "attr-val", attrVal.Str())
			}
			attrVal, ok = rm.Resource().Attributes().Get("k8s.hpa.uid")
			attrCount++
			assert.Equal(t, mb.resourceAttributesSettings.K8sHpaUID.Enabled, ok)
			if mb.resourceAttributesSettings.K8sHpaUID.Enabled {
				enabledAttrCount++
				assert.EqualValues(t, "attr-val", attrVal.Str())
			}
