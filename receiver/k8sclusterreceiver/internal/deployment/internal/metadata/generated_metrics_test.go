...
func TestMetricsBuilder(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			metrics := mb.Emit(WithK8sDeploymentName("k8s.deployment.name-val"), WithK8sDeploymentUID("k8s.deployment.uid-val"), WithK8sNamespaceName("k8s.namespace.name-val"), WithOpencensusResourcetype("opencensus.resourcetype-val"))
			...
			attrCount := 0
			enabledAttrCount := 0
			attrVal, ok := rm.Resource().Attributes().Get("k8s.deployment.name")
			attrCount++
			assert.Equal(t, mb.resourceAttributesConfig.K8sDeploymentName.Enabled, ok)
			if mb.resourceAttributesConfig.K8sDeploymentName.Enabled {
				enabledAttrCount++
				assert.EqualValues(t, "k8s.deployment.name-val", attrVal.Str())
			}
			attrVal, ok = rm.Resource().Attributes().Get("k8s.deployment.uid")
			attrCount++
			assert.Equal(t, mb.resourceAttributesConfig.K8sDeploymentUID.Enabled, ok)
			if mb.resourceAttributesConfig.K8sDeploymentUID.Enabled {
				enabledAttrCount++
				assert.EqualValues(t, "k8s.deployment.uid-val", attrVal.Str())
			}
