// Code generated by mdatagen. DO NOT EDIT.

// Package metadata contains the autogenerated telemetry and
// build information for the receiver/windows_service component.
package metadata

import (
	"go.opentelemetry.io/collector/component"
)

var (
	Type           = component.MustNewType("windows_service")
	DeprecatedType = component.MustNewType("windowsservice")
	ScopeName      = "github.com/open-telemetry/opentelemetry-collector-contrib/receiver/windowsservicereceiver"
)

const (
	MetricsStability = component.StabilityLevelAlpha
)
