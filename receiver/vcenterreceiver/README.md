...
| [Code Owners](https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/CONTRIBUTING.md#becoming-a-code-owner)    | [@schmikei](https://www.github.com/schmikei), [@ishleenk17](https://www.github.com/ishleenk17) \| Seeking more code owners! |
...
| proxy_url           |         | String           | Not Required. URL of the proxy to use when connecting to the vCenter SDK endpoint <br><br> i.e: `socks5://proxy.hostname.localnet:1080`. The `http`, `https`, `socks5` and `socks5h` schemes are supported. When unset, the standard `HTTP_PROXY`/`HTTPS_PROXY`/`NO_PROXY` environment variables apply. Unlike those, this setting is scoped to a single receiver instance, so several `vcenter/<name>` receivers in one collector can each use a different proxy. <br><br> When it is set, those environment variables are ignored for this receiver and all SDK traffic is sent to the proxy — there is no bypass list equivalent to `NO_PROXY`. With an `https` proxy the `tls` settings below also govern the connection to the proxy itself, since Go's HTTP client has no separate TLS configuration for it. |
