...
func testScrape(ctx context.Context, t *testing.T, cfg *Config, fileName string) {
	scraper := newVmwareVcenterScraper(zap.NewNop(), cfg, receivertest.NewNopSettingsWithType(metadata.Type))
...
func TestStartFailures_Metrics(t *testing.T) {
	...
	for _, tc := range cases {
		scraper := newVmwareVcenterScraper(zap.NewNop(), tc.cfg, receivertest.NewNopSettingsWithType(metadata.Type))
