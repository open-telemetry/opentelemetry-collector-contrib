...
func TestGRPCReception(t *testing.T) {
	...
	conn, err := grpc.NewClient(config.GRPCServerConfig.NetAddr.Endpoint, grpc.WithTransportCredentials(insecure.NewCredentials()), grpc.WithBlock())
	require.NoError(t, err)
	defer conn.Close()
...
func TestGRPCReceptionWithTLS(t *testing.T) {
	...
	require.NoError(t, err)
	conn, err := grpc.NewClient(grpcServerSettings.NetAddr.Endpoint, grpc.WithTransportCredentials(creds), grpc.WithBlock())
	require.NoError(t, err)
	defer conn.Close()
