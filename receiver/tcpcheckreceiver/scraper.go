...
func (s *tcpCheckScraper) scrapeTCP(now pcommon.Timestamp) error {
	...
	start := time.Now()
	err := s.Client.Dial(s.Config.TCPClientSettings.Endpoint, s.Config.TCPClientSettings.Timeout)
	if err == nil {
		success = 1
	}
	s.mb.RecordTcpcheckDurationDataPoint(now, time.Since(start).Milliseconds(), s.Config.TCPClientSettings.Endpoint)
...
func (s *tcpCheckScraper) scrape(ctx context.Context) (_ pmetric.Metrics, err error) {
	...
	if err = s.scrapeTCP(now); err != nil {
		s.mb.RecordTcpcheckErrorDataPoint(now, int64(1), err.Error())
	} else {
		go func() {
			<-ctx.Done()
			cleanup()
