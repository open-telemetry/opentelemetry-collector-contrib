...
func TestMetricBuilder(t *testing.T) {
	t.Run("WithSingleMetric", func(t *testing.T) {
		...
		require.Equal(t, pdata.MetricDataTypeHistogram, got.DataType())
		require.Equal(t, pdata.MetricAggregationTemporalityDelta, got.Histogram().AggregationTemporality())
		dps := got.Histogram().DataPoints()
		require.Equal(t, 1, dps.Len())
		dp := dps.At(0)
		require.Equal(t, uint64(metric.Value.Count), dp.Count())
		require.Equal(t, metric.Value.Sum, dp.Sum())
		require.Equal(t, []float64{metric.Value.Min, metric.Value.Max}, dp.ExplicitBounds())
	...
	t.Run("WithTimestampCollision", func(t *testing.T) {
		...
		require.Equal(t, []float64{metrics[0].Value.Min, metrics[0].Value.Max}, dp.ExplicitBounds())
