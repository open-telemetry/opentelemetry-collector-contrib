		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, mi := range allMetricsList {
				...
				case "memcached.bytes":
					...
					assert.Equal(t, "Current number of bytes used by this server to store items.", mi.Description())
				...
				case "memcached.commands":
					...
					assert.True(t, ok)
					assert.Equal(t, "get", commandAttrVal.Str())
				case "memcached.connections.current":
					...
					assert.Equal(t, pmetric.MetricTypeSum, mi.Type())
					assert.Equal(t, 1, mi.Sum().DataPoints().Len())
					assert.Equal(t, "The current number of open connections.", mi.Description())
				...
				case "memcached.connections.total":
					...
					assert.Equal(t, pmetric.MetricTypeSum, mi.Type())
					assert.Equal(t, 1, mi.Sum().DataPoints().Len())
					assert.Equal(t, "Total number of connections opened since the server started running.", mi.Description())
					assert.Equal(t, "{connections}", mi.Unit())
					assert.True(t, mi.Sum().IsMonotonic())
					assert.Equal(t, pmetric.AggregationTemporalityCumulative, mi.Sum().AggregationTemporality())
					dp := mi.Sum().DataPoints().At(0)
				...
				case "memcached.current_items":
					...
					assert.Equal(t, pmetric.MetricTypeSum, mi.Type())
					assert.Equal(t, 1, mi.Sum().DataPoints().Len())
					assert.Equal(t, "Number of items currently stored in the cache.", mi.Description())
					assert.Equal(t, "{items}", mi.Unit())
					assert.False(t, mi.Sum().IsMonotonic())
					assert.Equal(t, pmetric.AggregationTemporalityCumulative, mi.Sum().AggregationTemporality())
					dp := mi.Sum().DataPoints().At(0)
					assert.Equal(t, start, dp.StartTimestamp())
				...
				case "memcached.evictions":
					...
					assert.Equal(t, pmetric.MetricTypeSum, mi.Type())
					assert.Equal(t, 1, mi.Sum().DataPoints().Len())
					assert.Equal(t, "Cache item evictions.", mi.Description())
				...
				case "memcached.operation_hit_ratio":
					...
					assert.True(t, ok)
					assert.Equal(t, "increment", operationAttrVal.Str())
				...
				case "memcached.threads":
					...
					assert.Equal(t, pmetric.MetricTypeSum, mi.Type())
					assert.Equal(t, 1, mi.Sum().DataPoints().Len())
					assert.Equal(t, "Number of threads used by the memcached instance.", mi.Description())
