...
const sqlServerQueryTextAndPlanQueryTemplate = `
DECLARE @lookbackTime INT = -{{ .lookbackTime }};
DECLARE @topNValue INT = {{ .topNValue }};
with qstats as (
	...
	WHERE qs.last_execution_time BETWEEN DATEADD(SECOND, @lookbackTime, GETDATE()) AND GETDATE() {{ .instanceNameClause }}
	GROUP BY
		qs.query_hash,
		qs.query_plan_hash
)
...
func getSQLServerQueryTextAndPlanQuery(instanceName string, maxQuerySampleCount uint, lookbackTime uint) (string, error) {
	...
	tmpl := template.Must(template.New(filepath.Base(templateFile)).Option("missingkey=error").Parse(sqlServerQueryTextAndPlanQueryTemplate))
	buf := bytes.Buffer{}
