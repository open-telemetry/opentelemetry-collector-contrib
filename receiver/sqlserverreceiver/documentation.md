...
| sqlserver.blocking_start_time | Timestamp of when the current blocking wait began (ISO 8601 format). | Any Str |
...
| sqlserver.resource_id | SQL Server identifier for the locked or waited-on resource, if available. | Any Str |
| sqlserver.resource_type | SQL Server type of the locked or waited-on resource, if available. | Any Str |
