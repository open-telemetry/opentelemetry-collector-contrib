...
Aggregated performance metrics for the top stored procedures by elapsed time, read from sys.dm_exec_procedure_stats with delta computation on the cumulative counters. Correlates with db.server.top_query and db.server.query_sample via sqlserver.procedure_id.
