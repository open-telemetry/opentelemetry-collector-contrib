...
| service.instance.id | A unique identifier of the SQL Server instance in the format host:port. In direct connection mode the host and port come from the connection config. In Windows Performance Counter mode the host comes from computer_name (or os.Hostname() when monitoring locally) and the port defaults to 1433. | Any Str | true | - | - |
