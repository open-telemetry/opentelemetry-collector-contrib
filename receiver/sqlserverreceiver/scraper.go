	...
	for i, row := range rows {
		...
		case bytesReceivedFromReplicaPerSec:
			...
			if err != nil {
				...
				s.mb.RecordSqlserverReplicaDataRateDataPoint(now, val, metadata.AttributeDirectionRead)
			}
		case bytesSentForReplicaPerSec:
			...
			if err != nil {
				...
				s.mb.RecordSqlserverReplicaDataRateDataPoint(now, val, metadata.AttributeDirectionWrite)
			}
