...
func (s *sqlServerScraperHelper) recordAvailabilityGroupMetrics(ctx context.Context) error {
	...
	for _, row := range rows {
		...
		// secondary_lag_seconds can be NULL for primary replicas; only record when present
		...
		// log_send_rate is in KB/s from SQL Server; convert to By/s per semconv
		val, err = retrieveFloat(row, logSendRateKey)
		if err != nil {
			errs = append(errs, fmt.Errorf("availability_group=%q database=%q replica=%q: %w", agName, dbName, replicaName, err))
		} else {
			s.mb.RecordSqlserverLogDataIoRateDataPoint(now, val.(float64)*1024, agName, dbName, replicaName, metadata.AttributeReplicaDirectionReceive)
		}

		// log_send_queue_size and redo_queue_size are in KB from SQL Server; convert to By per semconv
