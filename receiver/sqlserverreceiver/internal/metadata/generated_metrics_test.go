...
func TestMetricsBuilder(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			allMetricsCount++
			mb.RecordSqlserverDatabaseIoWriteLatencyDataPoint(ts, 1, "physical_filename-val", "logical_filename-val", "file_type-val")
			...
			for i := 0; i < ms.Len(); i++ {
				...
				case "sqlserver.database.io.read_latency":
					...
					assert.True(t, ok)
					assert.EqualValues(t, "file_type-val", attrVal.Str())
				case "sqlserver.database.io.write_latency":
					assert.False(t, validatedMetrics["sqlserver.database.io.write_latency"], "Found a duplicate in the metrics slice: sqlserver.database.io.write_latency")
					validatedMetrics["sqlserver.database.io.write_latency"] = true
					assert.Equal(t, pmetric.MetricTypeSum, ms.At(i).Type())
					assert.Equal(t, 1, ms.At(i).Sum().DataPoints().Len())
					assert.Equal(t, "Total time that the users waited for writes.", ms.At(i).Description())
					assert.Equal(t, "s", ms.At(i).Unit())
					assert.Equal(t, true, ms.At(i).Sum().IsMonotonic())
					assert.Equal(t, pmetric.AggregationTemporalityCumulative, ms.At(i).Sum().AggregationTemporality())
					dp := ms.At(i).Sum().DataPoints().At(0)
