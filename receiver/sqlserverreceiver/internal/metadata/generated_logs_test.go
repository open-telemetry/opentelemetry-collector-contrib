		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < lrs.Len(); i++ {
				...
				case "db.server.top_query":
					...
					attrVal, ok = lr.Attributes().Get("sqlserver.query.plan_creation_time")
					assert.True(t, ok)
					assert.Equal(t, "sqlserver.query.plan_creation_time-val", attrVal.Str())
				}
			}
		})
	}
