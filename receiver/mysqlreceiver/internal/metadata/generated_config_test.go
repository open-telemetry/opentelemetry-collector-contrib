...
func TestMysqlMyisamKeyCacheBlocksMetricsConfig_Validate(t *testing.T) {
	cfg := DefaultMetricsConfig().MysqlMyisamKeyCacheBlocks
	require.NoError(t, cfg.Validate())

	cfg.EnabledAttributes = []MysqlMyisamKeyCacheBlocksMetricAttributeKey{"invalid"}
	require.ErrorContains(t, cfg.Validate(), "metric mysql.myisam.key_cache.blocks doesn't have an attribute invalid, valid attributes: [state]")
