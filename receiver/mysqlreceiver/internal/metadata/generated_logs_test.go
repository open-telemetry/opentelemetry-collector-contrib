		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < lrs.Len(); i++ {
				...
				case "db.server.query_sample":
					...
					attrVal, ok = lr.Attributes().Get("mysql.blocking.pids")
					assert.True(t, ok)
					assert.Equal(t, "mysql.blocking.pids-val", attrVal.Str())
					attrVal, ok = lr.Attributes().Get("mysql.blocking.start_time")
					assert.True(t, ok)
					assert.Equal(t, "mysql.blocking.start_time-val", attrVal.Str())
