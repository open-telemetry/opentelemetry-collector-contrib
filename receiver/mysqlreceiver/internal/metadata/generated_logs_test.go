...
func TestLogsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			allEventsCount++
			lb.RecordDbServerTopQueryEvent(ctx, timestamp, AttributeDbSystemNameMysql, "db.query.text-val", "mysql.query_plan-val", "mysql.query_plan.hash-val", "mysql.events_statements_summary_by_digest.digest-val", 52, 56.100000, "db.namespace-val", 55, 55)
			...
			for i := 0; i < lrs.Len(); i++ {
				...
				case "db.server.top_query":
					...
					assert.True(t, ok)
					assert.Equal(t, "db.namespace-val", attrVal.Str())
					attrVal, ok = lr.Attributes().Get("mysql.events_statements_summary_by_digest.examined_rows")
					assert.True(t, ok)
					assert.EqualValues(t, 55, attrVal.Int())
					attrVal, ok = lr.Attributes().Get("mysql.events_statements_summary_by_digest.returned_rows")
					assert.True(t, ok)
					assert.EqualValues(t, 55, attrVal.Int())
				}
			}
