...
func TestScrapeQuerySamplesBlockingInnoDB(t *testing.T) {
	...
	lockType, ok := attrs.Get("mysql.blocking.lock.type")
	assert.True(t, ok, "mysql.blocking.lock.type must be present")
	assert.Equal(t, "RECORD", lockType.Str())
	...
	lockIndex, ok := attrs.Get("mysql.blocking.lock.index")
	assert.True(t, ok, "mysql.blocking.lock.index must be present")
	assert.Equal(t, "PRIMARY", lockIndex.Str())

	txStart, ok := attrs.Get("mysql.blocking.transaction.start_time")
	assert.True(t, ok, "mysql.blocking.transaction.start_time must be present")
	assert.Equal(t, "2026-07-02 10:29:55", txStart.Str())
...
func TestScrapeQuerySamplesBlockingMDL(t *testing.T) {
	...
	startTime, ok := attrs.Get("mysql.blocking.start_time")
	assert.True(t, ok, "mysql.blocking.start_time must be present")
	assert.Equal(t, "2026-07-02 10:31:00", startTime.Str())

	waitDur, ok := attrs.Get("mysql.blocking.wait_duration")
	assert.True(t, ok, "mysql.blocking.wait_duration must be present")
	assert.InDelta(t, 5.2, waitDur.Double(), 0.001)

	lockMode, ok := attrs.Get("mysql.blocking.lock.mode")
	assert.True(t, ok, "mysql.blocking.lock.mode must be present for MDL")
	assert.Empty(t, lockMode.Str(), "InnoDB lock mode must be empty for MDL blocks")

	mdlObjType, ok := attrs.Get("mysql.blocking.mdl.object_type")
	assert.True(t, ok, "mysql.blocking.mdl.object_type must be present")
	assert.Equal(t, "TABLE", mdlObjType.Str())

	mdlSchema, ok := attrs.Get("mysql.blocking.mdl.object_schema")
	assert.True(t, ok, "mysql.blocking.mdl.object_schema must be present")
	assert.Equal(t, "adventureworks", mdlSchema.Str())

	mdlObjName, ok := attrs.Get("mysql.blocking.mdl.object_name")
	assert.True(t, ok, "mysql.blocking.mdl.object_name must be present")
	assert.Equal(t, "orders", mdlObjName.Str())
...
func TestScrapeQuerySamplesBlockingLegacy(t *testing.T) {
	v57 := mustDBVersion(t, "5.7.44")
	mc := &mockClient{querySamplesFile: "query_samples_blocking_legacy", dbVersionOverride: &v57}
	cfg := createDefaultConfig().(*Config)
	cfg.Username = "otel"
	cfg.Password = "otel"
	cfg.AddrConfig = confignet.AddrConfig{Endpoint: "localhost:3306"}
	...
	pids, ok := attrs.Get("mysql.blocking.pids")
	assert.True(t, ok, "mysql.blocking.pids must be present")
	assert.Equal(t, "101", pids.Str())

	startTime, ok := attrs.Get("mysql.blocking.start_time")
	assert.True(t, ok, "mysql.blocking.start_time must be present")
	assert.Equal(t, "2026-07-02 10:32:00", startTime.Str())

	waitDur, ok := attrs.Get("mysql.blocking.wait_duration")
	assert.True(t, ok, "mysql.blocking.wait_duration must be present")
	assert.InDelta(t, 3.7, waitDur.Double(), 0.001)

	lockTable, ok := attrs.Get("mysql.blocking.lock.table")
	assert.True(t, ok, "mysql.blocking.lock.table must be present")
	assert.Equal(t, "mydb.accounts", lockTable.Str())

	txStart, ok := attrs.Get("mysql.blocking.transaction.start_time")
	assert.True(t, ok, "mysql.blocking.transaction.start_time must be present")
	assert.Equal(t, "2026-07-02 10:31:58", txStart.Str())

	mdlType, ok := attrs.Get("mysql.blocking.mdl.object_type")
	assert.True(t, ok, "mysql.blocking.mdl.object_type attribute must exist")
	assert.Empty(t, mdlType.Str(), "MDL attributes must be empty for legacy InnoDB path")
}
...
func TestScrapeQuerySamplesNoBlocking(t *testing.T) {
	...
	assert.Empty(t, lockLevel.Str(), "blocking attributes must be empty when session is not blocked")
