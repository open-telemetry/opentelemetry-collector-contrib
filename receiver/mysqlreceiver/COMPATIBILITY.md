...
| `supportsDataLockWaits()` | MySQL 8.0.1+ | InnoDB blocking data uses `information_schema.innodb_lock_waits` + `innodb_locks` (legacy tables available on MySQL 5.7 and all MariaDB). Metadata lock (MDL) blocking attributes are empty (MDL requires `performance_schema.metadata_locks`, available only on MySQL 8.0+) |
...
| Product | Series | Exact Version | Platform | `supportsQuerySampleText` | `supportsProcesslist` | `supportsReplicaStatus` | `supportsDataLockWaits` | Timer Wait Tiers | Date |
|---|---|---|---|---|---|---|---|---|---|
