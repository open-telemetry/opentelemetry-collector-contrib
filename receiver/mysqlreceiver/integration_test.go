...
func TestVersionCompatibility(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, tc.wantSampleTextCol, dv.supportsQuerySampleText(), "supportsQuerySampleText mismatch")
			assert.Equal(t, tc.wantReplicaStatus, dv.isMySQL8Plus(), "isMySQL8Plus mismatch")
			...
			_, err = c.getQuerySamples(10, dv.isMySQL8Plus())
			...
			_, err = c.getReplicaStatusStats(dv.isMySQL8Plus())
