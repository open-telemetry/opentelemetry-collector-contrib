...
func TestOnAddForLogsWithHints(t *testing.T) {
	for _, test := range []struct {
	...
	}{
		{
			...
			hintsConfig:          DiscoveryConfig{Enabled: true, DefaultLogsDiscovery: false},
			...
			name:                 "dynamically generated standard filelog receiver with default enablement",
			target:               podContainerEndpointWithOutHints,
			hintsConfig:          DiscoveryConfig{Enabled: true, DefaultLogsDiscovery: true},
			expectedReceiverType: &nopWithFilelogReceiver{},
			expectedReceiverConfig: &nopWithFilelogConfig{
				Include:         []string{"/var/log/pods/default_pod-2_pod-2-UID/redis/*.log"},
				IncludeFileName: false,
