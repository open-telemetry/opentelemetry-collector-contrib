...
var (
	...
	azureResourceLogsJSON = []byte(`{"records":[{"time":"2025-04-15T10:16:32.9873441Z","resourceId":"/SUBSCRIPTIONS/11111111-1111-1111-1111-111111111111/PROVIDERS/MICROSOFT.WEB/SITES/EXAMPLESITE","operationName":"MICROSOFT.WEB/SITES/READ","category":"AppServiceHTTPLogs","resultType":"Success","level":"Informational","tenantId":"22222222-2222-2222-2222-222222222222","properties":{"foo":"bar"}}]}`)
)
...
func TestConsumeLogsAzureResourceLogs(t *testing.T) {
	for _, mode := range testModes {
		t.Run(mode, func(tt *testing.T) {
			receiver, err := getBlobReceiverWithEncodings(tt, mode, EncodingAzureResourceLogs, EncodingOTLPJSON)
			require.NoError(tt, err)
			...
			err = logsConsumer.consumeLogs(tt.Context(), azureResourceLogsJSON)
			require.NoError(tt, err)
			assert.Equal(tt, 1, logsSink.LogRecordCount())
		})
	}
