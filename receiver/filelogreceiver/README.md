...
| `max_surge_size`                      | 0                                    | The maximum surge size of log entries to be read between filesystem polls. New log entries will be skipped if their size is larger than max_surge_size. Protects against lagging behind when reading log files that suddenly burst in size.                     |
