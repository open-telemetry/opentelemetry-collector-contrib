...
| `gzip_file_suffix`                    |                                      | File suffix for detecting gzip compressed log files. If set, files with that suffix will be read using a gzip reader that uncompresses the file before scanning its content                                                                                     |
...
The above configuration will be able to read gzip compressed log files by setting the `gzip_file_suffix` option to `.gz`.
