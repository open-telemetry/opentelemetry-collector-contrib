...
| `header`                            | nil                                  | Specifies options for parsing header metadata. Requires that the `filelog.allowHeaderMetadataParsing` feature gate is enabled. See below for details. Cannot use if `start_at` is set to "end".                                                                     |
...
| `retry_on_failure.initial_interval` | `1s`                                 | Time to wait after the first failure before retrying. Takes `time.Duration` as value.                                                                                                                                                                               |
| `retry_on_failure.max_interval`     | `30s`                                | Upper bound on retry backoff interval. Once this value is reached the delay between consecutive retries will remain constant at the specified value. Takes `time.Duration` as value.                                                                                |
| `retry_on_failure.max_elapsed_time` | `5m`                                 | Maximum amount of time (including retries) spent trying to send a logs batch to a downstream consumer. Once this value is reached, the data is discarded. Retrying never stops if set to `0`. Takes `time.Duration` as value.                                       |
=======
| Field                               | Default                              | Description                                                                                                                                                                                                                                                                                 |
|-------------------------------------|--------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
