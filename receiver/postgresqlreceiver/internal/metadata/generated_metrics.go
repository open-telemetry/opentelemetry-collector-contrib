...
var MetricsInfo = metricsInfo{
	PostgresqlBackends: metricInfo{
		...
		Attributes: []string{"db.namespace", "session_state", "session_wait_event_type"},
	},
...
func (m *metricPostgresqlBackends) init() {
	...
	m.data.SetDescription("The number of backend processes associated with each database, broken down by connection state and wait event type. Counts all backend types, including non-client backends such as autovacuum and parallel workers.")
	m.data.SetUnit("1")
	m.data.SetEmptySum()
...
func (m *metricPostgresqlBackends) recordDataPoint(start pcommon.Timestamp, ts pcommon.Timestamp, val int64, dbNamespaceAttributeValue string, sessionStateAttributeValue string, sessionWaitEventTypeAttributeValue string) {
	if !m.config.Enabled {
		return
	}
	...
	if slices.Contains(m.config.EnabledAttributes, PostgresqlBackendsMetricAttributeKeySessionWaitEventType) {
		dp.Attributes().PutStr("wait_event_type", sessionWaitEventTypeAttributeValue)
	}
...
func (mb *MetricsBuilder) RecordPostgresqlBackendsDataPoint(ts pcommon.Timestamp, val int64, dbNamespaceAttributeValue string, sessionStateAttributeValue string, sessionWaitEventTypeAttributeValue string) {
	mb.metricPostgresqlBackends.recordDataPoint(mb.startTime, ts, val, dbNamespaceAttributeValue, sessionStateAttributeValue, sessionWaitEventTypeAttributeValue)
}
