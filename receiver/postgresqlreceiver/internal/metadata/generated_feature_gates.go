...
var ReceiverPostgresqlUseOTelSemconvFeatureGate = featuregate.GlobalRegistry().MustRegister(
	...
	featuregate.StageAlpha,
	featuregate.WithRegisterDescription("When enabled, uses a single resource per server with a service.instance.id (UUID v5) resource attribute, aligning with OpenTelemetry semantic conventions. When disabled, uses the legacy per-entity resource model with postgresql.database.name, postgresql.table.name, postgresql.index.name, and postgresql.schema.name resource attributes."),
