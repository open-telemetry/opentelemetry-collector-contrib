...
type metricsDefinitionMockInput struct {
	...
	timeGrain             string
	dimensions            []string
	supportedAggregations []armmonitor.AggregationType
}
...
func newMetricsDefinitionMockData(inputMap map[string][]metricsDefinitionMockInput) map[string][]armmonitor.MetricDefinitionsClientListResponse {
	...
	for resourceID, inputSlice := range inputMap {
		...
		for i, input := range inputSlice {
			...
			if input.supportedAggregations == nil {
				input.supportedAggregations = armmonitor.PossibleAggregationTypeValues()
			}
			for _, aggregation := range input.supportedAggregations {
				toAppend.SupportedAggregationTypes = append(toAppend.SupportedAggregationTypes, &aggregation)
			}
