...
# check-mdatagen-generate verifies that every module with a metadata.yaml and mdatagen-generated
# files (identified by the "Code generated by mdatagen" marker) also has a //go:generate make mdatagen
# directive wired up, so that "make generate" keeps generated files in sync with metadata.yaml.
# Sub-packages without their own Makefile are driven by their parent module's doc.go and are excluded.
...
check-mdatagen-generate:
	...
	while IFS= read -r metadata_yaml; do \
	  ...
	  if ! grep -l "Code generated by mdatagen" "$$dir"/*.go "$$dir"/*.md 2>/dev/null | grep -q .; then continue; fi; \
	  if ! grep -rql "go:generate make mdatagen" "$$dir"/*.go 2>/dev/null; then \
	    echo "ERROR: $$dir has metadata.yaml and mdatagen-generated files but is missing '//go:generate make mdatagen' in a .go file (e.g. doc.go)."; \
	    echo "       To fix: add a doc.go with '//go:generate make mdatagen' or run 'make generate-mdatagen-docs'."; \
	    failed=1; \
	  fi; \
	done < <(find $(SRC_ROOT) -name "metadata.yaml" -not -path "*/vendor/*"); \
	if [ "$$failed" -ne 0 ]; then exit 1; fi; \
	echo "All modules with mdatagen-generated files have //go:generate make mdatagen wired up."
