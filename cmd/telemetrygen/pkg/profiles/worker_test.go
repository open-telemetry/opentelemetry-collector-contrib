...
func TestStackDepth(t *testing.T) {
	...
	// Each stack should have `depth` location indices
...
func TestUniqueFunctions(t *testing.T) {
	...
	assert.Equal(t, uniqueFuncs, dict.FunctionTable().Len(),
		"function table should have exactly %d functions", uniqueFuncs)
}
...
func TestProfilesWithNoTraceCorrelation(t *testing.T) {
	...
	// LinkTable should be empty
	assert.Equal(t, 0, dict.LinkTable().Len(), "link table should be empty")
...
func TestBatchingDictionaryIntegrity(t *testing.T) {
	...
	assert.Equal(t, 10, dict.FunctionTable().Len(), "FunctionTable should have 10 functions")
	assert.Equal(t, 10, dict.LocationTable().Len(), "LocationTable should have one location per function")
	assert.Positive(t, dict.StackTable().Len(), "StackTable should not be empty")

	fn := dict.FunctionTable().At(0)
