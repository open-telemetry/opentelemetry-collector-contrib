...
> [!NOTE]
> The profiles signal is still in alpha in the OpenTelemetry Collector. You
> must run a recent collector with `--feature-gates=service.profilesSupport`.
> The profile data structures are not yet stable, so the collector and
> telemetrygen versions must be compatible.
