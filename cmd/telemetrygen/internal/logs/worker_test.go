...
func TestRateOfLogsLessThan1(t *testing.T) {
	...
	// verify
	// the minimum acceptable number of logs for the rate of .5/sec for 3 seconds
	assert.True(t, len(exp.logs) >= 2, "there should have been 2 or more logs, had %d", len(exp.logs))
	// the maximum acceptable number of logs for the rate of .5/sec for 3 seconds
	assert.True(t, len(exp.logs) <= 3, "there should have been less than 3 logs, had %d", len(exp.logs))
}
