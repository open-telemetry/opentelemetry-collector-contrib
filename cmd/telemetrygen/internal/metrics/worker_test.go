...
func TestRateOfMetricsLessThan1(t *testing.T) {
	...
	// assert
	// the minimum acceptable number of metrics for the rate of .5/sec for 3 seconds
	assert.True(t, len(m.rms) >= 2, "there should have been more than 2 metrics, had %d", len(m.rms))
	// the maximum acceptable number of metrics for the rate of .5/sec for 3 seconds
	assert.True(t, len(m.rms) <= 3, "there should have been less than 2 metrics, had %d", len(m.rms))
}
