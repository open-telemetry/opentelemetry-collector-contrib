		...
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Eventually(t, func() bool {
				...
				for _, attr := range identAttr {
					...
					case string(semconv.ServiceNameVersion):
...
func TestSupervisorAgentDescriptionConfigApplies(t *testing.T) {
	...
	expectedDescription := &protobufs.AgentDescription{
		IdentifyingAttributes: []*protobufs.KeyValue{
			...
			stringKeyValue(string(semconv.ServiceInstanceID), uuid.UUID(ad.InstanceUid).String()),
			stringKeyValue(string(semconv.ServiceNameKey), command),
			stringKeyValue(string(semconv.ServiceNameVersion), version),
		},
