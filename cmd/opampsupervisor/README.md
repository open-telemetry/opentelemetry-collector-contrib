...
The Supervisor can include the tail of the Collector's logs in health and remote config failure messages when the Collector process exits unexpectedly. This is disabled by default because Collector logs may contain sensitive data. To enable it, set `agent::collector_crash_log_snippet_bytes` to the maximum number of log bytes to include, such as `102400` for 100 KiB. A value of `0` disables the feature. The maximum value is `1048576` (1 MiB).
