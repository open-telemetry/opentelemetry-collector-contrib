...
const defaultFileName = "configwiz-out.yaml"
...
func CLI(io Clio, factories component.Factories) {
	...
	extensions := componentListWizard(io, pr, "extension", extensionNames(factories, isExtension))
	...
	if extensions != nil {
		handleComponent(io, factories, m, "extension", extensions, dr)
		service["extensions"] = extensions
	}

	yamlContents := buildYamlFile(m)
	pr.println(fmt.Sprintf("# Wrote the following to %q:", fileName))
...
func promptFileName(io Clio) string {
	pr := io.newIndentingPrinter(0)
	pr.println(fmt.Sprintf("Name of file (enter for default: %q):", defaultFileName))
	pr.print("> ")
	fileName := io.Read(defaultFileName)
