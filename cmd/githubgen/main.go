...
func main() {
	...
	issueTemplatesOnly := flag.Bool("issue-templates", false, "only generate issue templates")
	flag.Parse()
	if err := run(*folder, *allowlistFilePath, *dependabotOnly, *issueTemplatesOnly); err != nil {
		log.Fatal(err)
	}
}
...
func run(folder string, allowlistFilePath string, dependabotOnly bool, issueTemplatesOnly bool) error {
...
func generateIssueTemplates(folders []string) error {
	shortNames := make([]string, len(folders))
...
func generateDependabot(dependabotData *DependabotData) error {
