...
func run(ymlPath string) error {
	...
	if md.Status.Scaffold {
		if _, err = os.Stat(filepath.Join(ymlDir, "config.go")); os.IsNotExist(err) {
			if err = generateFile(filepath.Join(tmplDir, "config.go.tmpl"), filepath.Join(ymlDir, "config.go"), md); err != nil {
				return err
			}
		}
		if err = generateFile(filepath.Join(tmplDir, "factory_generated.go.tmpl"), filepath.Join(ymlDir, "factory_generated.go"), md); err != nil {
			return err
		}
		if err = generateFile(filepath.Join(tmplDir, "doc.go.tmpl"), filepath.Join(ymlDir, "doc.go"), md); err != nil {
			return err
		}
		if _, err = os.Stat(filepath.Join(ymlDir, "factory.go")); os.IsNotExist(err) {
			if err = generateFile(filepath.Join(tmplDir, "factory.go.tmpl"), filepath.Join(ymlDir, "factory.go"), md); err != nil {
				return err
			}
		}
		if _, err = os.Stat(filepath.Join(ymlDir, "go.mod")); os.IsNotExist(err) {
			if err = generateFile(filepath.Join(tmplDir, "go.mod.tmpl"), filepath.Join(ymlDir, "go.mod"), md); err != nil {
				return err
			}
		}
		if _, err = os.Stat(filepath.Join(ymlDir, "README.md")); os.IsNotExist(err) {
			if err = generateFile(filepath.Join(tmplDir, "scaffold-README.md.tmpl"), filepath.Join(ymlDir, "README.md"), md); err != nil {
				return err
			}
		}
	}
