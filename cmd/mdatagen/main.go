...
const (
	...
	scaffoldOff     = "// scaffold-off"
	scaffoldOn      = "// scaffold-on"
	scaffoldHtmlOff = "<!-- start manual content -->"
	scaffoldHtmlOn  = "<!-- end manual content -->"
)
...
func run(ymlPath string) error {
	...
	if md.Status.Scaffold {
		if err = inlinePreserve(filepath.Join(tmplDir, "config.go.tmpl"), filepath.Join(ymlDir, "config.go"), md, scaffoldOff, scaffoldOn); err != nil {
			return err
		}
		if err = inlinePreserve(filepath.Join(tmplDir, "doc.go.tmpl"), filepath.Join(ymlDir, "doc.go"), md, scaffoldOff, scaffoldOn); err != nil {
			return err
		}
		if err = inlinePreserve(filepath.Join(tmplDir, "factory.go.tmpl"), filepath.Join(ymlDir, "factory.go"), md, scaffoldOff, scaffoldOn); err != nil {
			return err
		}
		if err = inlinePreserve(filepath.Join(tmplDir, "go.mod.tmpl"), filepath.Join(ymlDir, "go.mod"), md, scaffoldOff, scaffoldOn); err != nil {
			return err
		}
		if err = inlinePreserve(filepath.Join(tmplDir, "scaffold-README.md.tmpl"), filepath.Join(ymlDir, "README.md"), md, scaffoldHtmlOff, scaffoldHtmlOn); err != nil {
			return err
		}
	}
...
func inlinePreserve(tmplFile string, outputFile string, md metadata, start string, end string) error {
	var contents []byte
	var err error
	if contents, err = os.ReadFile(outputFile); err != nil && errors.Is(os.ErrNotExist, err) {
		fmt.Println(err)
		return err
	}

	var re = regexp.MustCompile(fmt.Sprintf("\\s*%s([\\s\\S]*?)%s", start, end))
	toPreserve := re.FindAll(contents, -1)

	tmpl := templatize(tmplFile, md)
	buf := bytes.Buffer{}

	if err := tmpl.Execute(&buf, templateContext{metadata: md}); err != nil {
		return fmt.Errorf("failed executing template: %w", err)
	}
	...
	result = re.ReplaceAllFunc(result, func(b []byte) []byte {
		if index >= len(toPreserve) {
			return []byte(fmt.Sprintf("\n%s\n%s", start, end))
		}
		candidate := toPreserve[index]
		index++
		return candidate
	})
