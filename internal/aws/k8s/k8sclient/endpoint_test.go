...
func TestEpClient_PodKeyToServiceNames(t *testing.T) {
	...
	// awsutil.Prettify를 encoding/json으로 대체
	prettyJSON, _ := json.MarshalIndent(resultMap, "", "  ")
	log.Printf("PodKeyToServiceNames (len=%v): %s", len(resultMap), string(prettyJSON))
...
func TestEpClient_ServiceNameToPodNum(t *testing.T) {
	...
	// awsutil.Prettify를 encoding/json으로 대체
	prettyJSON, _ := json.MarshalIndent(resultMap, "", "  ")
	log.Printf("ServiceNameToPodNum (len=%v): %s", len(resultMap), string(prettyJSON))
