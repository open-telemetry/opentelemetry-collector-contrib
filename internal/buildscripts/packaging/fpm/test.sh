...
PKG_PATH="${1:-}"
CONFIG_PATH="${2:-"$REPO_DIR/examples/tracing/otel-collector-config.yml"}"

. $SCRIPT_DIR/common.sh

if [[ -z "$PKG_PATH" ]]; then
    echo "usage: ${BASH_SOURCE[0]} DEB_OR_RPM_PATH [CONFIG_PATH]" >&2
    exit 1
fi
...
# test service start after adding config
echo
docker_cp $container_name "$CONFIG_PATH" /etc/otel-contrib-collector/config.yaml

echo "Starting $SERVICE_NAME service ..."
$docker_exec systemctl start $SERVICE_NAME
...
# test install with existing config, service should auto start
echo
$docker_run
docker_cp $container_name "$CONFIG_PATH" /etc/otel-contrib-collector/config.yaml
install_pkg $container_name "$PKG_PATH"
