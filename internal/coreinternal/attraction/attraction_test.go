...
func TestAttributes_InsertValue(t *testing.T) {
	testCases := []testCase{
		...
		// Ensure `attribute1` is set.
		{
			...
			inputAttributes: map[string]pdata.Value{
				"anotherkey": pdata.NewValueString("bob"),
			},
			expectedAttributes: map[string]pdata.Value{
				"anotherkey": pdata.NewValueString("bob"),
				"attribute1": pdata.NewValueInt(123),
		...
		// Ensures no insert is performed because the keys `attribute1` already exists.
		{
			...
			inputAttributes: map[string]pdata.Value{
				"attribute1": pdata.NewValueString("bob"),
			},
			expectedAttributes: map[string]pdata.Value{
				"attribute1": pdata.NewValueString("bob"),
			},
...
func TestAttributes_InsertFromAttribute(t *testing.T) {
	...
	testCases := []testCase{
		...
		// Ensure `string key` is set.
		{
			...
			inputAttributes: map[string]pdata.Value{
				"anotherkey": pdata.NewValueInt(8892342),
			},
			expectedAttributes: map[string]pdata.Value{
				"anotherkey": pdata.NewValueInt(8892342),
				"string key": pdata.NewValueInt(8892342),
			},
		},
		// Ensures no insert is performed because the keys `string key` already exist.
		{
			...
			inputAttributes: map[string]pdata.Value{
				"anotherkey": pdata.NewValueInt(8892342),
				"string key": pdata.NewValueString("here"),
			},
			expectedAttributes: map[string]pdata.Value{
				"anotherkey": pdata.NewValueInt(8892342),
				"string key": pdata.NewValueString("here"),
...
func TestAttributes_UpdateValue(t *testing.T) {
	...
	testCases := []testCase{
		...
		// Ensure the attribute `db.secret` is updated.
		{
			...
			inputAttributes: map[string]pdata.Value{
				"db.secret": pdata.NewValueString("password1234"),
			},
			expectedAttributes: map[string]pdata.Value{
				"db.secret": pdata.NewValueString("redacted"),
			},
		},
	}
...
func TestAttributes_UpdateFromAttribute(t *testing.T) {
	...
	testCases := []testCase{
		...
		// Ensure no updates as the target key `boo` doesn't exists.
		{
			...
			inputAttributes: map[string]pdata.Value{
				"foo": pdata.NewValueString("over there"),
			},
			expectedAttributes: map[string]pdata.Value{
				"foo": pdata.NewValueString("over there"),
			},
		...
		// Ensure no updates as the target key `boo` doesn't exists.
		{
			...
			inputAttributes: map[string]pdata.Value{
				"foo": pdata.NewValueString("there is a party over here"),
				"boo": pdata.NewValueString("not here"),
			},
			expectedAttributes: map[string]pdata.Value{
				"foo": pdata.NewValueString("there is a party over here"),
				"boo": pdata.NewValueString("there is a party over here"),
			},
		},
	}
...
func TestAttributes_UpsertValue(t *testing.T) {
	testCases := []testCase{
		// Ensure `region` is set for spans with no attributes.
		{
			...
			inputAttributes: map[string]pdata.Value{},
			expectedAttributes: map[string]pdata.Value{
				"region": pdata.NewValueString("planet-earth"),
			},
		},
		// Ensure `region` is inserted for spans with some attributes(the key doesn't exist).
		{
			...
			inputAttributes: map[string]pdata.Value{
				"mission": pdata.NewValueString("to mars"),
			},
			expectedAttributes: map[string]pdata.Value{
				"mission": pdata.NewValueString("to mars"),
				"region":  pdata.NewValueString("planet-earth"),
		...
		// Ensure `region` is updated for spans with the attribute key `region`.
		{
			...
			inputAttributes: map[string]pdata.Value{
				"mission": pdata.NewValueString("to mars"),
				"region":  pdata.NewValueString("solar system"),
			},
			expectedAttributes: map[string]pdata.Value{
				"mission": pdata.NewValueString("to mars"),
				"region":  pdata.NewValueString("planet-earth"),
...
func TestAttributes_Extract(t *testing.T) {
	testCases := []testCase{
		...
		// Ensure `new_user_key` is not inserted for spans with missing attribute `user_key`.
		{
			...
			inputAttributes: map[string]pdata.Value{
				"boo": pdata.NewValueString("ghosts are scary"),
			},
			expectedAttributes: map[string]pdata.Value{
				"boo": pdata.NewValueString("ghosts are scary"),
			},
		...
		// Ensure `new_user_key` is not inserted for spans with missing attribute `user_key`.
		{
			...
			inputAttributes: map[string]pdata.Value{
				"boo":      pdata.NewValueString("ghosts are scary"),
				"user_key": pdata.NewValueInt(1234),
			},
			expectedAttributes: map[string]pdata.Value{
				"boo":      pdata.NewValueString("ghosts are scary"),
				"user_key": pdata.NewValueInt(1234),
		...
		// `user_key` because `user_key` does not match the regular expression.
		{
			...
			inputAttributes: map[string]pdata.Value{
				"user_key": pdata.NewValueString("does not match"),
				"boo":      pdata.NewValueString("ghosts are scary"),
			},
			expectedAttributes: map[string]pdata.Value{
				"user_key": pdata.NewValueString("does not match"),
		...
		// expression.
		{
			name: "No extract with no pattern matching",
			inputAttributes: map[string]pdata.Value{
				"user_key": pdata.NewValueString("/api/v1/document/12345678/update"),
				"boo":      pdata.NewValueString("ghosts are scary"),
			},
			expectedAttributes: map[string]pdata.Value{
				"user_key": pdata.NewValueString("/api/v1/document/12345678/update"),
		...
		// Ensure `new_user_key` and `version` is inserted for spans with attribute `user_key`.
		{
			...
			inputAttributes: map[string]pdata.Value{
				"user_key": pdata.NewValueString("/api/v1/document/12345678/update/v1"),
				"foo":      pdata.NewValueString("casper the friendly ghost"),
			},
			expectedAttributes: map[string]pdata.Value{
				"user_key":     pdata.NewValueString("/api/v1/document/12345678/update/v1"),
				"new_user_key": pdata.NewValueString("12345678"),
		...
		// Ensure `new_user_key` and `version` is updated for spans with attribute `user_key`.
		{
			...
			inputAttributes: map[string]pdata.Value{
				"user_key":     pdata.NewValueString("/api/v1/document/12345678/update/v1"),
				"new_user_key": pdata.NewValueString("2321"),
				"version":      pdata.NewValueString("na"),
				"foo":          pdata.NewValueString("casper the friendly ghost"),
			},
			expectedAttributes: map[string]pdata.Value{
				"user_key":     pdata.NewValueString("/api/v1/document/12345678/update/v1"),
...
func TestAttributes_UpsertFromAttribute(t *testing.T) {
	...
	testCases := []testCase{
		...
		// Ensure `new_user_key` is inserted for spans with attribute `user_key`.
		{
			...
			inputAttributes: map[string]pdata.Value{
				"user_key": pdata.NewValueInt(2245),
				"foo":      pdata.NewValueString("casper the friendly ghost"),
			},
			expectedAttributes: map[string]pdata.Value{
				"user_key":     pdata.NewValueInt(2245),
				"new_user_key": pdata.NewValueInt(2245),
				"foo":          pdata.NewValueString("casper the friendly ghost"),
			},
		},
		// Ensure `new_user_key` is updated for spans with attribute `user_key`.
		{
			...
			inputAttributes: map[string]pdata.Value{
				"user_key":     pdata.NewValueInt(2245),
				"new_user_key": pdata.NewValueInt(5422),
				"foo":          pdata.NewValueString("casper the friendly ghost"),
			},
			expectedAttributes: map[string]pdata.Value{
				"user_key":     pdata.NewValueInt(2245),
...
func TestAttributes_Delete(t *testing.T) {
	testCases := []testCase{
		...
		// Ensure `duplicate_key` is deleted for spans with the attribute set.
		{
			...
			inputAttributes: map[string]pdata.Value{
				"duplicate_key": pdata.NewValueDouble(3245.6),
				"original_key":  pdata.NewValueDouble(3245.6),
			},
			expectedAttributes: map[string]pdata.Value{
				"original_key": pdata.NewValueDouble(3245.6),
			},
...
func TestAttributes_HashValue(t *testing.T) {
	...
	testCases := []testCase{
		...
		// Ensure string data types are hashed correctly
		{
			name: "HashString",
			inputAttributes: map[string]pdata.Value{
				"updateme": pdata.NewValueString("foo"),
			},
			expectedAttributes: map[string]pdata.Value{
				"updateme": pdata.NewValueString(sha1Hash([]byte("foo"))),
			},
		},
		// Ensure int data types are hashed correctly
		{
			name: "HashInt",
			inputAttributes: map[string]pdata.Value{
				"updateme": pdata.NewValueInt(intVal),
			},
			expectedAttributes: map[string]pdata.Value{
				"updateme": pdata.NewValueString(sha1Hash(intBytes)),
			},
		},
		// Ensure double data types are hashed correctly
		{
			name: "HashDouble",
			inputAttributes: map[string]pdata.Value{
				"updateme": pdata.NewValueDouble(doubleVal),
			},
			expectedAttributes: map[string]pdata.Value{
				"updateme": pdata.NewValueString(sha1Hash(doubleBytes)),
			},
		},
		// Ensure bool data types are hashed correctly
		{
			name: "HashBoolTrue",
			inputAttributes: map[string]pdata.Value{
				"updateme": pdata.NewValueBool(true),
			},
			expectedAttributes: map[string]pdata.Value{
				"updateme": pdata.NewValueString(sha1Hash([]byte{1})),
			},
		},
		// Ensure bool data types are hashed correctly
		{
			name: "HashBoolFalse",
			inputAttributes: map[string]pdata.Value{
				"updateme": pdata.NewValueBool(false),
			},
			expectedAttributes: map[string]pdata.Value{
				"updateme": pdata.NewValueString(sha1Hash([]byte{0})),
			},
		},
	}
...
func TestAttributes_Ordering(t *testing.T) {
	testCases := []testCase{
		...
		// 3. delete `operation`.
		{
			...
			expectedAttributes: map[string]pdata.Value{
				"foo":           pdata.NewValueString("casper the friendly ghost"),
				"svc.operation": pdata.NewValueString("default"),
			},
		},
		// For this example, the operations performed are
		...
		// 3. delete `operation`.
		{
			...
			inputAttributes: map[string]pdata.Value{
				"foo":       pdata.NewValueString("casper the friendly ghost"),
				"operation": pdata.NewValueString("arithmetic"),
			},
			expectedAttributes: map[string]pdata.Value{
				"foo":           pdata.NewValueString("casper the friendly ghost"),
				"svc.operation": pdata.NewValueString("arithmetic"),
			},
		},
		...
		// 3. delete `operation`.
		{
			...
			inputAttributes: map[string]pdata.Value{
				"foo":           pdata.NewValueString("casper the friendly ghost"),
				"svc.operation": pdata.NewValueString("some value"),
			},
			expectedAttributes: map[string]pdata.Value{
				"foo":           pdata.NewValueString("casper the friendly ghost"),
				"svc.operation": pdata.NewValueString("default"),
		...
		// 3. delete `operation`.
		{
			...
			inputAttributes: map[string]pdata.Value{
				"foo":           pdata.NewValueString("casper the friendly ghost"),
				"operation":     pdata.NewValueString("arithmetic"),
				"svc.operation": pdata.NewValueString("add"),
			},
			expectedAttributes: map[string]pdata.Value{
				"foo":           pdata.NewValueString("casper the friendly ghost"),
				"svc.operation": pdata.NewValueString("arithmetic"),
...
func TestFromContext(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			name:               "single_value",
			ctx:                mdCtx,
			expectedAttributes: map[string]pdata.Value{"dest": pdata.NewValueString("single_val")},
			action:             &ActionKeyValue{Key: "dest", FromContext: "source_single_val", Action: INSERT},
		},
		{
			name:               "multiple_values",
			ctx:                mdCtx,
			expectedAttributes: map[string]pdata.Value{"dest": pdata.NewValueString("first_val;second_val")},
