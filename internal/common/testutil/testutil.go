...
func SetFeatureGateForTest(t testing.TB, gate string, enabled bool) func() {
	originalValue := featuregate.GlobalRegistry().IsEnabled(gate)
	require.NoError(t, featuregate.GlobalRegistry().Apply(map[string]bool{gate: enabled}))
	return func() {
		require.NoError(t, featuregate.GlobalRegistry().Apply(map[string]bool{gate: originalValue}))
	}
}
