...
func Test_NewSkipExpr_With_Bridge(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tCtx := ottlmetric.NewTransformContext(rm, pmetric.NewScopeMetrics(), metric)
			defer tCtx.Close()
...
func TestMatcherMatchesResourceAttributes(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			tCtx := ottlmetric.NewTransformContext(rm, pmetric.NewScopeMetrics(), createMetric(test.metricName))
			matches, err := matcher.Eval(t.Context(), tCtx)
			tCtx.Close()
