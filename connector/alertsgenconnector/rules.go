...
func compileRules(cfg *Config) (*ruleSet, error) {
    ...
    for _, rc := range cfg.Rules {
        ...
        case "traces": rs.traceRules = append(rs.traceRules, cr)
        case "logs": rs.logRules = append(rs.logRules, cr)
        case "metrics": rs.metricRules = append(rs.metricRules, cr)
        default: return nil, fmt.Errorf("unknown signal %q", rc.Signal)
        }
    }
    return rs, nil
}
...
func compileRule(rc RuleCfg, cfg *Config) (*compiledRule, error) {
    if rc.Name == "" { return nil, errors.New("rule.name required") }
    if rc.Window == 0 { rc.Window = cfg.WindowSize }
    if rc.Step == 0 { rc.Step = cfg.WindowSize }
    if rc.Expr.Type == "" { return nil, fmt.Errorf("rule %s expr.type required", rc.Name) }

    sel := map[string]*regexp.Regexp{}
    for k,v := range rc.Select {
        re, err := regexp.Compile(v)
        if err != nil { return nil, fmt.Errorf("rule %s: bad select regex for %s: %v", rc.Name, k, err) }
        sel[k] = re
    }

    expr, err := buildExpr(rc.Expr)
    if err != nil { return nil, fmt.Errorf("rule %s: %v", rc.Name, err) }
...
type exprAvg struct { Field, Op string; Threshold float64 }
func (e *exprAvg) kind() string { return "avg" }
func (e *exprAvg) evaluateTrace(rows []traceRow) (float64,bool) {
    if len(rows)==0 { return 0,false }
    var sum float64; for _,r := range rows { sum += r.durationNs }
    return sum/float64(len(rows)), true
}
func (e *exprAvg) evaluateLog(rows []logRow) (float64,bool) { if len(rows)==0 { return 0,false }; return float64(len(rows)), true }
func (e *exprAvg) evaluateMetric(rows []metricRow) (float64,bool) {
    if len(rows)==0 { return 0,false }
    var sum float64; for _,r := range rows { sum += r.value }
    return sum/float64(len(rows)), true
}
func (e *exprAvg) compare(v float64) bool { return cmp(e.Op, v, e.Threshold) }

type exprRate struct { Field, Op string; Threshold float64 }
func (e *exprRate) kind() string { return "rate" }
func (e *exprRate) evaluateTrace(rows []traceRow) (float64,bool) { return float64(len(rows)), len(rows)>0 }
func (e *exprRate) evaluateLog(rows []logRow) (float64,bool) { return float64(len(rows)), len(rows)>0 }
func (e *exprRate) evaluateMetric(rows []metricRow) (float64,bool) { return float64(len(rows)), len(rows)>0 }
func (e *exprRate) compare(v float64) bool { return cmp(e.Op, v, e.Threshold) }

type exprCount struct { Field, Op string; Threshold float64 }
func (e *exprCount) kind() string { return "count" }
func (e *exprCount) evaluateTrace(rows []traceRow) (float64,bool) { return float64(len(rows)), len(rows)>0 }
func (e *exprCount) evaluateLog(rows []logRow) (float64,bool) { return float64(len(rows)), len(rows)>0 }
func (e *exprCount) evaluateMetric(rows []metricRow) (float64,bool) { return float64(len(rows)), len(rows)>0 }
func (e *exprCount) compare(v float64) bool { return cmp(e.Op, v, e.Threshold) }

type exprQuantile struct { Field string; Q float64; Op string; Threshold float64 }
func (e *exprQuantile) kind() string { return "quantile" }
func (e *exprQuantile) evaluateTrace(rows []traceRow) (float64,bool) {
    if len(rows)==0 { return 0,false }
    vals := make([]float64, len(rows)); for i,r := range rows { vals[i]=r.durationNs }
    sort.Float64s(vals)
...
func (e *exprQuantile) evaluateLog(rows []logRow) (float64,bool) { return float64(len(rows)), len(rows)>0 }
func (e *exprQuantile) evaluateMetric(rows []metricRow) (float64,bool) {
    if len(rows)==0 { return 0,false }
    vals := make([]float64, len(rows)); for i,r := range rows { vals[i]=r.value }
    sort.Float64s(vals)
    idx := int(float64(len(vals)-1)*e.Q)
    if idx < 0 { idx = 0 }
    if idx >= len(vals) { idx = len(vals)-1 }
...
func (rs *ruleSet) evaluate(now time.Time, ing *ingester) ([]alertEvent, []stateMetric) {
    ...
    for _, cr := range rs.traceRules {
        ...
        for gk, rows := range groups {
            ...
            if len(rows)==0 && cr.expr.kind()!="absent" { continue }
...
func filterWindowTr(rows []traceRow, now time.Time, w time.Duration) []traceRow {
    ...
    out := rows[:0]
    for _,r := range rows {
        if r.ts.After(cutoff) { out = append(out, r) }
    }
    return out
}
...
func filterTraceRows(rows []traceRow, sel map[string]*regexp.Regexp) []traceRow {
    if len(sel)==0 { return rows }
    out := make([]traceRow, 0, len(rows))
    for _, r := range rows {
        if matchSel(r.attrs, sel) { out = append(out, r) }
    }
    return out
}
...
func fpMap(m map[string]string) []string {
    ...
    for _,k := range ks { out = append(out, k, m[k]) }
    return out
}
