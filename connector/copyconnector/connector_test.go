...
import (
	...
	"github.com/open-telemetry/opentelemetry-collector-contrib/connector/copyconnector/mocks"
)
...
func verifyConsumeMetricsInput(t testing.TB, input pmetric.Metrics, expectedTemporality pmetric.AggregationTemporality, numCumulativeConsumptions int) bool {
	...
	for i := 0; i < input.ResourceMetrics().Len(); i++ {
		...
		assert.Equal(t, "copyconnector", ilm.At(0).Scope().Name())
...
func TestConcurrentShutdown(t *testing.T) {
	...
	// Starting copyconnector...
	// Shutting down copyconnector...
	// Stopping ticker.
