...
func TestCopyResourcesIf(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			plogutil.CopyResourcesIf(tt.from, tt.to, tt.moveIf)
			assert.NoError(t, plogtest.CompareLogs(tt.expectFrom, tt.from), "from not modified as expected")
			assert.NoError(t, plogtest.CompareLogs(tt.expectTo, tt.to), "to not as expected")
		})
	}
...
func TestCopyRecordsWithContextIf(t *testing.T) {
	testCases := []struct {
		from       plog.Logs
		to         plog.Logs
		expectFrom plog.Logs
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			plogutil.CopyRecordsWithContextIf(tt.from, tt.to, tt.moveIf)
			assert.NoError(t, plogtest.CompareLogs(tt.expectFrom, tt.from), "from not modified as expected")
			assert.NoError(t, plogtest.CompareLogs(tt.expectTo, tt.to), "to not as expected")
		})
	}
