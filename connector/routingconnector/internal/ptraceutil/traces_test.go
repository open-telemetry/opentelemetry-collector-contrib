...
func TestCopyResourcesIf(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			ptraceutil.CopyResourcesIf(tt.from, tt.to, tt.moveIf)
			assert.NoError(t, ptracetest.CompareTraces(tt.expectFrom, tt.from), "from not modified as expected")
			assert.NoError(t, ptracetest.CompareTraces(tt.expectTo, tt.to), "to not as expected")
		})
	}
...
func TestCopySpansWithContextIf(t *testing.T) {
	testCases := []struct {
		from       ptrace.Traces
		to         ptrace.Traces
		expectFrom ptrace.Traces
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			ptraceutil.CopySpansWithContextIf(tt.from, tt.to, tt.moveIf)
			assert.NoError(t, ptracetest.CompareTraces(tt.expectFrom, tt.from), "from not modified as expected")
			assert.NoError(t, ptracetest.CompareTraces(tt.expectTo, tt.to), "to not as expected")
		})
	}
