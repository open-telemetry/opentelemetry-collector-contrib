...
func (c *counter[K]) update(ctx context.Context, attrs pcommon.Map, tCtx K) error {
	...
	for name, md := range c.metricDefs {
		countAttrs := pcommon.NewMap()  // map of attributes?
		for _, attr := range md.attrs { // loop through arrtibutes of the metric defintion from counter
			if attrVal, ok := attrs.Get(attr.Key); ok {
				countAttrs.PutStr(attr.Key, attrVal.Str()) // if the attrubte is found, put it in the map?
