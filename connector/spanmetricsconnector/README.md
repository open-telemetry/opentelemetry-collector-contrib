...
> **Warning**: Using all resource attributes (default behavior) can cause counter metrics to reset or appear to decrease. This happens because changes in attributes like `process.pid`, `container.id`, or `host.name` create new metric time series. To prevent this:
...
The following resource attributes are always excluded from metric grouping when the feature gate is enabled:
- `process.pid`
- `container.id`
- `host.name`
