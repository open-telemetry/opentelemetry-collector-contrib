...
- `include_root_span_attribute` (default: `false`): When enabled, adds the `span.is_root_span` boolean attribute to metrics **for `SPAN_KIND_INTERNAL` spans only**. The value is `true` when the span has no parent (i.e. `parentSpanID` is empty), and `false` when it has a parent. This allows INTERNAL spans that are trace entry points (e.g. background jobs, scheduled tasks, batch processors) to be identified alongside conventional entry-point kinds such as `SERVER` and `CONSUMER`. The attribute is omitted for all other span kinds because `span.kind` already conveys entry-point semantics for those cases. Cardinality impact is negligible — the attribute carries exactly two possible values.
