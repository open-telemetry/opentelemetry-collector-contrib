...
With the configuration metrics generated by `spanmetrics` from received span will be exported with
the Prometheus Remote Write exporter. The `taget_info` metric will be generated for each resource
scope and OpenTelemetry metric names and attributes will be [normalized](../../exporter/prometheusremotewriteexporter/README.md)
to be compliant with Prometheus naming rules. For example, the generated `calls` OTel sum metric can
result in multiple Prometheus `calls_total` (counter type) time series and the `taget_info` time series.
For example,

```yaml
target_info{job="shippingservice", instnace="...", ...} 1
