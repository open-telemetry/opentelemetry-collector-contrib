...
func TestBuildAttributes_InstrumentationScope(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			config: Config{
				ExcludeInstrumentationScope: []string{"express"},
			},
			want: map[string]string{
				serviceNameKey: "test_service",
				spanNameKey:    "test_span",
		...
		{
			name: "with excluded instrumentation scope and no instrumentation scope name",
			instrumentationScope: func() pcommon.InstrumentationScope {
				scope := pcommon.NewInstrumentationScope()
				scope.SetVersion("1.0.0")
				return scope
		...
		{
			name: "with instrumentation scope and instrumentation scope name but no version",
			instrumentationScope: func() pcommon.InstrumentationScope {
				scope := pcommon.NewInstrumentationScope()
				scope.SetName("express")
				return scope
		...
		{
			name: "with excluded instrumentation scope and no instrumentation scope name and version",
			instrumentationScope: func() pcommon.InstrumentationScope {
				scope := pcommon.NewInstrumentationScope()
				return scope
			}(),
