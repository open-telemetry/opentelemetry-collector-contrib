...
func TestConnectorStart(t *testing.T) {
	...
	// Test
	smp := traceConnector.(*connectorImp)
	ctx := context.Background()
	err = smp.Start(ctx, componenttest.NewNopHost())
...
func TestConfigureLatencyBounds(t *testing.T) {
	...
	p, err := newConnector(zaptest.NewLogger(t), cfg, next, nil)
...
func newConnectorImp(mcon *mocks.MetricsConsumer, defaultNullValue *pcommon.Value, temporality string, logger *zap.Logger, ticker *clock.Ticker) *connectorImp {
