...
func (p *connectorImp) buildAttributes(serviceName string, span ptrace.Span, resourceAttrs pcommon.Map, dimensions []utilattri.Dimension, instrumentationScope pcommon.InstrumentationScope) pcommon.Map {
	...
	if !contains(p.config.ExcludeInstrumentationScope, instrumentationScopeNameKey) && instrumentationScope.Name() != "" {
		attr.PutStr(instrumentationScopeNameKey, instrumentationScope.Name())
	}
	if !contains(p.config.ExcludeInstrumentationScope, instrumentationScopeVersionKey) && instrumentationScope.Version() != "" {
		attr.PutStr(instrumentationScopeVersionKey, instrumentationScope.Version())
	}

	for _, p := range p.config.InstrumentationScope {
		attrs := instrumentationScope.Attributes()
		if v, ok := attrs.Get(p.Name); ok {
			attr.PutStr(p.Name, v.Str())
		} else if p.Default != nil {
			attr.PutStr(p.Name, *p.Default)
		}
	}
