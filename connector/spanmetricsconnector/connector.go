...
func newConnector(logger *zap.Logger, config component.Config, nextConsumer consumer.Metrics, ticker *clock.Ticker) (*connectorImp, error) {
	logger.Info("Building spanmetricsconnector")
...
func (p *connectorImp) exportMetrics(ctx context.Context) {
	...
	if err := p.nextConsumer.ConsumeMetrics(ctx, m); err != nil {
