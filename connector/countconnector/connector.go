	...
	for i := 0; i < td.ResourceSpans().Len(); i++ {
		...
		for j := 0; j < resourceSpan.ScopeSpans().Len(); j++ {
			...
			for k := 0; k < scopeSpan.Spans().Len(); k++ {
				...
				errors = errors.Join(errors, spansCounter.update(ctx, span.Attributes(), sCtx))

				for l := 0; l < span.Events().Len(); l++ {
					event := span.Events().At(l)
					eCtx := ottlspanevent.NewTransformContext(event, span, scopeSpan.Scope(), resourceSpan.Resource())
					errors = errors.Join(errors, spanEventsCounter.update(ctx, event.Attributes(), eCtx))
				}
			}
		}
	...
	for i := 0; i < md.ResourceMetrics().Len(); i++ {
		...
		for j := 0; j < resourceMetric.ScopeMetrics().Len(); j++ {
			...
			for k := 0; k < scopeMetrics.Metrics().Len(); k++ {
				...
				errors = errors.Join(errors, metricsCounter.update(ctx, pcommon.NewMap(), mCtx))
				...
				case pmetric.MetricTypeGauge:
					...
					for i := 0; i < dps.Len(); i++ {
						...
						errors = errors.Join(errors, dataPointsCounter.update(ctx, dps.At(i).Attributes(), dCtx))
					}
				...
				case pmetric.MetricTypeEmpty:
					errors = errors.Join(errors, fmt.Errorf("metric %q: invalid metric type: %v", metric.Name(), metric.Type()))
				}
			}
		}
	...
	for i := 0; i < ld.ResourceLogs().Len(); i++ {
		...
		for j := 0; j < resourceLog.ScopeLogs().Len(); j++ {
			...
			for k := 0; k < scopeLogs.LogRecords().Len(); k++ {
				...
				errors = errors.Join(errors, counter.update(ctx, logRecord.Attributes(), lCtx))
			}
		}
