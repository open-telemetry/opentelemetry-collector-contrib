		...
		for _, kind := range []string{"sum", "histogram", "exponential_histogram"} {
			t.Run(field+"/"+kind, func(t *testing.T) {
				run := func(sources []string, values []uint64, times []pcommon.Timestamp) map[string][]string {
					...
					cfg.InitialValue = tracking.InitialValueDrop
					cfg.MaxStaleness = 0
					...
					for i, source := range sources {
						...
						case "sum":
							...
							data.SetAggregationTemporality(pmetric.AggregationTemporalityCumulative)
				...
				joint := run([]string{"A", "B", "A"}, []uint64{1, 3, 3}, []pcommon.Timestamp{20, 25, 40})
				a := run([]string{"A", "A"}, []uint64{1, 3}, []pcommon.Timestamp{20, 40})
