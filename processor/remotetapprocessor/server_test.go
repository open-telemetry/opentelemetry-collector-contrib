...
func TestSocketConnectionLogs(t *testing.T) {
	...
	processor, err := NewFactory().CreateLogs(context.Background(), processortest.NewNopSettingsWithType(metadata.Type), cfg,
		logSink)
...
func TestSocketConnectionMetrics(t *testing.T) {
	...
	processor, err := NewFactory().CreateMetrics(context.Background(), processortest.NewNopSettingsWithType(metadata.Type), cfg,
		metricsSink)
	require.NoError(t, err)
	err = processor.Start(context.Background(), componenttest.NewNopHost())
	require.NoError(t, err)
...
func TestSocketConnectionTraces(t *testing.T) {
	...
	processor, err := NewFactory().CreateTraces(context.Background(), processortest.NewNopSettingsWithType(metadata.Type), cfg,
		tracesSink)
	require.NoError(t, err)
	err = processor.Start(context.Background(), componenttest.NewNopHost())
	require.NoError(t, err)
