...
3. After the interval, the processor emits a single log with the count of logs that were deduplicated. The emitted log will have the same body, resource attributes, severity, and log attributes as the original log. The emitted log will also have the following new attributes:
    ...
    - `first_event_timestamp`: The timestamp of the first log received during the aggregation interval (only present when `timestamp_mode` is `observed`).
...
**Note**: The emitted log's `ObservedTimestamp` is set to the processor receipt time of the first log observed in the window (`first_observed_timestamp`), not the export time. The `Timestamp` of the emitted log depends on the configured `timestamp_mode`:
...
| timezone            | string   | `UTC`       | The timezone of the timestamp attributes (`first_observed_timestamp`, `last_observed_timestamp`, `first_event_timestamp`, and `last_event_timestamp`) on the emitted aggregated log. The available locations depend on the local IANA Time Zone database. [This page](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones) contains many examples, such as `America/New_York`.                                                                                                                               |
| timestamp_mode      | string   | `observed`  | Controls how the `Timestamp` field of the emitted log is set. `observed`: set to when the aggregated log was emitted. `preserved`: preserved from the first log received in the aggregation window. In `observed` mode, `first_event_timestamp` is added to record the original log's timestamp; in `preserved` mode, `first_event_timestamp` is omitted as it is identical to `Timestamp`. In all modes, `last_event_timestamp` records the last received log's timestamp. |
