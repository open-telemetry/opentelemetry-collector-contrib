...
func (p *processorImp) collectLatencyMetrics(ilm pmetric.ScopeMetrics) error {
	for key := range p.latencyCount {
		...
		dpLatency.SetExplicitBounds(pcommon.NewImmutableFloat64SliceFromCopy(p.latencyBounds))
		dpLatency.SetBucketCounts(pcommon.NewImmutableUInt64SliceFromCopy(p.latencyBucketCounts[key]))
