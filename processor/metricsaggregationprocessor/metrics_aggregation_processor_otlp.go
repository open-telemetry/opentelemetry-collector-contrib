...
func matchAttrs(attrMatchers map[string]StringMatcher, attrs pcommon.Map) bool {
	for k, v := range attrMatchers {
		...
		// attribute values doesn'ts match, drop datapoint
...
func (mtp *metricsAggregationProcessor) processMetrics(_ context.Context, md pmetric.Metrics) (pmetric.Metrics, error) {
	...
	rms.RemoveIf(func(rm pmetric.ResourceMetrics) bool {
		rm.ScopeMetrics().RemoveIf(func(sm pmetric.ScopeMetrics) bool {
			...
			for _, transform := range mtp.transforms {
				...
				case Insert:
					// Save len, so we don'ts iterate over the newly generated metrics that are appended at the end.
					mLen := metrics.Len()
					for i := 0; i < mLen; i++ {
						metric := metrics.At(i)
...
func combine(transform internalTransform, metrics pmetric.MetricSlice) pmetric.Metric {
	...
	// TODO crl: we lost transform.new_name
	combinedMetric.SetName(firstMetric.Name())
...
func transformMetric(metric pmetric.Metric, transform internalTransform) bool {
	...
	canChangeMetric := matchAllDps(metric, transform.MetricIncludeFilter)
	//
	...
	if transform.Interval > 0 {
		// For multiple data points to be combined, they must have the same timestamp and start_timestamp
		// Otherwise, data points will pass-thru unaggregated
		// TODO how should we handle old data and future data, or out-of-order data?
		// Option 1: truncate into interval based on source timestamp (aka respect source timestamps) (default)
		// Option 2: truncate into interval based on current timestamp (aka ignore source timestamps)
		// Edge Cases:
		// 1: source timestamp is prior to max_age (drop or assume current?)
		// 2: source timestamp is after min_age (aka in the future)
		// 3: source interval is larger than the configured interval (60s delta vs 10s delta)
		// 4: mixed source intervals (some 60s deltas, some 10s deltas)
		truncateTimestamps(metric, transform.Interval)
	}
