...
func TestTraceIsDispatchedAfterDuration(t *testing.T) {
	...
	p := newGroupByTraceProcessor(processortest.NewNopSettingsWithType(metadata.Type), mockProcessor, config)
...
func TestProcessorCapabilities(t *testing.T) {
	...
	// test
	p := newGroupByTraceProcessor(processortest.NewNopSettingsWithType(metadata.Type), consumertest.NewNop(), config)
	st := newMemoryStorage(p.telemetryBuilder)
	p.st = st
	caps := p.Capabilities()
...
func TestAddSpansToExistingTrace(t *testing.T) {
	...
	p := newGroupByTraceProcessor(processortest.NewNopSettingsWithType(metadata.Type), next, config)
	require.NotNil(t, p)
	st := newMemoryStorage(p.telemetryBuilder)
	p.st = st
