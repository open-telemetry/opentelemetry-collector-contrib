...
func sizeOf(v interface{}) int {
	...
	return _sizeOf(reflect.Indirect(reflect.ValueOf(v)), cache)
}

// _sizeOf returns the number of bytes the actual data represented by v occupies in memory.
// If there is an error, _sizeOf returns -1.
func _sizeOf(v reflect.Value, cache map[uintptr]bool) int {
	switch v.Kind() {

	case reflect.Array:
		sum := 0
		for i := 0; i < v.Len(); i++ {
			s := _sizeOf(v.Index(i), cache)
			if s < 0 {
				return -1
			}
			sum += s
	...
	case reflect.Ptr:
		...
		s := _sizeOf(reflect.Indirect(v), cache)
		if s < 0 {
			return -1
		}
	...
	case reflect.Map:
		...
		for i := range keys {
			...
			sv := _sizeOf(val, cache)
			if sv < 0 {
				return -1
			}
			sum += sv
			sk := _sizeOf(keys[i], cache)
			if sk < 0 {
				return -1
			}
			sum += sk
	...
	case reflect.Interface:
		return _sizeOf(v.Elem(), cache) + int(v.Type().Size())
