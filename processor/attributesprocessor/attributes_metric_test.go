...
func TestMetricProcessor_NilEmptyData(t *testing.T) {
	...
	mp, err := factory.CreateMetrics(context.Background(), processortest.NewNopSettingsWithType(metadata.Type), oCfg, consumertest.NewNop())
...
func TestMetricAttributes_Convert(t *testing.T) {
	...
	tp, err := factory.CreateMetrics(context.Background(), processortest.NewNopSettingsWithType(metadata.Type), cfg, consumertest.NewNop())
	require.NoError(t, err)
	require.NotNil(t, tp)
