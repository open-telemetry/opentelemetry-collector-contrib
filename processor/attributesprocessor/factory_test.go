...
func TestFactoryCreateTraces_InvalidActions(t *testing.T) {
	...
	ap, err := factory.CreateTraces(context.Background(), processortest.NewNopSettingsWithType(metadata.Type), cfg, consumertest.NewNop())
	assert.Error(t, err)
	assert.Nil(t, ap)
	// Invalid target type
	oCfg.Actions = []attraction.ActionKeyValue{
	...
	ap2, err2 := factory.CreateTraces(context.Background(), processortest.NewNopSettingsWithType(metadata.Type), cfg, consumertest.NewNop())
...
func TestFactoryCreateLogs_InvalidActions(t *testing.T) {
	...
	ap, err := factory.CreateLogs(context.Background(), processortest.NewNopSettingsWithType(metadata.Type), cfg, consumertest.NewNop())
	assert.Error(t, err)
	assert.Nil(t, ap)
}
...
func TestFactoryCreateLogs(t *testing.T) {
	...
	tp, err := factory.CreateLogs(
		context.Background(), processortest.NewNopSettingsWithType(metadata.Type), cfg, consumertest.NewNop())
	assert.NotNil(t, tp)
	assert.NoError(t, err)
