...
This condition translates to: For each span event, check if its parent span's resource attribute `host.name` is `localhost` and if the span event's name is `grpc.timeout`.
