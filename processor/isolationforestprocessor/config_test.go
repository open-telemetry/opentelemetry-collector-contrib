...
func TestConfigurationValidation(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "contamination rate - smallest valid value",
			modifyConfig: func(cfg *Config) { cfg.ContaminationRate = 0.001 },
			expectError:  false,
		},
		{
			name:         "contamination rate - largest valid value",
			modifyConfig: func(cfg *Config) { cfg.ContaminationRate = 0.999 },
			expectError:  false,
		},
		{
			name:          "invalid contamination rate - zero is degenerate",
			modifyConfig:  func(cfg *Config) { cfg.ContaminationRate = 0.0 },
			expectError:   true,
			errorContains: "contamination_rate must be greater than 0.0 and less than 1.0",
		},
		{
			name:          "invalid contamination rate - one is degenerate",
