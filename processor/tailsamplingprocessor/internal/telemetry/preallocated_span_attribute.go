...
type DecisionAttributes struct {
	...
	attrs       [samplingpolicy.NumDecisions]attribute.KeyValue // Array indexed by Decision
...
// for all decision types defined in samplingpolicy.Decision.
// Parameters:
...
func NewDecisionAttributes(attrKey attribute.Key) *DecisionAttributes {
	...
	// Pre-compute all decision attributes
...
func (d *DecisionAttributes) Get(decision samplingpolicy.Decision) attribute.KeyValue {
	if decision < 0 || decision >= samplingpolicy.NumDecisions {
