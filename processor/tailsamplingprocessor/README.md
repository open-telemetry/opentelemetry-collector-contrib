...
- The `rate_limiting` and `bytes_limiting` policies report the sampling threshold they actually applied, instead of always-sample, when configured as top-level policies. The underlying token bucket is unchanged — same continuous refill, same `burst_capacity` — so the limit is still respected exactly, not just on average. When there's enough budget for every trace up for a decision, nothing is throttled; when there isn't, the highest-randomness traces are kept (the same selection a probabilistic sampler would make), and the reported threshold matches that selection so adjusted counts stay correct. Nesting either policy inside another policy (for example `and` or `composite`) does not currently get this treatment and falls back to today's always-sample reporting.
