...
- The `rate_limiting` and `bytes_limiting` policies report an estimated threshold instead of always-sample. Each decision tick, they evaluate the traces eligible for a decision as a group: they spend the available budget (spans for `rate_limiting`, bytes for `bytes_limiting`) on the traces with the highest randomness first and set `th` at the point where the budget runs out, so kept traces carry a threshold reflecting the effective sampling probability. When the whole group fits within the budget nothing is limited and the threshold is `th=0` / always-sample. This keeps adjusted counts correct when the limiter is dropping data. This also applies when either policy is nested inside an `and` policy (for example, rate-limiting only the traces that match an attribute filter): the budget is spent on the traces that pass the `and`'s other sub-policies.
