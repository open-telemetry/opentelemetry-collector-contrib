...
- The `rate_limiting` and `bytes_limiting` policies stop being hard token buckets and become probabilistic limiters that report the threshold they applied. Each policy measures its arrival rate over a one second window and converts the configured limit into the share of arriving traces it must keep: at an observed rate `R` with limit `L` that share is `L/R`, which admits `L` per second on average. That share is relative to what arrives, so it is composed with each trace's incoming sampling probability — a trace that arrives already sampled at `p` is judged against the threshold for `p * L/R`, so it passes with conditional probability `L/R`. Because the composed threshold is both what the trace is judged by and what is reported as `th`, kept traces carry the probability actually applied to them and adjusted counts stay correct. Three consequences are worth understanding before enabling the gate:
  - The limit is respected **on average, not exactly**. The share comes from the previous window, so a sudden traffic spike is not tracked until the next window rolls, and short-term output can exceed the configured limit. Burst behavior is no longer the `burst_capacity` token bucket; that setting only applies with the gate disabled.
  - **The first window after startup, and after any idle period longer than the window, is not limited.** The policy has no measurement to act on, so it keeps everything until the first window closes. A token bucket instead capped that initial burst at `burst_capacity`.
  - Because the decision is a function of the trace's randomness, the *same* traces are kept from window to window rather than an arbitrary subset. This is what makes the sampling consistent with other samplers in the pipeline, but it does mean low-randomness traces are systematically dropped while the limiter is engaged.
