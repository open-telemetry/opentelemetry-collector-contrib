	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i, res := range tt.detectedResources {
				...
				mockDetectors[mockDetectorType] = func(component.ComponentSettings, DetectorConfig) (Detector, error) {
					return md, nil
				}
			...
			p, err := f.CreateResourceProvider(component.ComponentSettings{Logger: zap.NewNop()}, time.Second, &mockDetectorConfig{}, mockDetectorTypes...)
...
func TestDetectResource_InvalidDetectorType(t *testing.T) {
	...
	_, err := p.CreateResourceProvider(component.ComponentSettings{Logger: zap.NewNop()}, time.Second, &mockDetectorConfig{}, mockDetectorKey)
...
func TestDetectResource_DetectoryFactoryError(t *testing.T) {
	mockDetectorKey := DetectorType("mock")
	p := NewProviderFactory(map[DetectorType]DetectorFactory{
		mockDetectorKey: func(component.ComponentSettings, DetectorConfig) (Detector, error) {
