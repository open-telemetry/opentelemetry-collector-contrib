...
func TestMetrics_DefaultRouteIsUsedWhenRouteCantBeDetermined(t *testing.T) {
	...
	rm.InstrumentationLibraryMetrics().AppendEmpty().Metrics().AppendEmpty().SetDataType(pdata.MetricDataTypeGauge)
	...
	assert.Equal(t, 0, mExp.getMetricCount(),
...
func TestMetrics_DataIsProperlyRoutedPerResourceAttribute(t *testing.T) {
	defaultExp := &mockMetricsExporter{}
	mExp := &mockMetricsExporter{}
	...
	ctx := context.Background()
	require.NoError(t, exp.Start(ctx, host))
	assert.NoError(t, exp.ConsumeMetrics(ctx, m))
...
func TestLogs_DefaultRouteIsUsedWhenRouteCantBeDetermined(t *testing.T) {
	defaultExp := &mockLogsExporter{}
	mExp := &mockLogsExporter{}
	...
	ctx := context.Background()
	require.NoError(t, exp.Start(ctx, host))
	assert.NoError(t, exp.ConsumeLogs(ctx, l))

	assert.Equal(t, 1, defaultExp.getLogCount(),
		"metric should be routed to default exporter",
	)

	assert.Equal(t, 0, mExp.getLogCount(),
		"metric should not be routed to non default exporter",
	)
}

func TestLogs_DataIsProperlyRoutedPerResourceAttribute(t *testing.T) {
	defaultExp := &mockLogsExporter{}
	mExp := &mockLogsExporter{}
	...
	assert.Equal(t, 1, mExp.getLogCount(),
