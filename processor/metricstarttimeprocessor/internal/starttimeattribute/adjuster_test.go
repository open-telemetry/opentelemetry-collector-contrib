...
func (m *mockPodClient) GetPodStartTime(_ context.Context, podID podIdentifier) time.Time {
	key := podID.Value
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			adjuster, err := NewAdjusterWithFactory(component.TelemetrySettings{Logger: componenttest.NewNopTelemetrySettings().Logger},
				func(context.Context, k8sconfig.APIConfig, informerFilter, bool) (podClient, error) {
					return mockClient, nil
				}, AttributesFilterConfig{}, false, common.AdjustmentOptions{
					Filter:         metricNameFilter,
					SkipIfCTExists: !tt.expectAdjusted,
				})
			require.NoError(t, err)
			adjustedMetrics, err := adjuster.AdjustMetrics(t.Context(), metrics)
			require.NoError(t, err)
...
func TestExtractPodIdentifier(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := extractPodIdentifier(attrs)
...
func TestIsCumulativeMetric(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := isCumulativeMetric(metric)
