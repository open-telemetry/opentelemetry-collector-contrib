...
func TestLoadDeprecatedConfig(t *testing.T) {
	...
	tests := []struct {
		...
		{
			id: component.NewIDWithName(metadata.DeprecatedType, "all"),
			expected: &Config{
				MaxStale:   1 * time.Minute,
				MaxStreams: 10,
			},
		...
		{
			id: component.NewIDWithName(metadata.DeprecatedType, "set-valid-max_stale"),
			expected: &Config{
				MaxStale:   2 * time.Minute,
				MaxStreams: math.MaxInt,
			},
		...
		{
			id: component.NewIDWithName(metadata.DeprecatedType, "set-valid-max_streams"),
			expected: &Config{
				MaxStale:   5 * time.Minute,
				MaxStreams: 20,
			},
