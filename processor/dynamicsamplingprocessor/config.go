...
type SamplerConfig struct {
	...
	// Fingerprint is the list of attribute names that identify what kind of
...
func (s *SamplerConfig) validate(ruleName string) error {
	...
	case DynamicPercentage:
		...
		if len(s.Fingerprint) == 0 {
			return fmt.Errorf("rule %q: fingerprint must contain at least one entry", ruleName)
		}
...
func (s *SamplerConfig) rejectUnusedFields(ruleName, typeName string, allowed map[string]bool) error {
	...
	if err := set("fingerprint", len(s.Fingerprint) > 0); err != nil {
		return err
	}
