...
import (
	...
	"github.com/open-telemetry/opentelemetry-collector-contrib/processor/filterhelper"
...
func (ma AttributesMatcher) Match(attrs pdata.AttributeMap) bool {
	fmt.Println("filterLog: skipLog MatchLogRecord PropertiesMatcher AttributesMatcher")
	...
	fmt.Println("filterLog: skipLog MatchLogRecord PropertiesMatcher AttributesMatcher skip length")

	// Check that all expected properties are set.
	for _, property := range ma {
		attr, exist := attrs.Get(property.Key)
		fmt.Println(fmt.Sprintf("filterLog: skipLog MatchLogRecord PropertiesMatcher AttributesMatcher attr-key %s \n", attr))
		if !exist {
			return false
		}
		...
		if property.StringFilter != nil {
			...
			if !attr.Equal(*property.AttributeValue) {
				fmt.Println("filterLog: skipLog MatchLogRecord PropertiesMatcher AttributesMatcher attr-value false")
				return false
			}
		}
	}
	fmt.Println("filterLog: skipLog MatchLogRecord PropertiesMatcher AttributesMatcher attr-value true")
	return true
}
