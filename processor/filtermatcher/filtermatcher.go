...
func (mp *PropertiesMatcher) Match(attributes pdata.AttributeMap, resource pdata.Resource, library pdata.InstrumentationLibrary) bool {
	fmt.Println("filterLog: skipLog MatchLogRecord Match")
	...
	fmt.Println("filterLog: skipLog MatchLogRecord PropertiesMatcher skip libraries")

	if mp.resources != nil && !mp.resources.Match(resource.Attributes()) {
		return false
	}

	fmt.Println("filterLog: skipLog MatchLogRecord PropertiesMatcher skip resource")
