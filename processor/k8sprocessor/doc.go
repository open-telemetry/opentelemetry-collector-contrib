...
//The config for associating the data passing through the processor (spans, metrics and logs) with specific Pod/Namespace
//annotations/labels are configured via "annotations"  and "labels" keys.
//It represents a list of annotations/labels that are extracted from pods/namespaces and added to spans, metrics and logs.
...
//"from" has only two possible values "pod" and "namespace" and defaults to "pod" if none is specified.
//
