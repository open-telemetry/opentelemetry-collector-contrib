...
func TestToolCallEventOpenLLMetry(t *testing.T) {
	cfg := &Config{Profiles: []string{"openllmetry"}, RemoveOriginals: true}
	sink := new(consumertest.TracesSink)
	p, err := createTracesProcessor(context.Background(), processortest.NewNopSettings(component.MustNewType(typeStr)), cfg, sink)
	if err != nil {
	...
	outSpan := sink.AllTraces()[0].ResourceSpans().At(0).ScopeSpans().At(0).Spans().At(0)
	assertAttrStr(t, outSpan.Attributes(), "gen_ai.request.model", "gpt-4o")
	assertAttrStr(t, outSpan.Attributes(), "gen_ai.operation.name", "chat")

	outEvt := outSpan.Events().At(0).Attributes()
	assertAttrStr(t, outEvt, "gen_ai.tool.definitions", `[{"name":"get_weather"}]`)
	// finish_reason should be wrapped into string[] via WrapSlice
	v, ok := outEvt.Get("gen_ai.response.finish_reasons")
	if !ok {
		t.Fatal("expected gen_ai.response.finish_reasons on event")
	}
	if v.Slice().Len() != 1 || v.Slice().At(0).Str() != "tool_calls" {
		t.Errorf("expected [tool_calls], got %v", v.AsRaw())
	}
}
