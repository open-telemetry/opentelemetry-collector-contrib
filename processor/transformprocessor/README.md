...
The Transform Processor operates only on the telemetry flowing through the pipeline and has no
external service or system dependencies. Its behavior is defined entirely by the OTTL functions,
paths, and enums it exposes, which are versioned together with the processor in each OpenTelemetry
Collector release. There are no external dependency versions to track for compatibility.

Configuration syntax and OTTL semantics follow the [OpenTelemetry Collector's versioning and
stability policies](https://github.com/open-telemetry/opentelemetry-collector/blob/main/docs/component-stability.md).
Breaking changes to configuration or OTTL behavior are gated behind [feature gates](#feature-gate)
and announced in the [changelog](https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/CHANGELOG.md).

## Performance and scaling

The Transform Processor is stateless across batches: each incoming batch is processed independently,
so the processor requires no coordination and can be scaled horizontally by running additional
Collector replicas. The `shared_cache` option and OTTL's `cache` only hold state for the duration of
a single execution and are never shared across batches or replicas.

CPU and memory cost scale with the number and complexity of the statements you configure and with the
volume of telemetry flowing through the pipeline. To keep the processor efficient:
...
The following metrics are emitted for the processor:
...
The amount of detail the processor logs is controlled by the Collector's
[`service::telemetry::logs::level`](https://opentelemetry.io/docs/collector/internal-telemetry/#configure-internal-logs)
setting:

- At `info` (the default), the processor logs no per-statement information. Errors are logged
  according to the configured [`error_mode`](#general-config): `ignore` logs each failed statement or
  condition at `warn` level, `silent` suppresses them entirely, and `propagate` returns the error up
  the pipeline (where the payload is dropped and the failure is surfaced by the pipeline rather than
  by the processor).
- At `debug`, OTTL additionally logs the details it uses to evaluate your statements: the OTTL context
  it infers for each statement group, the `TransformContext` before a statement sequence runs and
  after every individual statement executes, and the result of each condition evaluation. This is very
  verbose but is the most useful level for debugging why a statement is or isn't behaving as expected,
  since it shows exactly how OTTL sees the underlying data. See [Troubleshooting](#troubleshooting) for
