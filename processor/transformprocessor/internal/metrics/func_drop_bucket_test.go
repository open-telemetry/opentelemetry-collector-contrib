...
func Test_DropBucket(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			exprFunc := dropBucketFunc(tt.pattern)
...
func Test_CreateDropBucketFunction(t *testing.T) {
	tests := []struct {
		name    string
		args    ottl.Arguments
		wantErr bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := createDropBucketFunction(ottl.FunctionContext{}, tt.args)
			if tt.wantErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
...
func Test_DropBucketMultipleDataPoints(t *testing.T) {
	...
	exprFunc := dropBucketFunc(pattern)
	_, err := exprFunc(context.Background(), target)
	assert.NoError(t, err)
