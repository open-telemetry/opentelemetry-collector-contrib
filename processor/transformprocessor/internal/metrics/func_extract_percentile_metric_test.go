...
func getConfiguredSummaryMetric(config summaryConfig) pmetric.Metric {
	metricInput := pmetric.NewMetric()
	...
	for quantile, value := range config.quantiles {
		qv := input.QuantileValues().AppendEmpty()
...
func Test_extractPercentileMetric_Summary(t *testing.T) {
	tests := []struct {
		name          string
		...
		{
			name: "p75 not found - should error",
			summaryConfig: summaryConfig{
				name:  "metric",
				count: 100,
				quantiles: map[float64]float64{
			...
			percentile: 75.0,
			wantError:  "percentile p75 not found in summary metric",
		},
		{
			name: "p90 with custom suffix",
			summaryConfig: summaryConfig{
				name:  "custom_metric",
				count: 500,
				quantiles: map[float64]float64{
			...
			percentile: 90.0,
			suffix:     ottl.NewTestingOptional("_percentile_90"),
			wantSuffix: "_percentile_90",
			wantValue:  85.0,
		},
		{
			name: "single quantile p50",
			summaryConfig: summaryConfig{
				name:  "simple",
				count: 200,
				quantiles: map[float64]float64{
		...
		},
		{
			...
			summaryConfig: summaryConfig{
				...
				startTime: ottl.NewTestingOptional[uint64](3000000000),
				timestamp: ottl.NewTestingOptional[uint64](4000000000),
			},
			percentile: 95.0,
			wantSuffix: "_p95",
			wantValue:  500.0,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			metric := getConfiguredSummaryMetric(tt.summaryConfig)
...
func validatePercentileMetric(t *testing.T, tCtx *ottlmetric.TransformContext, originalMetric pmetric.Metric, expectedSuffix string, expectedValue float64) {
	...
	case pmetric.MetricTypeSummary:
		originalDataPoint = originalMetric.Summary().DataPoints().At(0)
	}
