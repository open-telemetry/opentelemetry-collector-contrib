...
func (pc parserCollection) parseCommonContextStatements(contextStatement ContextStatements) (baseContext, error) {
	...
	case Resource:
		...
		rStatements := ottlresource.NewStatementSequence(parsedStatements, pc.errorMode, pc.settings)
	...
	case Scope:
		...
		sStatements := ottlscope.NewStatementSequence(parsedStatements, pc.errorMode, pc.settings)
