...
func (pc MetricParserCollection) ParseContextStatements(contextStatements ContextStatements) (consumer.Metrics, error) {
	...
	case Metric:
		...
		mStatements := ottlmetric.NewStatementSequence(parseStatements, pc.errorMode, pc.settings)
	...
	case DataPoint:
		...
		dpStatements := ottldatapoint.NewStatementSequence(parsedStatements, pc.errorMode, pc.settings)
