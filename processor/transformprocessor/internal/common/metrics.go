...
type MetricParserCollection ottl.ParserCollection[ContextStatements, consumer.Metrics]

type MetricParserCollectionOption ottl.ParserCollectionOption[ContextStatements, consumer.Metrics]

func WithMetricParser(functions map[string]ottl.Factory[ottlmetric.TransformContext]) MetricParserCollectionOption {
	return func(pc *ottl.ParserCollection[ContextStatements, consumer.Metrics]) error {
		metricParser, err := ottlmetric.NewParser(functions, pc.Settings, ottlmetric.WithPathContextNames())
		if err != nil {
			return err
		}
...
func convertMetricStatements(pc *ottl.ParserCollection[ContextStatements, consumer.Metrics], _ *ottl.Parser[ottlmetric.TransformContext], _ string, statements ContextStatements, parsedStatements []*ottl.Statement[ottlmetric.TransformContext]) (consumer.Metrics, error) {
	globalExpr, errGlobalBoolExpr := parseGlobalExpr(filterottl.NewBoolExprForMetric, statements.Conditions, pc.ErrorMode, pc.Settings, filterottl.StandardMetricFuncs())
...
func WithDataPointParser(functions map[string]ottl.Factory[ottldatapoint.TransformContext]) MetricParserCollectionOption {
	return func(pc *ottl.ParserCollection[ContextStatements, consumer.Metrics]) error {
		dataPointParser, err := ottldatapoint.NewParser(functions, pc.Settings, ottldatapoint.WithPathContextNames())
		if err != nil {
			return err
		}
...
func convertDataPointStatements(pc *ottl.ParserCollection[ContextStatements, consumer.Metrics], _ *ottl.Parser[ottldatapoint.TransformContext], _ string, statements ContextStatements, parsedStatements []*ottl.Statement[ottldatapoint.TransformContext]) (consumer.Metrics, error) {
	globalExpr, errGlobalBoolExpr := parseGlobalExpr(filterottl.NewBoolExprForDataPoint, statements.Conditions, pc.ErrorMode, pc.Settings, filterottl.StandardDataPointFuncs())
	if errGlobalBoolExpr != nil {
		return nil, errGlobalBoolExpr
	}
...
func WithMetricErrorMode(errorMode ottl.ErrorMode) MetricParserCollectionOption {
	return MetricParserCollectionOption(ottl.WithParserCollectionErrorMode[ContextStatements, consumer.Metrics](errorMode))
}
...
func NewMetricParserCollection(settings component.TelemetrySettings, options ...MetricParserCollectionOption) (*MetricParserCollection, error) {
	...
	for _, option := range options {
		pcOptions = append(pcOptions, ottl.ParserCollectionOption[ContextStatements, consumer.Metrics](option))
	}
...
func (mpc *MetricParserCollection) ParseContextStatements(contextStatements ContextStatements) (consumer.Metrics, error) {
	pc := ottl.ParserCollection[ContextStatements, consumer.Metrics](*mpc)
