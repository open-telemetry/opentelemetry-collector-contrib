...
func (pc TraceParserCollection) ParseContextStatements(contextStatements ContextStatements) (consumer.Traces, error) {
	...
	case Span:
		...
		sStatements := ottlspan.NewStatementSequence(parsedStatements, pc.errorMode, pc.settings)
	...
	case SpanEvent:
		...
		seStatements := ottlspanevent.NewStatementSequence(parsedStatements, pc.errorMode, pc.settings)
