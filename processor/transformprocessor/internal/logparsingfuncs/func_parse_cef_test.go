...
func Test_parseCEF(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assertMapValue(t, resultMap, "device_vendor", tt.expected["device_vendor"])
			assertMapValue(t, resultMap, "device_product", tt.expected["device_product"])
			assertMapValue(t, resultMap, "device_version", tt.expected["device_version"])
			assertMapValue(t, resultMap, "device_event_class_id", tt.expected["device_event_class_id"])
			assertMapValue(t, resultMap, "name", tt.expected["name"])
			assertMapValue(t, resultMap, "severity", tt.expected["severity"])

			expectedExt := tt.expected["extensions"].(map[string]any)
			extVal, ok := resultMap.Get("extensions")
