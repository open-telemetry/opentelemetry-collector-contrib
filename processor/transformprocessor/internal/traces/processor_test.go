...
func TestProcess(t *testing.T) {
	tests := []struct {
		...
		{
			query: `set(attributes["test"], Join(": ", attributes["http.method"], attributes["http.url"]))`,
			want: func(td ptrace.Traces) {
		...
		{
			query: `set(attributes["test"], Join(": ", attributes["http.method"], attributes["http.url"])) where name == Join("", "operation", "A")`,
			want: func(td ptrace.Traces) {
				td.ResourceSpans().At(0).ScopeSpans().At(0).Spans().At(0).Attributes().InsertString("test", "get: http://localhost/health")
			},
		},
		{
			query: `set(attributes["kind"], Join("", "kind", ": ", kind)) where kind == 1`,
			want: func(td ptrace.Traces) {
