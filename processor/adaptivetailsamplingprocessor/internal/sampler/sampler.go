...
// Package sampler provides sampler implementations used by the adaptive tail sampling
...
// NewEMAPercentage constructs an EMA percentage sampler from the supplied config.
func NewEMAPercentage(cfg EMAPercentageConfig) (Sampler, error) {
	if cfg.GoalSamplingPercentage <= 0 || cfg.GoalSamplingPercentage > 100 {
		return nil, errors.New("adaptive_percentage sampler: goal_percentage must be in (0, 100]")
	}
