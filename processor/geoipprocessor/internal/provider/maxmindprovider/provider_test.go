...
func TestProviderLocation(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			testDatabase: "GeoIP2-City-Test.mmdb",
			expectedAttributes: attribute.NewSet([]attribute.KeyValue{
				attribute.String(string(conventions.GeoContinentCodeKey), "AS"),
				attribute.String(string(conventions.GeoContinentNameKey), "Asia"),
				attribute.String(string(conventions.GeoCountryIsoCodeKey), "KR"),
				attribute.String(string(conventions.GeoCountryNameKey), "South Korea"),
				attribute.String(string(conventions.GeoTimezoneKey), "Asia/Seoul"),
				attribute.Float64(string(conventions.GeoLocationLatKey), 1),
				attribute.Float64(string(conventions.GeoLocationLonKey), 1),
			}...),
		},
	}
