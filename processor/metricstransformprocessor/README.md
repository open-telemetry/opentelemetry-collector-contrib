...
| Split by label key and value  | For label `state` with value `idle`, create a new metric named `system.cpu.usage.idle`          |
...
- The `action` property allows metrics to be:
  ...
  - Create new metrics from existing metrics based name and labels (`splitby`).
    The original matched metric is left unchanged by this action.
  ...
  # transforms is a list of transformations with each element transforming a metric selected by metric name
    transforms:
      ...
      - include: <metric_name>
        ...
        # action specifies if the operations (specified below) are performed on metrics in place (update), on an inserted clone (insert), on a new combined metric (combine), or a new metric extracted from a label(s) (splitby)
        action: {update, insert, combine, splitby}
        
        # SPECIFY HOW TO TRANSFORM THE METRIC GENERATED AS A RESULT OF APPLYING THE ABOVE ACTION
        
        # new_name specifies the updated name of the metric; if action is insert, combine, or splitby, new_name is required
...
### Split by name and labels
```yaml
# Convert the metric `system.cpu.usage` with the attribute `state=idle` to be a new metric named `cpu.idle`
#
# This can be usage for migrating from existing telemetry agents to the open telemetry collector while preserving
# existing names used in dashboards, charts, alerts and runbook. 
include: system.cpu.usage
action: splitby
new_name:  cpu.idle
match_type: strict
experimental_match_labels:
  state: idle
operations:
  # Any operations defined here will be applied to the new metric if matched
  ...
```
