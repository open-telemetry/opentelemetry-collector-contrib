...
| Code coverage | [![codecov](https://codecov.io/github/open-telemetry/opentelemetry-collector-contrib/graph/main/badge.svg?component=processor_metricstransform)](https://app.codecov.io/gh/open-telemetry/opentelemetry-collector-contrib/tree/main/?components%5B0%5D=processor_metricstransform&displayType=list) |
...
> **Note:** The processor type has been renamed from `metricstransform` to `metrics_transform` to follow the
> snake_case naming convention. The old name `metricstransform` is preserved as a deprecated alias and
> will continue to work, but a deprecation warning will be logged at startup. Please update your
> configuration to use `metrics_transform:`.
