...
func (cfg *Config) Validate() error {
	...
	for _, f := range append(cfg.Extract.Labels, cfg.Extract.Annotations...) {
		...
		switch f.From {
		case "", kube.MetadataFromPod, kube.MetadataFromNamespace, kube.MetadataFromNode, kube.MetadataFromDeployment, kube.MetadataFromStatefulSet, kube.MetadataFromDaemonSet, kube.MetadataFromJob, kube.MetadataFromCronJob:
		default:
			return fmt.Errorf("%s is not a valid choice for From. Must be one of: pod, namespace, deployment, statefulset, daemonset, job, cronbjob, node", f.From)
		}
