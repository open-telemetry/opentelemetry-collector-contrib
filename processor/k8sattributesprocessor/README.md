...
are then also available for the use within association rules. Available attributes are:
  ...
  - k8s.node.name
  - k8s.node.uid
  - k8s.owner.kind (the Kind of the pod's controlling owner reference. A pod's [`metadata.ownerReferences`](https://kubernetes.io/docs/reference/kubernetes-api/definitions/owner-reference-v1-meta/#OwnerReference) can list several references, e.g. for garbage collection purposes, but at most one is ever marked as the controller (`controller: true`) - the same single reference client-go's `meta_v1.GetControllerOf` returns. This field comes from that one controlling reference, not the full list. Unlike the fixed fields above, it isn't limited to known controller kinds - it also covers pods owned by third-party CRDs, e.g. Flux's `HelmRelease` or cert-manager's `Challenge`.)
