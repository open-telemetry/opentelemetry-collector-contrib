...
When using default configuration, the following resource attributes are added:

| Attribute | Type | Description | Example |
|-----------|------|-------------|---------|
| `k8s.namespace.name` | string | Kubernetes namespace name | `default`, `production` |
| `k8s.pod.name` | string | Pod name | `my-app-7d8f6c9b5-xyz12` |
| `k8s.pod.uid` | string | Pod unique identifier | `a1b2c3d4-e5f6-7890-abcd-ef1234567890` |
| `k8s.pod.start_time` | string | Pod start time (RFC3339) | `2024-12-02T10:30:00Z` |
| `k8s.deployment.name` | string | Deployment name (if pod is managed by deployment) | `my-app` |
| `k8s.node.name` | string | Node name where pod is running | `node-1` |

### Optional Pod Attributes

These attributes can be added by including them in `extract.metadata`:

| Attribute | Type | Description | Example |
|-----------|------|-------------|---------|
| `k8s.pod.hostname` | string | Pod hostname | `my-app-7d8f6c9b5-xyz12` |
| `k8s.pod.ip` | string | Pod IP address | `10.244.0.15` |

### Workload Owner Attributes

These attributes identify the workload controller managing the pod:

| Attribute | Type | Description | Example |
|-----------|------|-------------|---------|
| `k8s.deployment.name` | string | Deployment name | `my-app` |
| `k8s.deployment.uid` | string | Deployment UID | `abc123...` |
| `k8s.replicaset.name` | string | ReplicaSet name | `my-app-7d8f6c9b5` |
| `k8s.replicaset.uid` | string | ReplicaSet UID | `def456...` |
| `k8s.statefulset.name` | string | StatefulSet name | `my-stateful-app` |
| `k8s.statefulset.uid` | string | StatefulSet UID | `ghi789...` |
| `k8s.daemonset.name` | string | DaemonSet name | `my-daemon` |
| `k8s.daemonset.uid` | string | DaemonSet UID | `jkl012...` |
| `k8s.job.name` | string | Job name | `my-job` |
| `k8s.job.uid` | string | Job UID | `mno345...` |
| `k8s.cronjob.name` | string | CronJob name (parent of Job) | `my-cronjob` |
| `k8s.cronjob.uid` | string | CronJob UID | `pqr678...` |

### Container Attributes

These attributes require `container.id` or `k8s.container.name` in incoming telemetry:

| Attribute | Type | Description | Example |
|-----------|------|-------------|---------|
| `k8s.container.name` | string | Container name | `app-container` |
| `container.id` | string | Container ID | `docker://abc123...` |
| `container.image.name` | string | Container image name | `my-app` |
| `container.image.tag` | string | Container image tag | `v1.2.3` |
| `container.image.repo_digests` | []string | Repository digests | `["sha256:abc..."]` |
...
| `k8s.node.uid` | string | Node UID | `node-uid-123` |
| `k8s.cluster.uid` | string | Cluster UID (from kube-system namespace) | `cluster-abc-123` |
...
| Attribute | Type | Description | Example |
|-----------|------|-------------|---------|
| `service.namespace` | string | Logical namespace (from k8s namespace) | `production` |
| `service.name` | string | Service name (from pod labels/annotations) | `my-app` |
| `service.version` | string | Service version (from container image tag) | `v1.2.3` |
| `service.instance.id` | string | Unique instance identifier (pod UID) | `a1b2c3d4-e5f6-7890-abcd-ef1234567890` |
...
The processor exposes the following metrics for monitoring its operation:

### Metrics

| Metric | Type | Description |
|--------|------|-------------|
| `otelcol_processor_k8sattributes_ip_lookup_miss` | Counter | Number of times pod lookup by IP failed |
| `otelcol_processor_k8sattributes_pod_added` | Counter | Number of pods added to cache |
| `otelcol_processor_k8sattributes_pod_deleted` | Counter | Number of pods deleted from cache |
| `otelcol_processor_k8sattributes_pod_updated` | Counter | Number of pod updates processed |

These metrics can be used to:
- **Monitor cache health**: High `ip_lookup_miss` indicates association issues
- **Track pod churn**: Monitor `pod_added` and `pod_deleted` rates
- **Debug issues**: Correlate lookup misses with telemetry data gaps
