...
- Deployment names are derived from ReplicaSet names by trimming the pod-template-hash suffix. If `k8s.deployment.uid` is included in the `extract metadata` section, or deployment labels or annotations are being extracted (i.e. any `extract.labels` or `extract.annotations` rule with `from: deployment`), then the Deployment/ReplicaSet informers are started, and the deployment name is resolved from the Kubernetes API instead.
...
- **`otelcol.k8s.watcher.pod.added`** / **`otelcol.k8s.watcher.pod.added`**: Track pod churn rates
