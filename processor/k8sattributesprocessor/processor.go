...
func (kp *kubernetesprocessor) processResource(ctx context.Context, resource pcommon.Resource, signalType string) {
	...
	telemetryAttr := metric.WithAttributes(attribute.String("k8s_telemetry_type", signalType))
	initialAttrCount := resource.Attributes().Len()
	...
	if podIdentifierValue.IsNotEmpty() {
		if pod, podFound = kp.kc.GetPod(podIdentifierValue); podFound {
			...
			if kp.telemetry != nil {
				kp.telemetry.ProcessorK8sattributesPodAssociationSuccess.Add(ctx, 1, telemetryAttr)
			}
			...
			if kp.telemetry != nil {
				kp.telemetry.ProcessorK8sattributesPodAssociationError.Add(ctx, 1, telemetryAttr)
			}
		}
	...
	// Record the number of resource attributes added
	finalAttrCount := resource.Attributes().Len()
	if finalAttrCount > initialAttrCount && kp.telemetry != nil {
		kp.telemetry.ProcessorK8sattributesResourceAttributesAdded.Add(ctx, int64(finalAttrCount-initialAttrCount), telemetryAttr)
	}
}
