...
// It returns successful IP/hostname. Empty slice from server is considered a valid resolution that no need to continue the chain.
...
func (c *ChainResolver) resolveInSequence(resolverFn func(resolver Resolver) ([]string, error)) ([]string, error) {
	...
	for _, r := range c.resolvers {
		...
		// Successful resolution and empty slice from server
		if result != nil {
			return result, nil
		}
