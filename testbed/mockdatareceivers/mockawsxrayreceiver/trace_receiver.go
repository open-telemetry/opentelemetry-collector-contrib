...
func (ar *MockAwsXrayReceiver) handleRequest(ctx context.Context, req *http.Request) error {
	...
	ctx = obsreport.ReceiverContext(ctx, ar.config.ID().String(), transport)
	ctx = obsreport.StartTraceDataReceiveOp(ctx, ar.config.ID().String(), transport)
...
func (ar *MockAwsXrayReceiver) HTTPHandlerFunc(rw http.ResponseWriter, req *http.Request) {
	...
	ctx := obsreport.ReceiverContext(req.Context(), ar.config.ID().String(), "http")
