...
| `UserAddress`             | `client.address`          | Log Attribute |
...
| Azure "properties" Field  | OpenTelemetry                         | OpenTelemetry Scope |
|---------------------------|---------------------------------------|---------------------|
| `CIp`                     | `client.address` + `client.port`. If unparsable -  `client.address.original` | Log Attribute |
| `ComputerName`            | `destination.address`                 | Log Attribute |
...
| `SPort`                   | `destination.port`                    | Log Attribute |
| `TimeTaken`               | `azure.duration`                      | Log Attribute |
...
| `details`                 | `azure.auth.event.details`                | Log Attribute |
| `Result`                  | Body                                      | Log |
| `ServiceEndpoint`         | `azure.service.endpoint`                  | Log Attribute |
...
| Azure "properties" Field  | OpenTelemetry                 | OpenTelemetry Scope |
|---------------------------|-------------------------------|---------------------|
| `trackingReference`       | `azure.ref`                   | Log Attribute |
...
| `socketIp`                | `source.address`              | Log Attribute |
...
| `pop`                     | `azure.pop`                   | Log Attribute |
| `cacheStatus`             | `azure.cache_status`          | Log Attribute |
| `errorInfo`               | `exception.type`              | Log Attribute |
| `endpoint`                | `destination.address` + `destination.port` if it is equal to `backendHostname` or `network.peer.address` + `network.peer.port` otherwise. If unparsable - `destination.address.original` | Log Attribute |
| `isReceivedFromClient`    | `network.io.direction` with value `receive` (if `true`) or `transmit` (if `false`) | Log Attribute |
| `backendHostname`         | `destination.address` + `destination.port`. If unparsable - `destination.address.original` | Log Attribute |
...
| `socketIP`                | `source.address`                  | Log Attribute |
| `requestUri`              | `url.full` with parsed `url.scheme`, `url.domain`, `url.fragment`, `url.query`, `url.path` and `url.port`. If unparsable - only `url.original` | Log Attribute |
| `ruleName`                | `azure.frontdoor.waf.rule.name`   | Log Attribute |
| `policy`                  | `azure.frontdoor.waf.policy.name` | Log Attribute |
| `action`                  | `azure.frontdoor.waf.action`      | Log Attribute |
| `host`                    | `http.request.header.host`        | Log Attribute |
| `trackingReference`       | `azure.ref`                       | Log Attribute |
| `policyMode`              | `azure.frontdoor.waf.policy.mode` | Log Attribute |
...
| `originIP`                        | `server.address` + `server.port`. If unparsable - `server.address.original` | Log Attribute |
