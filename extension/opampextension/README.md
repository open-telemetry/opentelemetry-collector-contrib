...
- `capabilities`: Keys with boolean true/false values that enable a particular OpAMP capability.
  ...
  - `reports_heartbeat`: Whether to enable the OpAMP ReportsHeartbeat capability. When enabled, the server can adjust the agent's heartbeat interval via `ConnectionSettingsOffers`. Default is `true`.
