...
- `capabilities`: Keys with boolean true/false values that enable a particular OpAMP capability.
  ...
  - `reports_heartbeat`: Whether to allow the OpAMP server to configure the agent's heartbeat interval via `ConnectionSettingsOffers`. The default is `false`, unless the `extension.opampextension.HeartbeatIntervalNegotiation` feature gate (alpha) is enabled, in which case the default is `true`.
