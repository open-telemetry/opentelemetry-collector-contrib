...
- `capabilities`: Keys with boolean true/false values that enable a particular OpAMP capability.
  ...
  - `reports_heartbeat`: Whether to allow the OpAMP server to configure the agent's heartbeat interval via `ConnectionSettingsOffers`. Default is `true`. Only takes effect when the `extension.opampextension.HeartbeatIntervalNegotiation` feature gate (alpha) is also enabled.
