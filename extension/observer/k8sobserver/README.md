...
| observe_pod_phases | []string | `["Running"]`    | List of pod phases to observe. Only pods in the listed phases will have endpoints created. Valid values: `Pending`, `Running`, `Succeeded`, `Failed`, `Unknown`. Terminated init containers are visible in `Running` pods (within `container_terminated_ttl`). Include `Pending` only if you need to observe init containers while they're still running. |
| observe_init_containers | bool | `false`          | Whether to report init container endpoints when `observe_pods` is enabled. To observe running init containers, include `Pending` in `observe_pod_phases`. Terminated init containers in running pods are observed for a limited time (see `container_terminated_ttl`). |
| container_terminated_ttl | duration | `15m` | How long after termination a container endpoint remains observable. This applies to both regular containers and init containers. Running containers are always observed regardless of this setting. After this TTL expires, terminated container endpoints are removed, allowing receivers to shut down. This is useful for collecting logs from crashed sidecars or completed init containers. |
...
To observe init containers, enable `observe_init_containers` and include `Pending` in `observe_pod_phases`:
...
- **Running init containers** require `Pending` in `observe_pod_phases` since init containers run while the pod is in `Pending` phase.
- **Terminated containers** (both init and regular) are included only if they terminated within `container_terminated_ttl` (default 15 minutes). After the TTL expires, the endpoint is removed, allowing receivers to shut down. This is useful for collecting logs from crashed sidecars and terminated init containers. Note that for short lived containers (like init containers) you may only really get a chance to observe them in their terminated state.
- Each init container exposes `is_init_container` in the endpoint environment for receiver_creator rules.

#### Example: Collecting logs from init containers

The following example shows how to collect logs from both init containers and regular application containers using the `filelog` receiver:
...
Note: The collector pod must have access to the node's `/var/log/pods` directory, typically mounted as a `hostPath` volume.
