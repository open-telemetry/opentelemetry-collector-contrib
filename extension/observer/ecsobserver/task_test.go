...
func TestTask_PrivateIP(t *testing.T) {
	...
	t.Run("not found", func(t *testing.T) {
		...
		modes := []string{"", string(ecstypes.NetworkModeBridge), string(ecstypes.NetworkModeHost), string(ecstypes.NetworkModeAwsvpc), string(ecstypes.NetworkModeNone), "not even a network mode"}
		for _, mode := range modes {
			task.Definition.NetworkMode = ecstypes.NetworkMode(mode)
