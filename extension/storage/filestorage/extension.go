...
func (lfs *localFileStorage) GetClient(_ context.Context, kind component.Kind, ent component.ID, name string) (storage.Client, error) {
	...
	if lfs.cfg.Recreate {
		if err := os.Rename(absoluteName, filepath.Join(absoluteName, ".backup")); err != nil {
			return nil, fmt.Errorf("error renaming the database. Please remove %s manually: %v", absoluteName, err)
		}
	}
