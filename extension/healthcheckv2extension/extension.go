...
func newExtension(
	...
	// Note, we will wire up the grpc service here in a follow up PR. The
	// extension manages the http and grpc services as separate subcomponents.
...
func (hc *healthCheckExtension) Shutdown(ctx context.Context) error {
	// Preemptively send the stopped event, so it can be seen by watchers before
	// shutdown
