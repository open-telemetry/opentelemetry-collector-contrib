...
* When using the `static` resolver, there's a risk of data loss if one of the defined endpoint targets becomes unavailable and both resiliency option 2 and resiliency option 1 (if enabled) are exhausted. This is due to the deterministic behavior of the load balancing exporter, which routes data based on a predefined routing key. As a result, specific data can only be exported through a single sub-exporter to its assigned endpoint, with no fallback to other sub-exporters. This limitation stems from the nature of the `static` resolver, where endpoints are configured manually and remain fixed. In contrast, resolvers like `dns` and `k8s` automatically update the list of available endpoints, offering greater flexibility and resilience.
