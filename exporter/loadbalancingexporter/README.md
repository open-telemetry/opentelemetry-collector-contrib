...
* The `routing_key` property is used to specify how to route values (spans, logs, or metrics) to exporters based on different parameters. It supports one of the following values:
  ...
  * `randomness`: Routes spans based on their [OpenTelemetry randomness value](https://opentelemetry.io/docs/specs/otel/trace/tracestate-probability-sampling/): the explicit randomness value (`rv`) from the span's W3C tracestate when present, otherwise randomness derived from the trace ID, per the sampling specification. Traces that share an explicit randomness value (for example the traces of one session, conversation, or workflow, stamped with a common `rv` at their roots) are routed to the same backend, so group-level tail decisions see the whole group. Traffic without an explicit randomness value keeps per-trace locality at the same grouping granularity as `traceID` routing, but with a different distribution: the hash input is the trace ID's low 56 bits (the W3C randomness portion) rather than all 16 bytes, so it assumes trace IDs carry a conformant random suffix. Trace ID generators that don't randomize the low bytes will distribute unevenly across backends, and should use `traceID` routing instead. The sampling specification requires the randomness value to be constant across all spans of a trace, and this exporter relies on that contract rather than enforcing it (the backend is resolved from the first span seen for each trace ID in a batch). Producers that emit inconsistent tracestate within a trace, or drop it on some spans, can cause that trace to route to different backends across batches. Tracestate parse errors are handled at two levels: member- and value-level errors (such as a malformed `th` beside a valid `rv`) keep the members that did parse, so a recovered `rv` still routes the trace, while header-level errors (W3C syntax violations such as an uppercase vendor key, or a header over the 1024-byte size limit) discard the whole tracestate including any `rv`. Traces that fall back to trace ID randomness because of a parse error are counted in `otelcol_loadbalancer_randomness_tracestate_unparseable` (once per trace per batch).
