...
func TestExponentialHistogramToDDSketch(t *testing.T) {
	...
	point.Negative().SetBucketCounts(pcommon.NewImmutableUInt64SliceFromValue(&[]uint64{3, 2, 5}))

	point.Positive().SetOffset(3)
	point.Positive().SetBucketCounts(pcommon.NewImmutableUInt64SliceFromValue(&[]uint64{1, 1, 1, 2, 2, 3}))
