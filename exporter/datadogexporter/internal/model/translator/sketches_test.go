...
func TestHistogramSketches(t *testing.T) {
	...
	fromCDF := func(cdf func(x float64) float64) pmetric.Metrics {
		...
		bounds[N] = float64(N)
		buckets[N+1] = 0
		p.SetExplicitBounds(pcommon.NewImmutableFloat64SliceFromValue(&bounds))
		p.SetBucketCounts(pcommon.NewImmutableUInt64SliceFromValue(&buckets))
		p.SetCount(count)
	...
	tests = append(tests,
		struct {
			...
			getHist: func() pmetric.Metrics {
				...
				p := dp.AppendEmpty()
				p.SetExplicitBounds(pcommon.NewImmutableFloat64SliceFromValue(&[]float64{0, 5_000, 10_000, 15_000,
					20_000}))
				// Points from contrib issue 6129: 0, 5_000, 10_000, 15_000, 20_000
				p.SetBucketCounts(pcommon.NewImmutableUInt64SliceFromValue(&[]uint64{0, 1, 1, 1, 1, 1}))
				p.SetCount(5)
				p.SetSum(50_000)
				return md
			},
		...
		struct {
			...
			getHist: func() pmetric.Metrics {
				...
				bounds := pcommon.NewImmutableFloat64SliceFromValue(&[]float64{0, 5_000, 10_000, 15_000, 20_000})
				for i := 1; i <= 2; i++ {
					p := dp.AppendEmpty()
					p.SetExplicitBounds(bounds)
					cnt := uint64(i)
					p.SetBucketCounts(pcommon.NewImmutableUInt64SliceFromValue(&[]uint64{0, cnt, cnt, cnt, cnt, cnt}))
	...
	for pos, val := range []float64{500, 5_000, 50_000} {
		...
		tests = append(tests, struct {
			...
			getHist: func() pmetric.Metrics {
				...
				m.Histogram().SetAggregationTemporality(pmetric.MetricAggregationTemporalityCumulative)
				bounds := pcommon.NewImmutableFloat64SliceFromValue(&[]float64{1_000, 10_000, 100_000})
				...
				for i := 0; i < 2; i++ {
					...
					p.SetBucketCounts(pcommon.NewImmutableUInt64SliceFromValue(&counts))
					p.SetCount(uint64(i))
...
func TestInfiniteBounds(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			getHist: func() pmetric.Metrics {
				p := pmetric.NewHistogramDataPoint()
				p.SetExplicitBounds(pcommon.NewImmutableFloat64SliceFromValue(&[]float64{}))
				p.SetBucketCounts(pcommon.NewImmutableUInt64SliceFromValue(&[]uint64{100}))
				p.SetCount(100)
				p.SetSum(0)
				return newHistogramMetric(p)
			},
		},
		{
			...
			getHist: func() pmetric.Metrics {
				p := pmetric.NewHistogramDataPoint()
				p.SetExplicitBounds(pcommon.NewImmutableFloat64SliceFromValue(&[]float64{0}))
				p.SetBucketCounts(pcommon.NewImmutableUInt64SliceFromValue(&[]uint64{100, 100}))
				p.SetCount(200)
				p.SetSum(0)
				return newHistogramMetric(p)
			},
		},
		{
			...
			getHist: func() pmetric.Metrics {
				p := pmetric.NewHistogramDataPoint()
				p.SetExplicitBounds(pcommon.NewImmutableFloat64SliceFromValue(&[]float64{-1, 1}))
				p.SetBucketCounts(pcommon.NewImmutableUInt64SliceFromValue(&[]uint64{100, 10, 100}))
				p.SetCount(210)
				p.SetSum(0)
				return newHistogramMetric(p)
			},
