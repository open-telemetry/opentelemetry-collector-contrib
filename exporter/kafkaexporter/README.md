...
- `partition_traces_by_resource_attributes` (default = []): configures the exporter to set the message key of trace messages to a hash of the listed resource attribute values (in the order given). All spans whose resources share the same values for these attributes are routed to the same partition. This keeps a service's spans together even when other resource attributes (host, pod, ...) differ, which is required for stateful, scaled-out consumers such as the [spanmetrics connector](../../connector/spanmetricsconnector/README.md). Mutually exclusive with `partition_traces_by_id` and `traces::message_key_from_metadata_key`; enabling more than one leads to an error.
