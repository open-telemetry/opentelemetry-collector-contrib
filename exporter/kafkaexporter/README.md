...
- `topic` (default = otlp_spans for traces, otlp_metrics for metrics, otlp_logs for logs): The name of the default kafka topic to export to. When used in conjunction with `topic_from_attribute` or `topic_from_context`, this is the default/fallback topic.
- `topic_from_attribute` (default = ""): Specify the resource attribute whose value should be used as the message's topic. This option, when set, will take precedence over the default topic and `topic_from_context`. If `topic_from_attribute` is not set, the message's topic will be set to the value of the configuration option `topic` instead. 
- `topic_from_context` (default = ""): Specify the context key with a string value that should be used as the message's topic. This option, when set, will take precedence over the default topic. If `topic_from_context` is not set to a string value, the message's topic will be set to the value of the configuration option `topic` instead. 
