...
- `propagate_trace_context` (default = false): Adds W3C Trace Context headers (`traceparent`, `tracestate`) for the exporter's span to every record, when the span is sampled. The Kafka receiver links its span to this trace context. When enabled, `traceparent` and `tracestate` headers from `record_headers` or `include_metadata_keys` are dropped.
