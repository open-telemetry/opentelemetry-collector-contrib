...
- `partition_traces_by_resource_attributes` (default = []): configures the exporter to set the message key of trace messages to a hash of the listed resource attribute values, in the order given. Listing keys instead of hashing the whole resource keeps a service's spans on one partition even when attributes such as `host.name` differ, as needed by stateful consumers like the [spanmetrics connector](../../connector/spanmetricsconnector/README.md). Mutually exclusive with `partition_traces_by_id` and `traces::message_key_from_metadata_key`.
