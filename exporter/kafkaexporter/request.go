...
// newRequestConverter builds the converter for the Request-type path.
// It marshals pdata to kgo.Records at request-creation time so the queue
...
func newRequestConverter[T any](exp *kafkaExporter[T]) xexporterhelper.RequestConverterFunc[T] {
	return func(ctx context.Context, data T) (xexporterhelper.Request, error) {
		...
		for partitionKey, sub := range exp.messenger.partitionData(data) {
			...
			if err != nil {
				...
				exp.logger.Error("kafka marshal failed (request path)",
...
// newRequestPusher hands the request's records to the producer.
// Headers (config + context-metadata) are applied by ExportData itself,
// matching the immediate-send path behavior.
func newRequestPusher[T any](exp *kafkaExporter[T]) xexporterhelper.RequestConsumeFunc {
