...
func TestFactory_EnabledBatchingMakesExporterMutable(t *testing.T) {
	...
	me, err := createMetricsExporter(context.Background(), exportertest.NewNopSettingsWithType(metadata.Type), config)
	require.NoError(t, err)
	assert.False(t, me.Capabilities().MutatesData)
	te, err := createTracesExporter(context.Background(), exportertest.NewNopSettingsWithType(metadata.Type), config)
	require.NoError(t, err)
	assert.False(t, te.Capabilities().MutatesData)
	le, err := createLogsExporter(context.Background(), exportertest.NewNopSettingsWithType(metadata.Type), config)
	require.NoError(t, err)
