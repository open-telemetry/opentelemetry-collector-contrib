...
- `heartbeat/startup` (default: false): Check heartbeat at start up time. This action enforces a synchronous heartbeat action during the collector start up sequence, and will fail the collector from starting if the heartbeat returns an error.
