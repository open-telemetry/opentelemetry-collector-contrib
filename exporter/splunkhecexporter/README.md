...
* `log_data_enabled`: LogDataEnabled can be used to disable sending logs by the exporter.

* `profiling_data_enabled`: ProfilingDataEnabled can be used to disable sending profiling data by the exporter.

* `token`: HEC Token is the authentication token provided by Splunk: https://docs.splunk.com/Documentation/Splunk/latest/Data/UsetheHTTPEventCollector.

* `source`: Optional Splunk source: https://docs.splunk.com/Splexicon:Source.
Sources identify the incoming data.

* `sourcetype`: Optional Splunk source type: https://docs.splunk.com/Splexicon:Sourcetype.

* `index`: Splunk index, optional name of the Splunk index.
...
* `disable_compression`: Disable GZip compression. Defaults to false.

* `max_content_length_logs`: Maximum log payload size in bytes. Default value is 2097152 bytes (2MiB).
Maximum allowed value is 838860800 (~ 800 MB).

* `max_content_length_metrics`: Maximum metric payload size in bytes. Default value is 2097152 bytes (2MiB).
Maximum allowed value is 838860800 (~ 800 MB).

* `max_content_length_traces`: Maximum trace payload size in bytes. Default value is 2097152 bytes (2MiB).
Maximum allowed value is 838860800 (~ 800 MB).

* `max_event_size`: Maximum payload size, raw uncompressed. Default value is 5242880 bytes (5MiB).
Maximum allowed value is 838860800 (~ 800 MB).

* `splunk_app_name`: App name is used to track telemetry information for Splunk App's using HEC by App name. Defaults to "OpenTelemetry Collector Contrib".

* `splunk_app_version`: App version is used to track telemetry information for Splunk App's using HEC by App version. Defaults to the current OpenTelemetry Collector Contrib build version.

* `hec_metadata_to_otel_attrs`: HecToOtelAttrs creates a mapping from attributes to HEC specific metadata: source, sourcetype, index and host.

* `otel_to_hec_fields`: HecFields creates a mapping from attributes to HEC fields.

* `health_path`: HealthPath for health API, default is '/services/collector/health'

* `health_check_enabled`: HecHealthCheckEnabled can be used to verify Splunk HEC health on exporter's startup

* `export_raw`: ExportRaw to send only the log's body, targeting a Splunk HEC raw endpoint.

* `use_multi_metric_format`: UseMultiMetricFormat combines metric events to save space during ingestion.

* `heartbeat`: Heartbeat is the configuration to enable heartbeat

* `telemetry`: Telemetry is the configuration for splunk hec exporter telemetry
