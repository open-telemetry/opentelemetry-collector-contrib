...
- blob_name_format: the final blob name will be blob_name
  ...
  - time_parser_ranges (default `nil`): a list of index ranges specifying which substrings of the blob name should be parsed with `time.Format`. Each range is a string in the format `"start-end"` (e.g., `["0-10", "15-25"]`). When set, only the specified ranges are time-formatted; the rest of the blob name remains unchanged. This is useful when `time_parser_enabled` is `true` but you only want certain parts of the name to be time-formatted. Invalid ranges are logged as warnings and skipped.
