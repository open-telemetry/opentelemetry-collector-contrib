...
When a payload contains multiple resource entries, each template is evaluated against a single resource entry (always at index `0`), and entries that render to the same blob name are grouped and uploaded together, one upload per distinct name. Grouping is per resource entry, not per individual log record, metric, or span. Scope- and record-level values (for example `getScopeLogAttr` or `getLogRecord`) are read from the first scope and record of each resource entry, so every record in a resource entry is written to that blob; the exporter logs a warning at startup when a template reads such data. If rendering fails for any resource entry, the whole payload is uploaded once using the default (non-template) name format. Groups are uploaded sequentially within an export request; `sending_queue::num_consumers` controls concurrency between requests. If some groups fail to upload, only the failed groups are returned for retry; groups that uploaded successfully are not re-sent. If the request is canceled mid-way, groups that had not yet started are also returned for retry. Time-based name segments (`time_parser_enabled`) and serial numbers (`serial_num_enabled`) are resolved separately for each blob when it is uploaded, so the blobs from one export request can fall into different time buckets.
