...
func TestStartWarnsWhenTemplateReadsBelowResource(t *testing.T) {
	const warning = "Blob name template reads scope- or record-level data, but blob names are resolved once per resource entry; " +
	...
	for _, tc := range []struct {
		...
		{"resource_attr", pipeline.SignalLogs, `{{ getResourceLogAttr . 0 "service.name" }}.json`, false},
		{"count", pipeline.SignalLogs, `{{ .LogRecordCount }}.json`, false},
		{"resource_chain", pipeline.SignalLogs, `{{ (.ResourceLogs.At 0).Resource.Attributes.Len }}.json`, false},
		{"scope_log_attr", pipeline.SignalLogs, `{{ getScopeLogAttr . 0 0 "scope.name" }}.json`, true},
		{"log_record", pipeline.SignalLogs, `{{ (getLogRecord . 0 0 0).TraceID }}.json`, true},
		{"scope_metric_attr", pipeline.SignalMetrics, `{{ getScopeMetricAttr . 0 0 "scope.name" }}.json`, true},
		{"metric", pipeline.SignalMetrics, `{{ (getMetric . 0 0 0).Name }}.json`, true},
		{"scope_span_attr", pipeline.SignalTraces, `{{ getScopeSpanAttr . 0 0 "scope.name" }}.json`, true},
		{"span", pipeline.SignalTraces, `{{ (getSpan . 0 0 0).Name }}.json`, true},
		{"direct_chain", pipeline.SignalLogs, `{{ ((.ResourceLogs.At 0).ScopeLogs.At 0).Scope.Name }}.json`, true},
		{"variable", pipeline.SignalLogs, `{{ $rl := .ResourceLogs.At 0 }}{{ ($rl.ScopeLogs.At 0).Scope.Name }}.json`, true},
		{"nested_in_if", pipeline.SignalLogs, `{{ if true }}a{{ else }}{{ getScopeLogAttr . 0 0 "x" }}{{ end }}.json`, true},
		{"nested_in_with", pipeline.SignalLogs, `{{ with getLogRecord . 0 0 0 }}{{ .TraceID }}{{ end }}.json`, true},
		{"defined_template", pipeline.SignalLogs, `{{ define "n" }}{{ getScopeLogAttr . 0 0 "x" }}{{ end }}{{ template "n" . }}.json`, true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			cfg := newPartitionTestConfig(`{{ getResourceLogAttr . 0 "service.name" }}.json`, true)
			switch tc.signal {
			case pipeline.SignalLogs:
				cfg.BlobNameFormat.LogsFormat = tc.format
			case pipeline.SignalMetrics:
				cfg.BlobNameFormat.MetricsFormat = tc.format
			case pipeline.SignalTraces:
				cfg.BlobNameFormat.TracesFormat = tc.format
			}
			core, observed := observer.New(zap.WarnLevel)
			exporter := newAzureBlobExporter(cfg, zap.New(core), tc.signal)
			require.NoError(t, exporter.start(t.Context(), componenttest.NewNopHost()))

			warnings := observed.FilterMessage(warning)
			if !tc.wantWarn {
				assert.Zero(t, warnings.Len())
				return
			}
			require.Equal(t, 1, warnings.Len())
			fields := warnings.All()[0].ContextMap()
			assert.Equal(t, tc.signal.String(), fields["signal"])
			assert.Equal(t, tc.format, fields["template"])
		})
	}
}

func TestStartWarnsOnlyForOwnSignalTemplate(t *testing.T) {
	// The metrics and traces templates read record-level data, but a logs
	// exporter only uses the logs template.
	cfg := newPartitionTestConfig(`{{ getResourceLogAttr . 0 "service.name" }}.json`, true)
	cfg.BlobNameFormat.MetricsFormat = `{{ (getMetric . 0 0 0).Name }}.json`
	cfg.BlobNameFormat.TracesFormat = `{{ (getSpan . 0 0 0).Name }}.json`
	core, observed := observer.New(zap.WarnLevel)
	exporter := newAzureBlobExporter(cfg, zap.New(core), pipeline.SignalLogs)
	require.NoError(t, exporter.start(t.Context(), componenttest.NewNopHost()))
...
func TestStartDoesNotWarnWhenTemplateDisabled(t *testing.T) {
	cfg := newPartitionTestConfig(`{{ getScopeLogAttr . 0 0 "scope.name" }}.json`, false)
	core, observed := observer.New(zap.WarnLevel)
	exporter := newAzureBlobExporter(cfg, zap.New(core), pipeline.SignalLogs)
	require.NoError(t, exporter.start(t.Context(), componenttest.NewNopHost()))
	assert.Zero(t, observed.Len())
