...
func TestExporter_pushMetricsData(t *testing.T) {
	...
	t.Run("push failure", func(t *testing.T) {
		...
		exporter := newTestMetricsExporter(t, "tcp://127.0.0.1:9000")
...
func newTestMetricsExporter(t *testing.T, endpoint string, fns ...func(*Config)) *metricsExporter {
	exporter, err := newMetricsExporter(zaptest.NewLogger(t), withTestExporterConfig(fns...)(endpoint))
	require.NoError(t, err)
