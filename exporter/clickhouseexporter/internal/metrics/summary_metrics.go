...
func (s *summaryMetrics) insert(ctx context.Context, db driver.Conn) error {
	...
	defer func(batch driver.Batch) {
		err := batch.Close()
		if err != nil {
			logger.Warn("failed to close summary metrics batch", zap.Error(err))
		}
	}(batch)
	...
	for _, model := range s.summaryModel {
		...
		for i := 0; i < model.summary.DataPoints().Len(); i++ {
			...
			if err != nil {
				return fmt.Errorf("failed to append summary metric: %w", err)
			}
		}
	}
	...
	if err := batch.Send(); err != nil {
		return fmt.Errorf("summary metric insert failed: %w", err)
	}
