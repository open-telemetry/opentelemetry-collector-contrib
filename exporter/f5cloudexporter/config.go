...
type Config struct {
	...
	AuthConfig AuthConfig `mapstructure:"f5_cloud_auth"`
}
...
func (c *Config) sanitize() error {
	...
	if len(c.AuthConfig.CredentialFile) == 0 {
		return fmt.Errorf("missing required \"f5_cloud_auth.credential_file\" setting")
	}

	if _, err := os.Stat(c.AuthConfig.CredentialFile); os.IsNotExist(err) {
		return fmt.Errorf("the provided \"f5_cloud_auth.credential_file\" does not exist")
	}
