		...
		for j := 0; j < libraryMetrics.Len(); j++ {
			...
			for k := 0; k < metrics.Len(); k++ {
				...
				if normalizationError != nil {
					return 0, fmt.Errorf("error generating metric lines: %s", normalizationError.Error())
				}
...
func (e *exporter) send(message string) error {
	...
	req, err := http.NewRequest("POST", e.cfg.API.URL, bytes.NewBufferString(message))
	if err != nil {
		ret := fmt.Errorf("dynatrace error while creating HTTP request: %s", err.Error())
		e.logger.Error(ret.Error())
		return ret
	}

	req.Header.Add("Content-Type", "text/plain; charset=UTF-8")

	if len(e.cfg.API.APIToken) != 0 {
		req.Header.Add("Authorization", "Api-Token "+e.cfg.API.APIToken)
	}
...
func normalizeMetricName(prefix, name string) (string, error) {
	...
	return serialization.NormalizeString(name, maxMetricKeyLen)
}
