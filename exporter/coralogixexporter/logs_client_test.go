...
func TestLogsExporter_PushLogs_WhenCannotSend(t *testing.T) {
	tests := []struct {
		...
		{
			description:        "Rate limit exceeded feature gate enabled",
			featureGateEnabled: true,
		},
		{
			description:        "Rate limit exceeded feature gate disabled",
	...
	for _, tt := range tests {
		t.Run(tt.description, func(t *testing.T) {
			...
			require.NoError(t, featuregate.GlobalRegistry().Set(enableRateLimiterFeatureGateID, tt.featureGateEnabled))
