...
func newSigningRoundTripperWithCredentials(authConfig AuthConfig, creds *credentials.Credentials, next http.RoundTripper) (http.RoundTripper, error) {
	if !isSigv4Requested(authConfig) {
		return next, nil
	}
	if creds == nil {
