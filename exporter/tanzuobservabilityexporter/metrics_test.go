...
func TestCumulativeHistogramDataPointConsumer(t *testing.T) {
	...
	histogramDataPoint.SetExplicitBounds(pcommon.NewImmutableFloat64SliceFromValue(&[]float64{2.0, 5.0, 10.0}))
	histogramDataPoint.SetBucketCounts(pcommon.NewImmutableUInt64SliceFromValue(&[]uint64{5, 1, 3, 2}))
...
func TestCumulativeHistogramDataPointConsumerLeInUse(t *testing.T) {
	...
	mi := metricInfo{Metric: metric, Source: "test_source", SourceKey: "host.name"}
	histogramDataPoint := pmetric.NewHistogramDataPoint()
	histogramDataPoint.SetExplicitBounds(pcommon.NewImmutableFloat64SliceFromValue(&[]float64{10.0}))
	histogramDataPoint.SetBucketCounts(pcommon.NewImmutableUInt64SliceFromValue(&[]uint64{4, 12}))
...
func TestDeltaHistogramDataPointConsumer_OneBucket(t *testing.T) {
	...
	mi := metricInfo{Metric: metric, Source: "test_source", SourceKey: "host.name"}
	// Creates bounds of -Inf to <=2.0; >2.0 to <=5.0; >5.0 to <=10.0; >10.0 to +Inf
	histogramDataPoint.SetExplicitBounds(pcommon.NewImmutableFloat64SliceFromValue(&[]float64{}))
	histogramDataPoint.SetBucketCounts(pcommon.NewImmutableUInt64SliceFromValue(&[]uint64{17}))
...
func TestExponentialHistogramDataPoint(t *testing.T) {
	...
	dataPoint.Negative().SetBucketCounts(pcommon.NewImmutableUInt64SliceFromValue(&[]uint64{15, 16, 17}))
	dataPoint.Positive().SetOffset(3)
	dataPoint.Positive().SetBucketCounts(pcommon.NewImmutableUInt64SliceFromValue(&[]uint64{5, 6, 7, 8}))
	...
	assert.Equal(t, []uint64{0, 17, 16, 15, 2, 5, 6, 7, 8, 0}, h.BucketCounts().Value())
	assert.InDeltaSlice(
		t,
		[]float64{-22.6274, -16.0, -11.3137, -8.0, 2.8284, 4.0, 5.6569, 8.0, 11.3137},
		h.ExplicitBounds().Value(),
		0.0001)
...
func TestExponentialHistogramDataPoint_ZeroOnly(t *testing.T) {
	...
	h := newExponentialHistogramDataPoint(dataPoint)
	assert.Equal(t, []uint64{0, 5, 0}, h.BucketCounts().Value())
	assert.InDeltaSlice(t, []float64{-4.0, 2.0}, h.ExplicitBounds().Value(), 0.0001)
}
