...
func Test_batchTimeMetadata(t *testing.T) {
	// First we will instantiate a dummy TimeSeries instance to pass into both the export call and compare the http request
	md1 := getMetadata(validMetrics1[validSum])
	md2 := getMetadata(validMetrics1[validIntGauge])

	mdMap1 := getMetadataMap([]*prompb.MetricMetadata{})
	mdMap2 := getMetadataMap([]*prompb.MetricMetadata{md1})
	mdMap3 := getMetadataMap([]*prompb.MetricMetadata{md1, md2})
	fmt.Println(mdMap3)
	...
	tests := []struct {
		...
		{
			"two_requests_clubbed_since_larger_batch_size",
			mdMap3,
			300,
			1,
			false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			requests, err := batchMetadata(tt.mdMap, tt.maxBatchByteSize)
			if tt.returnErr {
				assert.Error(t, err)
				return
			}
