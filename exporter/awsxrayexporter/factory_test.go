...
func TestCreateTraces(t *testing.T) {
	...
	exporter, err := factory.CreateTraces(ctx, exportertest.NewNopSettingsWithType(metadata.Type), cfg)
	assert.NoError(t, err)
...
func TestCreateMetrics(t *testing.T) {
	...
	ctx := context.Background()
	exporter, err := factory.CreateMetrics(ctx, exportertest.NewNopSettingsWithType(metadata.Type), cfg)
	assert.Error(t, err)
