...
func TestNew(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := newCarbonExporter(tt.config, component.ComponentSettings{Logger: zap.NewNop()})
			if tt.wantErr {
				assert.Nil(t, got)
				assert.Error(t, err)
				return
...
func TestConsumeMetricsData(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(testName, func(t *testing.T) {
			...
			exp, err := newCarbonExporter(config, component.ComponentSettings{Logger: zap.NewNop()})
			require.NoError(t, err)
