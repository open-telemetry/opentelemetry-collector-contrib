...
type Config struct {
	...
	BackOffConfig  configretry.BackOffConfig                                `mapstructure:"retry_on_failure"` // exporter helper retry settings https://pkg.go.dev/go.opentelemetry.io/collector/exporter/exporterhelper#RetrySettings
...
func (c *Config) checkAndWarnDeprecatedOptions(logger hclog.Logger) {
	...
	if c.QueueMaxLength != 0 {
		...
		c.QueueSettings.GetOrInsertDefault().QueueSize = int64(c.QueueMaxLength)
	}
