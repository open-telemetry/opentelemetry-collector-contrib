...
func newLogzioTracesExporter(config *Config, set exporter.CreateSettings) (exporter.Traces, error) {
	...
	exporter.config.HTTPClientConfig.Endpoint, err = generateEndpoint(config)
	if err != nil {
		return nil, err
	}
...
func (exporter *logzioExporter) start(_ context.Context, host component.Host) error {
	client, err := exporter.config.HTTPClientConfig.ToClient(host, exporter.settings)
	if err != nil {
		return err
	}
...
func (exporter *logzioExporter) pushLogData(ctx context.Context, ld plog.Logs) error {
	...
	err := exporter.export(ctx, exporter.config.HTTPClientConfig.Endpoint, dataBuffer.Bytes())
