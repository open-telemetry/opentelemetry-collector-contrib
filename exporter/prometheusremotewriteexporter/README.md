...
- `wal`: Write-Ahead-Log settings for the exporter.
  ...
  - `segment_cache_size` (default = `2`): Number of WAL segments to be cached in memory. Lower values reduce memory usage at the cost of additional disk reads during WAL replay. Increase this if WAL replay performance is more important than memory footprint.
