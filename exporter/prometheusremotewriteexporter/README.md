...
- `resource_to_telemetry_conversion`
  ...
  - `exclude_service_info` (default = false): If set to `true`, the `service.name` and `service.instance.id` resource attributes, which are already converted to `job` and `instance` labels respectively, will be excluded from the final metrics.
