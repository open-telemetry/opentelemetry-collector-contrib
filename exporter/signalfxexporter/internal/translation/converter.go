...
func NewMetricsConverter(
	...
	enableHistogramBuckets bool) (*MetricsConverter, error) {
	...
	return &MetricsConverter{
		...
		translator:             &signalfx.FromTranslator{},
		enableHistogramBuckets: enableHistogramBuckets,
	}, nil
}
	...
	for i := 0; i < rms.Len(); i++ {
		...
		for j := 0; j < rm.ScopeMetrics().Len(); j++ {
			...
			for k := 0; k < ilm.Metrics().Len(); k++ {
				if !c.enableHistogramBuckets && (ilm.Metrics().At(k).Type() == pmetric.MetricTypeHistogram || ilm.Metrics().At(k).Type() == pmetric.MetricTypeExponentialHistogram) {
					fmt.Println("Disabled histogram metrics")
					continue
				}
