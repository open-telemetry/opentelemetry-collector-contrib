...
func TestNew(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := newSignalFxExporter(tt.config, exportertest.NewNopSettingsWithType(metadata.Type))
			if tt.wantErr {
				require.Error(t, err)
...
func TestConsumeMetrics(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			client, err := cfg.ToClient(context.Background(), componenttest.NewNopHost(), exportertest.NewNopSettingsWithType(metadata.Type).TelemetrySettings)
...
func TestConsumeMetricsWithAccessTokenPassthrough(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			sfxExp, err := NewFactory().CreateMetrics(context.Background(), exportertest.NewNopSettingsWithType(metadata.Type), cfg)
			require.NoError(t, err)
...
func TestConsumeLogsAccessTokenPassthrough(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cfg.AccessTokenPassthrough = tt.accessTokenPassthrough
			cfg.QueueSettings.Enabled = false
			sfxExp, err := NewFactory().CreateLogs(context.Background(), exportertest.NewNopSettingsWithType(metadata.Type), cfg)
			require.NoError(t, err)
			require.NoError(t, sfxExp.Start(context.Background(), componenttest.NewNopHost()))
			defer func() {
				require.NoError(t, sfxExp.Shutdown(context.Background()))
...
func TestNewEventExporter(t *testing.T) {
	got, err := newEventExporter(nil, exportertest.NewNopSettingsWithType(metadata.Type))
	...
	got, err = newEventExporter(cfg, exportertest.NewNopSettingsWithType(metadata.Type))
	assert.NoError(t, err)
	require.NotNil(t, got)
...
func TestSignalFxExporterConsumeMetadata(t *testing.T) {
	...
	rCfg.Realm = "realm"
	exp, err := f.CreateMetrics(context.Background(), exportertest.NewNopSettingsWithType(metadata.Type), rCfg)
	require.NoError(t, err)
...
func TestTLSExporterInit(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			sfx, err := newSignalFxExporter(tt.config, exportertest.NewNopSettingsWithType(metadata.Type))
			assert.NoError(t, err)
