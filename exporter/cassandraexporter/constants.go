...
const (
	...
	//language=SQL
	createLogTableSQL = `CREATE TABLE IF NOT EXISTS %s.%s (TimeStamp DATE, TraceId text, SpanId text, TraceFlags int, SeverityText text, SeverityNumber int, ServiceName text, Body text, ResourceAttributes map<text, text>, LogAttributes map<text, text>), PRIMARY KEY (TraceId)) WITH COMPRESSION = {'class': '%s'}`
	//language=SQL
	insertLogTableSQL = `INSERT INTO %s.%s (timestamp, traceid, spanid, traceflags, severitytext, severitynumber, servicename, resourceattributes, logattributes) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)`
)
