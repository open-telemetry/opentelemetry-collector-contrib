...
func TestIndexResolver_ResolveIndex(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "invalid placeholder - path traversal",
			indexPattern:  "otel-%{service.name}",
			fallback:      "default-service",
			timeFormat:    "",
			resourceAttrs: map[string]string{"service.name": "../../system"},
			expected:      "otel-default-service",
		},
		{
			name:          "invalid placeholder - double dots",
			indexPattern:  "otel-%{service.name}",
			fallback:      "",
			timeFormat:    "",
			resourceAttrs: map[string]string{"service.name": ".."},
			expected:      "otel-unknown",
		},
		{
			name:          "invalid placeholder - leading dot",
			indexPattern:  "otel-%{service.name}",
			fallback:      "fallback",
			timeFormat:    "",
			resourceAttrs: map[string]string{"service.name": ".kibana"},
			expected:      "otel-fallback",
		},
		{
			name:          "invalid placeholder - forbidden char",
			indexPattern:  "otel-%{service.name}",
			fallback:      "fallback",
			timeFormat:    "",
			resourceAttrs: map[string]string{"service.name": "my/service"},
			expected:      "otel-fallback",
		},
		{
			name:          "invalid placeholder - space",
			indexPattern:  "otel-%{service.name}",
			fallback:      "fallback",
			timeFormat:    "",
			resourceAttrs: map[string]string{"service.name": "my service"},
		...
		{
			name:          "fallback on invalid item attribute, resolve to valid scope attribute",
			indexPattern:  "otel-%{service.name}",
			fallback:      "fallback",
			timeFormat:    "",
			resourceAttrs: map[string]string{"service.name": "valid-resource"},
			scopeAttrs:    map[string]string{"service.name": "valid-scope"},
			itemAttrs:     map[string]string{"service.name": "../../invalid-item"},
			expected:      "otel-valid-scope",
		},
	}
