...
func TestConsumeMetrics(t *testing.T) {
	...
	expCfg.MaxRetries = 0
	exp, err := newEmfExporter(expCfg, exportertest.NewNopSettingsWithType(metadata.Type))
...
func TestNewExporterWithoutSession(t *testing.T) {
	exp, err := newEmfExporter(nil, exportertest.NewNopSettingsWithType(metadata.Type))
	assert.Error(t, err)
	assert.Nil(t, exp)
}
