...
| `log_stream_name` | Customized log stream name which supports `{TaskId}`, `{ClusterName}` and `{NodeName}` placeholders. One valid example is `{TaskId}`. It will search for `TaskId` (or `aws.ecs.task.id`) resource attribute in the metrics data and replace with the actual task id. If none of them are found in the resource attribute map, `{TaskId}` will be replaced by `undefined`. Similar way, for the `{ClusterName}` or `{NodeName}` placeholder, it searches for `ClusterName` (or `aws.ecs.cluster.name`) or `NodeName` key in the resource attribute map.                                             |"otel-stream"|
