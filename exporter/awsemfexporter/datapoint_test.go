...
func TestHistogramDataPointSliceAt(t *testing.T) {
	...
	testDP.SetSum(17.13)
	testDP.SetBucketCounts(pcommon.NewImmutableUInt64SliceFromValue(&[]uint64{1, 2, 3}))
	testDP.SetExplicitBounds(pcommon.NewImmutableFloat64SliceFromValue(&[]float64{1, 2, 3}))
	pcommon.NewMapFromRaw(labels).CopyTo(testDP.Attributes())
	...
	for _, tc := range testCases {
		...
		t.Run(tc.testName, func(t *testing.T) {
			...
			case histogramDataPointSlice:
				...
				assert.Equal(t, []float64{0, 10}, dp.ExplicitBounds().Value())
				assert.Equal(t, expectedAttributes, dp.Attributes())
