...
func newElasticsearchExporter(cfg *Config, logger *zap.Logger) (*elasticsearchExporter, error) {
	if err := cfg.ElasticsearchConfig.Validate(); err != nil {
		return nil, err
	}
...
func newElasticsearchClient(logger *zap.Logger, config *Config) (*elasticsearch7.Client, error) {
	...
	transport, err := newElasticsearchTransport(&config.ElasticsearchConfig, tlsCfg)
	if err != nil {
		return nil, err
	}
...
func newElasticsearchTransport(config *ElasticsearchConfig, tlsCfg *tls.Config) (*http.Transport, error) {
...
func createElasticsearchBackoffFunc(config *RetryConfig) func(int) time.Duration {
	if !config.Enabled {
		return nil
	}
