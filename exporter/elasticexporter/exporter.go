...
func newElasticTracesExporter(
	...
	exporter, err := newElasticExporter(cfg.(*Config), componentSettings.Logger)
	if err != nil {
		return nil, fmt.Errorf("cannot configure Elastic APM trace exporter: %v", err)
	}
	return exporterhelper.NewTracesExporter(cfg, componentSettings.Logger, func(ctx context.Context, traces pdata.Traces) error {
		var errs []error
...
func newElasticMetricsExporter(
	...
	return exporterhelper.NewMetricsExporter(cfg, componentSettings.Logger, func(ctx context.Context, input pdata.Metrics) error {
		var errs []error
