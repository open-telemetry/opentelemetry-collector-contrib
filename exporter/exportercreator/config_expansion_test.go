...
func Test_evalBackticksInConfigValue(t *testing.T) {
	type args struct {
		env         map[string]any
		configValue string
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("evalBackticksInConfigValue() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
...
func Test_expandConfig(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"map with regular expression", userConfigMap{
				"one": map[string]any{
		...
		{
			"optional nested field (nil access)", userConfigMap{
				"tls": map[string]any{
					"insecure":             "`spec[\"tls\"][\"insecure\"]`",
					"insecure_skip_verify": "`spec[\"tls\"][\"insecure_skip_verify\"]`",
				},
			}, args{observer.EndpointEnv{"spec": map[string]any{}}}, userConfigMap{}, false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Remove nil values (this is what happens in observerhandler.go)
			if got != nil {
				got = removeNilValuesFromMap(got)
			}
			assert.Equal(t, tt.want, got)
		})
	}
