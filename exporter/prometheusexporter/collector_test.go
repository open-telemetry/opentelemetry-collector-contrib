...
func TestAccumulateCBNHHistograms(t *testing.T) {
	tests := []struct {
		name             string
		metric           func(time.Time, bool) pmetric.Metric
		wantCount        uint64
		wantSum          float64
		wantBucketBounds []float64
		wantBucketCounts []uint64
		wantCreatedTS    bool
		wantNoCreatedTS  bool
	}{
		{
			name:             "CBNH basic (scale -53)",
			wantCount:        15,
			wantSum:          100.0,
			wantBucketBounds: []float64{0, 1, 2},
			wantBucketCounts: []uint64{3, 8, 15},
			metric: func(ts time.Time, withStartTime bool) (metric pmetric.Metric) {
				metric = pmetric.NewMetric()
				metric.SetName("test_cbnh_hist")
				metric.SetEmptyExponentialHistogram().SetAggregationTemporality(pmetric.AggregationTemporalityCumulative)
				dp := metric.ExponentialHistogram().DataPoints().AppendEmpty()
				dp.SetScale(-53) // CBNH
				dp.Positive().SetOffset(0)
				dp.Positive().BucketCounts().FromRaw([]uint64{3, 5, 7})
				dp.SetCount(15)
				dp.SetSum(100.0)
				dp.Attributes().PutStr("label_1", "1")
				dp.Attributes().PutStr("label_2", "2")
		...
		{
			name:             "CBNH with non-zero offset",
			wantCount:        10,
			wantSum:          50.0,
			wantBucketBounds: []float64{2, 3, 4},
			wantBucketCounts: []uint64{1, 5, 10},
			metric: func(ts time.Time, withStartTime bool) (metric pmetric.Metric) {
				metric = pmetric.NewMetric()
				metric.SetName("test_cbnh_hist_offset")
				metric.SetEmptyExponentialHistogram().SetAggregationTemporality(pmetric.AggregationTemporalityCumulative)
				dp := metric.ExponentialHistogram().DataPoints().AppendEmpty()
				dp.SetScale(-53) // CBNH
				dp.Positive().SetOffset(2)
				dp.Positive().BucketCounts().FromRaw([]uint64{1, 4, 5})
				dp.SetCount(10)
				dp.SetSum(50.0)
				dp.Attributes().PutStr("label_1", "1")
				dp.Attributes().PutStr("label_2", "2")
		...
		{
			name:             "CBNH with no sum",
			wantCount:        5,
			wantSum:          0.0,
			wantBucketBounds: []float64{0, 1},
			wantBucketCounts: []uint64{2, 5},
			metric: func(ts time.Time, withStartTime bool) (metric pmetric.Metric) {
				metric = pmetric.NewMetric()
				metric.SetName("test_cbnh_hist_nosum")
				metric.SetEmptyExponentialHistogram().SetAggregationTemporality(pmetric.AggregationTemporalityCumulative)
				dp := metric.ExponentialHistogram().DataPoints().AppendEmpty()
				dp.SetScale(-53) // CBNH
				dp.Positive().SetOffset(0)
				dp.Positive().BucketCounts().FromRaw([]uint64{2, 3})
				dp.SetCount(5)
				// Sum is not set (HasSum() returns false).
				dp.Attributes().PutStr("label_1", "1")
				dp.Attributes().PutStr("label_2", "2")
				dp.SetTimestamp(pcommon.NewTimestampFromTime(ts))
				if withStartTime {
		...
		{
			name:             "CBNH with empty buckets",
			wantCount:        0,
			wantSum:          0.0,
			wantBucketBounds: nil,
			wantBucketCounts: nil,
			metric: func(ts time.Time, withStartTime bool) (metric pmetric.Metric) {
				metric = pmetric.NewMetric()
				metric.SetName("test_cbnh_hist_empty")
				metric.SetEmptyExponentialHistogram().SetAggregationTemporality(pmetric.AggregationTemporalityCumulative)
				dp := metric.ExponentialHistogram().DataPoints().AppendEmpty()
				dp.SetScale(-53) // CBNH
				dp.SetCount(0)
			...
			t.Run(name, func(t *testing.T) {
				...
				for m := range ch {
					...
					if sendTimestamp {
						...
						require.Nil(t, pbMetric.TimestampMs)
						// No start time → no created timestamp for classic histograms.
						require.Nil(t, pbMetric.Histogram.CreatedTimestamp)
					}

					// CBNH should produce a classic histogram, not a native histogram.
					h := pbMetric.Histogram
					require.NotNil(t, h)
					require.Equal(t, tt.wantCount, h.GetSampleCount())
					require.InDelta(t, tt.wantSum, h.GetSampleSum(), 1e-12)

					// Classic histograms should not have native histogram fields.
					require.Equal(t, int32(0), h.GetSchema())
					require.Equal(t, uint64(0), h.GetZeroCount())

					// Verify bucket structure.
					if tt.wantBucketBounds == nil {
						require.Empty(t, h.GetBucket())
					} else {
						require.Len(t, h.GetBucket(), len(tt.wantBucketBounds))
						for i, b := range h.GetBucket() {
							require.InDelta(t, tt.wantBucketBounds[i], b.GetUpperBound(), 1e-12, "bucket %d bound", i)
							require.Equal(t, tt.wantBucketCounts[i], b.GetCumulativeCount(), "bucket %d cumulative count", i)
						}
					}
				}
				require.Equal(t, 1, n)
