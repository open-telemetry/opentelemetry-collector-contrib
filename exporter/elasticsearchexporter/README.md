...
| (`batcher::enabled`: nil) AND no queue                     | Retry                    | If status code is configured in `retry::retry_on_status`, retry         | Complete data loss                                                              | Propagated up the pipeline when all bulk indexer buffers are full  |
