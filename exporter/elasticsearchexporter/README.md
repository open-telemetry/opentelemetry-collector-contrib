...
The following additional options apply only to ECS mapping mode:

- `mapping`:
  - `ecs_span_events_to_logs` (default=false): When enabled, span events are extracted as
    separate ECS-formatted log documents instead of remaining embedded in the parent span
    document. Routing follows the APM data stream convention:
    - Span events named `exception` with `exception.type` or `exception.message` present
      are routed to `logs-apm.error-<namespace>`.
