...
func TestEncodeLogECSMode(t *testing.T) {
	...
	err := resource.Attributes().FromRaw(map[string]any{
		...
		string(semconv.KKey)8SJobName:            "job.name",
		string(semconv.KKey)8SCronJobName:        "cronjob.name",
		string(semconv.KKey)8SStatefulSetName:    "statefulset.name",
		string(semconv.KKey)8SReplicaSetName:     "replicaset.name",
		string(semconv.KKey)8SDaemonSetName:      "daemonset.name",
		string(semconv.KKey)8SContainerName:      "container.name",
