...
// DownsampleEvent runs the downsampling loop over count and calls push for each downsampled
// index where the reduced count is greater than zero.
func DownsampleEvent(count uint16, indexSuffix string, push func(uint16, string) error) error {
	...
	for _, index := range eventIndices {
		...
		if err := push(count, index+indexSuffix); err != nil {
			return err
		}
	}
