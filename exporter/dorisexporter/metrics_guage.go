...
const (
	metricsGaugeTableSuffix = "_gauge"
	metricsGaugeDDL         = `
CREATE TABLE IF NOT EXISTS %s` + metricsGaugeTableSuffix + `
(
...
type metricModelGuage struct {
	data []*dMetricGauge
}

func (m *metricModelGuage) metricType() pmetric.MetricType {
	return pmetric.MetricTypeGauge
}

func (m *metricModelGuage) tableSuffix() string {
	return metricsGaugeTableSuffix
}

func (m *metricModelGuage) add(pm pmetric.Metric, dm *dMetric, e *metricsExporter) error {
	if pm.Type() != pmetric.MetricTypeGauge {
		return fmt.Errorf("metric type is not Guage: %v", pm.Type().String())
	}
...
func (m *metricModelGuage) raw() any {
	return m.data
}

func (m *metricModelGuage) size() int {
	return len(m.data)
}

func (m *metricModelGuage) bytes() ([]byte, error) {
