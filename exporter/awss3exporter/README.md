...
| `endpoint`            | overrides the endpoint used by the exporter instead of constructing it from `region` and `s3_bucket`. Overrides `region` and `s3_bucket` when provided.                                       |             |
