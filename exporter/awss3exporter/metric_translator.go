...
func (mt metricTranslator) translateOTelToParquetMetric(rm *pmetric.ResourceMetrics,
	parquetMetrics *[]*ParquetMetric, config *Config, logger *zap.Logger) {
	...
	for j := 0; j < ilms.Len(); j++ {
		...
		for k := 0; k < metrics.Len(); k++ {
			metric := metrics.At(k)
			addToParquetMetric(&metric, parquetMetrics, instrumentationLibName, logger, mt.metricDescriptor)
		}
	}
